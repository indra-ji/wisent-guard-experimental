...
class WisentLlamaForCausalLM(LlamaForCausalLM):
    ...
    def _initialize_wisent_guard(self):
        ...
        try:
            ...
            guard_kwargs = {
                ...
                "tokenizer": getattr(
                    self, "tokenizer", None
                ),  # Will be set later if not available
            ...
            print("✅ Wisent-guard initialized successfully")
    ...
    def generate(
        ...
        **kwargs,
        ...
        if (
            inputs is not None
            and hasattr(self.wisent_guard, "tokenizer")
            and self.wisent_guard.tokenizer
        ):
            # Decode the input tokens to get the prompt
            prompt = self.wisent_guard.tokenizer.decode(
            ...
            if "max_length" in kwargs and "max_new_tokens" not in kwargs:
                ...
                max_new_tokens = (
                    kwargs["max_length"] - inputs.shape[-1]
                    if inputs is not None
                    else kwargs["max_length"]
                )
            ...
            try:
                ...
                if self.wisent_guard.tokenizer:
                    output_ids = self.wisent_guard.tokenizer.encode(
                ...
                warnings.warn(
                    f"Wisent-guard generation failed, falling back to standard generation: {e}"
                )
