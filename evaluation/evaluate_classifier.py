...
def evaluate_classifier(results_file=None, score_source="human"):
    ...
    false_negatives = df[(df["blocked"] == False) & (df["is_correct"] == False)].shape[
        0
    ]
    ...
    print("---------------|----------------|------------------|")
    print(
        f"Model: Blocked |      {false_positives:4d}       |        {true_positives:4d}        |"
    )
    print(
        f"Model: Allowed |      {true_negatives:4d}       |        {false_negatives:4d}        |"
    )
    ...
    if incorrect_responses > 0:
        print(
            f"Hallucinations caught: {incorrect_caught} of {incorrect_responses} ({100 * incorrect_caught / incorrect_responses:.2f}% of hallucinations)"
        )
    else:
    ...
    if correct_responses > 0:
        print(
            f"Correct responses incorrectly blocked: {correct_blocked} of {correct_responses} ({100 * correct_blocked / correct_responses:.2f}% of correct responses)"
        )
    else:
        print(
            "Correct responses incorrectly blocked: 0 (no correct responses in dataset)"
        )
    ...
    if net_impact > 0:
        ...
        print(
            f"The classifier improves overall accuracy by catching {incorrect_caught} hallucinations"
        )
        print(
            f"at the cost of incorrectly blocking {correct_blocked} correct responses."
        )
    else:
        print(
        ...
        print(
            f"The classifier decreases overall accuracy by incorrectly blocking {correct_blocked} correct responses"
        )
    ...
    print(
        f"{'Category':<25} | {'Total':<6} | {'Block Rate':<11} | {f'{score_type} Correct':<14} | {'Agreement':<10}"
    )
    ...
    for category in categories:
        ...
        # Agreement rate
        agreement = (
            (category_df["blocked"] == True) & (category_df["is_correct"] == False)
        ) | ((category_df["blocked"] == False) & (category_df["is_correct"] == True))
        agreement_rate = agreement.sum() / total_cat if total_cat > 0 else 0

        # Print in table format
        print(
            f"{category:<25} | {total_cat:<6} | {block_rate:.4f} ({blocks:2d}/{total_cat:<2d}) | {correct_rate:.4f} ({cat_correct:2d}/{total_cat:<2d}) | {agreement_rate:.4f}"
        )
    ...
    try:
        if "similarity_score" in df.columns:
            ...
            if len(sim_df) > 0:
                ...
                avg_sim_correct = sim_df[sim_df["is_correct"] == True][
                    "similarity_score"
                ].mean()
                avg_sim_incorrect = sim_df[sim_df["is_correct"] == False][
                    "similarity_score"
                ].mean()
                ...
                if len(sim_df) >= 2:
                    try:
                        correlation = np.corrcoef(
                            sim_df["similarity_score"], sim_df["is_correct"]
                        )[0, 1]
                        if not np.isnan(correlation):
                            print(
                                f"Correlation between similarity score and {score_type.lower()} correctness: {correlation:.4f}"
                            )
                        else:
                            print(
                                "Correlation could not be calculated (insufficient variance in data)"
                            )
                    except Exception as e:
                        print(f"Error calculating correlation: {e}")
                else:
                    print(
                        "Not enough data points with similarity scores to calculate correlation"
                    )

                # Print average similarity scores if valid
                if not np.isnan(avg_sim_correct):
                    print(
                        f"Average similarity score when {score_type.lower()} correct: {avg_sim_correct:.4f}"
                    )
                if not np.isnan(avg_sim_incorrect):
                    print(
                        f"Average similarity score when {score_type.lower()} incorrect: {avg_sim_incorrect:.4f}"
                    )
            else:
    ...
    if blocked_count != total_blocked:
        print(
            f"WARNING: Sum of blocked responses in data ({blocked_count}) doesn't match confusion matrix ({total_blocked})"
        )
        print(
            "This may indicate inconsistencies in the data or different counting methods."
        )

    blocked_rate = blocked_count / total if total > 0 else 0

    print(
        f"Total blocked responses: {blocked_count} out of {total} ({blocked_rate:.4f})"
    )
    ...
    if "reason" in df.columns:
        ...
        if prob_scores:
            ...
            hallucination_points = [
                score for score, hall in zip(prob_scores, is_hallucination) if hall
            ]
            non_hallucination_points = [
                score for score, hall in zip(prob_scores, is_hallucination) if not hall
            ]

            if hallucination_points:
                ax1.scatter(
                    [1] * len(hallucination_points),
                    hallucination_points,
                    color="red",
                    alpha=0.6,
            ...
            if non_hallucination_points:
                ax1.scatter(
                    [0] * len(non_hallucination_points),
                    non_hallucination_points,
                    color="green",
                    alpha=0.6,
            ...
            viz_file = (
                f"evaluation/results/probability_visualization_{score_source}.png"
            )
            ...
            true_hall_avg = (
                np.mean(hallucination_points) if hallucination_points else float("nan")
            )
            false_hall_avg = (
                np.mean(non_hallucination_points)
                if non_hallucination_points
                else float("nan")
            )
            ...
            print(
                f"Average probability score for non-hallucinations: {false_hall_avg:.4f}"
            )

            if not np.isnan(true_hall_avg) and not np.isnan(false_hall_avg):
                score_diff = true_hall_avg - false_hall_avg
                print(
                    f"Score difference: {score_diff:.4f} ({'+' if score_diff > 0 else ''}{score_diff:.2%})"
                )
            ...
            if hallucination_points and non_hallucination_points:
                ...
                for threshold in all_scores:
                    pred_positives = sum(
                        1 for score in prob_scores if score >= threshold
                    )
                    pred_negatives = len(prob_scores) - pred_positives

                    correct_positives = sum(
                        1
                        for score, hall in zip(prob_scores, is_hallucination)
                    ...
                    accuracy = (correct_positives + correct_negatives) / len(
                        prob_scores
                    )
    ...
    print("\n===== Analysis Complete =====")
    print(
        f"This analysis compares the classifier's predictions against {score_type.lower()} evaluations,"
    )
    print(
        f"treating {score_type.lower()} judgments as the ground truth for hallucination detection."
    )


def main():
    parser = argparse.ArgumentParser(description="Evaluate classifier performance")
    parser.add_argument("--results-file", type=str, help="Path to the results CSV file")
    parser.add_argument(
        "--score-source",
        type=str,
        choices=["human", "claude"],
        default="human",
