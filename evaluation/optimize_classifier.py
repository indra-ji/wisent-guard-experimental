...
def extract_token_scores_from_csv(results_file):
    ...
    try:
        ...
        for idx, row in df.iterrows():
            ...
            response = (
                row["guard_response"]
                if "guard_response" in df.columns
                else row["response"]
            )
    ...
    for item in token_data:
        ...
        if strategy == "max":
            ...
            aggregated_score = sum(
                s * w for s, w in zip(similarity_scores, normalized_weights)
            )
        else:
...
def train_evaluate_classifier(
    train_data, test_data, classifier_type="logistic", params=None
):
    """
    ...
    if classifier_type == "logistic":
        ...
        print(
            f"Warning: Unknown classifier type '{classifier_type}', defaulting to logistic"
        )
...
def optimize_classifier_parameters(train_data, test_data, classifier_type="logistic"):
    ...
    best_result = train_evaluate_classifier(
        train_data, test_data, classifier_type, best_params
    )
...
def evaluate_all_strategies(
    token_data, classifier_type="logistic", params=None, output_dir=None
):
    """
...
def visualize_strategy_results(results, output_dir):
    ...
    for strategy in strategies:
        if (
            "pr_curve" in results[strategy]
            and results[strategy]["pr_curve"]["precision"]
        ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Optimize hallucination classifier parameters and token aggregation strategies"
    )
    parser.add_argument(
        "--input-file",
        type=str,
    ...
    if args.single_strategy:
        ...
        if "best_strategy" in results:
            ...
            best_config_file = os.path.join(
                args.output_dir, "best_strategy_config.json"
            )
