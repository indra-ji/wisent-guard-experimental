...
def extract_scores_from_csv(results_file):
    ...
    try:
        ...
        ground_truth_valid = df["human_score"].isin(
            [0, 1]
        )  # Filter out ambiguous (-1) scores
        ...
        print(
            f"Extracted {len(valid_data)} valid data points for threshold optimization"
        )

        # Prepare final data
        data = {
            "probability_scores": valid_data["probability_score"].values,
            "is_correct": valid_data["human_score"].values
            == 1,  # True if correct (not harmful)
            "is_hallucination": valid_data["human_score"].values
            == 0,  # True if incorrect (harmful)
            "categories": valid_data["category"].values
...
def evaluate_threshold(data, threshold):
    ...
    false_positives = np.sum(
        (predictions == True) & (data["is_hallucination"] == False)
    )
    true_negatives = np.sum(
        (predictions == False) & (data["is_hallucination"] == False)
    )
    false_negatives = np.sum(
        (predictions == False) & (data["is_hallucination"] == True)
    )
...
def optimize_threshold(data, metric="f1", threshold_range=None, num_thresholds=50):
    ...
    precision, recall, pr_thresholds = precision_recall_curve(
        data["is_hallucination"], data["probability_scores"]
    )
...
def visualize_optimization_results(results, output_dir):
    ...
    plt.plot(
        df["threshold"], df["balanced_accuracy"], label="Balanced Accuracy", marker="."
    )
    ...
    if results["pr_curve"]["thresholds"]:
        ...
        precision = np.array(
            results["pr_curve"]["precision"][:-1]
        )  # Precision has one more point
    ...
    best_precision = df.loc[
        df["threshold"] == results["threshold"], "precision"
    ].values[0]
    plt.plot(best_recall, best_precision, "go", markersize=8)
    plt.annotate(
        f"Optimal (T={results['threshold']:.2f})",
    ...
    # Add the optimal point
    best_fpr = (
        1 - df.loc[df["threshold"] == results["threshold"], "specificity"].values[0]
    )
    ...
    plt.figure(figsize=(10, 7))
    plt.plot(
        df["threshold"], df["net_impact_rate"], label="Net Impact Rate", marker="."
    )
    ...
    plt.plot(
        df["threshold"],
        df["true_positives"],
        label="True Positives (Hallucinations Caught)",
        marker=".",
    )
    plt.plot(
        df["threshold"],
        df["false_positives"],
        label="False Positives (Correct Responses Blocked)",
        marker=".",
    )
    plt.plot(
        df["threshold"],
        df["false_negatives"],
        label="False Negatives (Hallucinations Missed)",
        marker=".",
    )
    plt.plot(
        df["threshold"],
        df["true_negatives"],
        label="True Negatives (Correct Responses Allowed)",
        marker=".",
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Optimize hallucination classifier threshold"
    )
    parser.add_argument(
        "--input-file",
        type=str,
    ...
    print(
        f"Optimal threshold {results['threshold']:.4f} selected based on {args.metric}."
    )
    ...
    print(
        f"For a net impact of {results['net_impact']} ({results['net_impact_rate']:.2%})"
    )
