...
def run_evaluation_with_layer(
    ...
    cmd = [
        "python",
        "evaluation/evaluate_llama_truthfulqa_classifier.py",
        "--model-name",
        model_name,
        "--eval-data",
        dataset,
    ...
    try:
        results_file = os.path.join(
            output_dir,
            "guard_results_classifier-based.csv"
            if classifier_path
            else "guard_results.csv",
        )
...
def parse_evaluation_output(output, layer):
    ...
    human_correct_match = re.search(
        r"Correct responses \(Human\): (\d+) \(([0-9.]+)%\)", output
    )
    ...
    human_incorrect_match = re.search(
        r"Incorrect responses \(Human\): (\d+) \(([0-9.]+)%\)", output
    )
...
def process_results_file(results_file):
    ...
    try:
        ...
        has_human_scores = (
            "human_score" in df.columns and df["human_score"].isin([0, 1]).any()
        )
        ...
        if has_human_scores:
            ...
            if len(df_valid) > 0:
                ...
                tp = (
                    (df_valid["blocked"] == True)
                    & (df_valid["is_hallucination"] == True)
                ).sum()
                fp = (
                    (df_valid["blocked"] == True)
                    & (df_valid["is_hallucination"] == False)
                ).sum()
                tn = (
                    (df_valid["blocked"] == False)
                    & (df_valid["is_hallucination"] == False)
                ).sum()
                fn = (
                    (df_valid["blocked"] == False)
                ...
                metrics.update(
                    {"net_impact": int(net_impact), "net_impact_rate": net_impact_rate}
                )
...
def visualize_layer_results(layer_results, output_dir, num_layers=None):
    ...
    plt.plot(
        layers, blocked_rates, marker="o", linestyle="-", label="Response Blocking Rate"
    )
    ...
    for layer, pos_cat in zip(layers, positions_cat):
        for metric in metrics_for_heatmap:
            if metric in layer_results[layer]:
                position_metrics[position_labels[pos_cat]][metric].append(
                    layer_results[layer][metric]
                )
    ...
    plt.xticks(
        range(len(metrics_for_heatmap)),
        [m.capitalize() for m in metrics_for_heatmap],
        rotation=45,
    )
...
def count_model_layers(model_name):
    ...
    try:
        ...
        if hasattr(model, "config") and hasattr(model.config, "num_hidden_layers"):
            ...
            print(
                "Warning: Could not determine number of layers from model. Using default value."
            )
            num_layers = 24
...
def main():
    parser = argparse.ArgumentParser(
        description="Perform a layer sweep to find the best layer for hallucination detection"
    )
    parser.add_argument(
        "--model-name",
        type=str,
    ...
    for layer in layers_to_evaluate:
        if layer < 0 or layer >= num_layers:
            print(
                f"Warning: Layer {layer} is out of range (0-{num_layers - 1}). Skipping."
            )
            continue
    ...
    for i, (layer, score) in enumerate(top_layers):
        ...
        print(
            f"   Hallucinations Detected: {metrics.get('harmful_detected', 0)} ({metrics.get('harmful_rate', 0):.2%})"
        )
        print(
            f"   Responses Blocked: {metrics.get('blocked_count', 0)} ({metrics.get('blocked_rate', 0):.2%})"
        )
        print(
            f"   Net Impact: {metrics.get('net_impact', 0)} ({metrics.get('net_impact_rate', 0):.2%})"
        )
    ...
    best_layers = {
        "evaluation_metric": args.evaluation_metric,
        "top_layers": [
            {"layer": layer, "score": float(score)} for layer, score in top_layers
        ],
    }
    ...
    print(
        f"Best layer for {args.evaluation_metric}: Layer {top_layers[0][0]} with score {top_layers[0][1]:.4f}"
    )
