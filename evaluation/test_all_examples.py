...
# Import detectors
from examples.detect_harmful_content import (
    create_harmful_content_pairs,
    get_test_statements_and_ground_truth as get_harmful_statements,
)
from examples.detect_bad_code import (
    create_bad_code_pairs,
    get_test_statements_and_ground_truth as get_bad_code_statements,
)
from examples.detect_bias import (
    create_bias_pairs,
    get_test_statements_and_ground_truth as get_bias_statements,
)
from examples.detect_gender_bias import (
    create_gender_bias_pairs,
    get_test_statements_and_ground_truth as get_gender_bias_statements,
)
from examples.detect_hallucinations import (
    create_hallucination_pairs,
    get_test_statements_and_ground_truth as get_hallucination_statements,
)
from examples.detect_personal_info import (
    create_personal_info_pairs,
    get_test_statements_and_ground_truth as get_personal_info_statements,
)
from examples.detect_scheming import (
    create_scheming_pairs,
    get_test_statements_and_ground_truth as get_scheming_statements,
)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Test all examples and save accuracy results as JSON"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="./evaluation/results/detector_accuracy_results.json",
...
def test_detector(
    ...
    try:
        ...
        metrics = test_classifier(
            classifier_guard, test_statements, ground_truth, content_type=content_type
        )

        print(
            f"{content_type} detection testing complete with F1 score: {metrics['f1']:.4f}"
        )
        return metrics
...
def main():
    ...
    results["summary"] = {
        "average_accuracy": sum(
            d.get("accuracy", 0) for d in results["detectors"].values()
        )
        / len(results["detectors"]),
        "average_f1": sum(d.get("f1", 0) for d in results["detectors"].values())
        / len(results["detectors"]),
        "best_detector": max(
            results["detectors"].items(), key=lambda x: x[1].get("f1", 0)
        )[0],
        "worst_detector": min(
            results["detectors"].items(), key=lambda x: x[1].get("f1", 0)
        )[0],
