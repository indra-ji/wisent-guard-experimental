...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Test all examples with pre-trained classifiers and save accuracy results as JSON"
    )
    parser.add_argument(
        "--output",
        type=str,
...
def test_detector_with_predictions(
    test_statements, ground_truth, content_type, predictions=None
):
    """
    ...
    print("---------------|--------------|---------------|")
    print(
        f"Pred harmful   |      {true_positives:4d}     |      {false_positives:4d}      |"
    )
    print(
        f"Pred harmless  |      {false_negatives:4d}     |      {true_negatives:4d}      |"
    )
...
def main():
    ...
    for detector in detectors:
        ...
        classifier_file = os.path.join(
            args.models_dir, f"{detector_name}_classifier.pkl"
        )

        # Simulate predictions
        predictions = simulate_classifier_predictions(
            classifier_file, test_statements, ground_truth
        )
