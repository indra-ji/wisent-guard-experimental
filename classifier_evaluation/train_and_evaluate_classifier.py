    ...
    try:
        ...
        for i, item in enumerate(ds):
            ...
            if i < 10:  # Only log first 10 for debugging
                logger.debug(
                    f"Task {task_id}: pass@1 = {pass_rate:.3f}, difficulty = {item_difficulty}"
                )

        logger.info(
            f"Found {len(positive_codes)} positive and {len(negative_codes)} negative examples"
        )
...
def train_and_evaluate_classifier(
    config_path: str, overrides: Dict[str, Any] = None
) -> Dict[str, Any]:
    """
    ...
    logger.info(
        f"Training with {len(train_positive)} positive and {len(train_negative)} negative examples"
    )
    ...
    try:
        ...
        true_labels = [0] * len(test_positive) + [1] * len(
            test_negative
        )  # 0=positive, 1=negative

        logger.info(
            f"Testing with {len(test_codes)} examples ({len(test_positive)} positive, {len(test_negative)} negative)"
        )
...
def calculate_metrics(
    true_labels: List[int], predictions: List[int], train_time: float, eval_time: float
) -> Dict[str, Any]:
    ...
    sample_counts = {
        ...
        "correct_predictions": sum(
            1 for i in range(len(true_labels)) if true_labels[i] == predictions[i]
        ),
    }
    ...
    return {
        ...
        "timing": {"train_time_seconds": train_time, "eval_time_seconds": eval_time},
    }
...
def save_results(results: Dict[str, Any], config: Dict[str, Any]) -> None:
    ...
    filename = (
        f"train_eval_{train_version}_to_{test_version}_layer{layer}_{timestamp}.json"
    )
...
def display_results_summary(results: Dict[str, Any]) -> None:
    ...
    logger.info(
        f"  Confidence (Incorrect): {conf_stats['confidence_when_incorrect']:.4f}"
    )
...
def compare_versions(
    config_path: str, versions: list, overrides: Dict[str, Any] = None
) -> None:
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train and evaluate WisentGuard classifiers"
    )
    parser.add_argument("config", type=str, help="Path to configuration YAML file")
    parser.add_argument(
        "--override",
        action="append",
        help="Override config values (e.g., --override data.limit=100)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--compare", action="store_true", help="Run cross-version comparison"
    )
    parser.add_argument(
        "--versions", nargs="+", help="Versions to compare (requires --compare)"
    )
