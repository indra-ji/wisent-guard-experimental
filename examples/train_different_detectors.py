...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Train threshold-based and classifier-based detectors for code quality"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        "--threshold-save-dir",
        type=str,
        default="./models/code_threshold_detector",
        help="Directory to save threshold detector data",
    )
    parser.add_argument(
        "--classifier-path",
        type=str,
        default="./models/code_quality_classifier.pkl",
...
def train_threshold_detector(model, tokenizer, training_pairs, args):
    ...
    for pair in training_pairs:
        formatted_pairs.append(
            {"harmful": pair["bad_code"], "harmless": pair["good_code"]}
        )
...
def train_classifier_detector(model, tokenizer, training_pairs, args):
    ...
    print(
        f"Collected {len(bad_code_activations)} bad code and {len(good_code_activations)} good code activations"
    )
    ...
    try:
        ...
        print(
            f"Classifier trained in {train_time:.2f} seconds and saved to {args.classifier_path}"
        )
