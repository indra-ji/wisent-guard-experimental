...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Compare is_harmful vs blocked in hallucination detection"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        "--classifier-path",
        type=str,
        default="./models/hallucination_classifier_comparison.pkl",
...
def get_user_evaluation(response, question):
    ...
    while True:
        ...
        if user_input in ["y", "yes"]:
            ...
            print(
                "Please enter 'y' (yes, hallucination), 'n' (no, factual), 's' (skip), or 'q' (quit)"
            )
...
def calculate_effectiveness_metrics(results):
    ...
    is_harmful_tp = sum(
        1 for r in evaluated_results if r["is_harmful"] and r["user_evaluation"]
    )
    is_harmful_fp = sum(
        1 for r in evaluated_results if r["is_harmful"] and not r["user_evaluation"]
    )
    is_harmful_tn = sum(
        1 for r in evaluated_results if not r["is_harmful"] and not r["user_evaluation"]
    )
    is_harmful_fn = sum(
        1 for r in evaluated_results if not r["is_harmful"] and r["user_evaluation"]
    )

    # Calculate metrics for blocked
    blocked_tp = sum(
        1 for r in evaluated_results if r["blocked"] and r["user_evaluation"]
    )
    blocked_fp = sum(
        1 for r in evaluated_results if r["blocked"] and not r["user_evaluation"]
    )
    blocked_tn = sum(
        1 for r in evaluated_results if not r["blocked"] and not r["user_evaluation"]
    )
    blocked_fn = sum(
        1 for r in evaluated_results if not r["blocked"] and r["user_evaluation"]
    )
    ...
    is_harmful_metrics = calc_metrics(
        is_harmful_tp, is_harmful_fp, is_harmful_tn, is_harmful_fn
    )
...
def analyze_scenario(guard, scenario, enable_user_evaluation=True):
    ...
    try:
        result = guard.generate_safe_response(scenario["prompt"], max_new_tokens=100)
        ...
        print(
            f"Generated response: {response[:200]}{'...' if len(response) > 200 else ''}"
        )
            ...
            try:
                ...
                if activations and guard.layers:
                    ...
                    if layer in activations:
                        ...
                        prediction = guard.classifier.predict(
                            activation_data, response_text=response
                        )
                        print(
                            f"Classification score: {prediction.get('score', 'N/A'):.4f}"
                        )
    ...
    if user_evaluation is not None:
        print(
            f"User evaluation (ground truth): {'Hallucination' if user_evaluation else 'Factual'}"
        )
    ...
    if is_harmful_result and blocked:
        ...
        print(
            "→ SCENARIO: Response flagged by post-analysis but NOT blocked during generation"
        )
    elif not is_harmful_result and blocked:
        print(
            "→ SCENARIO: Response blocked during generation but NOT flagged by post-analysis"
        )
    else:
...
def main():
    ...
    model, tokenizer = load_model(
        args.model, device, args.half_precision, args.load_in_8bit
    )
    ...
    classifier_guard = train_classifier_detector(
        model, tokenizer, training_pairs, detector_config
    )
    ...
    print(
        f"{'Scenario':<25} {'is_harmful':<12} {'blocked':<10} {'User Eval':<12} {'Combination'}"
    )
    print("-" * 80)
    ...
    for result in results:
        ...
        print(
            f"{result['name']:<25} {str(is_harmful):<12} {str(blocked):<10} {user_eval_str:<12} {combination}"
        )
    ...
    print("KEY TAKEAWAYS:")
    print(
        "• is_harmful() analyzes RESPONSE text patterns without generating new content"
    )
