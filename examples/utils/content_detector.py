...
def load_model(model_name, device, half_precision=False, load_in_8bit=False):
    ...
    print(
        f"Model has {(sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e9):.2f} billion parameters"
    )
...
def train_classifier_detector(model, tokenizer, training_pairs, config):
    ...
    print(
        f"Collected {len(harmful_activations)} {config['positive_class_label']} and {len(harmless_activations)} normal activations"
    )
    ...
    try:
        ...
        print(
            f"Classifier trained in {train_time:.2f} seconds and saved to {config['classifier_path']}"
        )
...
def test_classifier(
    classifier_guard, test_statements, ground_truth, content_type="harmful content"
):
    """
            ...
            if classifier_guard.classifier and classifier_guard.monitor:
                ...
                if success:
                    ...
                    if layer in activations:
                        ...
                        prediction = classifier_guard.classifier.predict(
                            activation_data, response_text=statement
                        )
...
def evaluate_responses(model, tokenizer, guard, prompts, show_input=True):
    ...
    for i, prompt in enumerate(prompts):
        ...
        result = guard.generate_safe_response(formatted_prompt, max_new_tokens=200)
        ...
        if blocked:
            print(
                f"⚠️ Content flagged as potentially {guard.classifier.positive_class_label}"
            )
        ...
        if classification_score is not None:
            print(
                f"Classification score: {classification_score:.4f} (threshold: {classification_threshold:.4f})"
            )
