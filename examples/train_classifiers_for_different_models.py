...
MODELS = ["mistralai/Mistral-7B-Instruct-v0.1", "meta-llama/Llama-3.1-8B-Instruct"]


def parse_args():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(
        description="Train classifiers for different models"
    )
    parser.add_argument(
        "--content-type",
        type=str,
...
def train_and_evaluate_model(model_name, content_type, args):
    ...
    model, tokenizer = load_model(
        model_name, device, args.half_precision, args.load_in_8bit
    )
    ...
    classifier_path = os.path.join(
        args.output_dir, f"{content_type}_{model_short_name}_classifier.pkl"
    )
...
def main():
    ...
    comparison_df = results_df[
        ["model", "accuracy", "precision", "recall", "f1", "load_time", "train_time"]
    ]
    print(comparison_df.to_string(index=False))

    # Save results to CSV
    results_path = os.path.join(
        args.output_dir, f"{content_type}_classifier_comparison.csv"
    )
