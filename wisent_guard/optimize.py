...
def get_cache_key(
    model_name: str, task_name: str, limit: int, ground_truth_method: str
) -> str:
        ...
        try:
            ...
            for field in required_fields:
                if cached_data.get(field) is None:
                    print(
                        f"⚠️ Invalid cached results (field '{field}' is None), ignoring cache"
                    )
            ...
            if cached_data.get("best_accuracy", 0.0) <= 0.0:
                print(
                    f"⚠️ Invalid cached results (accuracy {cached_data.get('best_accuracy', 0.0)}), ignoring cache"
                )
                cache_path.unlink()
                return None
    ...
    for layer_idx in candidate_layers:
        try:
            ...
            for pair in processed_pairs:
                if (
                    hasattr(pair, "positive_activations")
                    and pair.positive_activations is not None
                ):
                    X_train.append(
                        pair.positive_activations.detach().cpu().flatten().numpy()
                    )
                    y_train.append(0)  # truthful
                if (
                    hasattr(pair, "negative_activations")
                    and pair.negative_activations is not None
                ):
                    X_train.append(
                        pair.negative_activations.detach().cpu().flatten().numpy()
                    )
            ...
            training_results = classifier.fit(
            ...
            if verbose:
                print(
                    f"     ✅ Layer {layer_idx}: Train accuracy = {train_accuracy:.3f}"
                )
...
def optimize_aggregation_on_ground_truth(
    ...
    if verbose:
        ...
        if len(test_qa_pairs) > 0:
            print(
                f"   • First test question: {test_qa_pairs[0].get('formatted_question', 'N/A')[:100]}..."
            )
        else:
            print(
                "   ❌ CRITICAL: NO TEST QA PAIRS! Cannot perform ground truth optimization."
            )
    ...
    evaluator = GroundTruthEvaluator.from_string(
        ground_truth_method, task_name=task_name
    )
    if verbose:
    ...
    for layer_idx in layer_results.keys():
        ...
        for classifier_type in classifier_types:
            try:
                if verbose:
                    print(
                        f"     🎯 Training {classifier_type} classifier for layer {layer_idx}..."
                    )
                ...
                if verbose:
                    print(
                        f"       ✅ {classifier_type} trained: {training_results.get('accuracy', 'N/A'):.3f}"
                    )
        ...
        for classifier_type, steering_method in layer_classifiers.items():
            if verbose:
                print(
                    f"     🎭 Generating responses with {classifier_type} classifier..."
                )
            ...
            for i, qa_pair in enumerate(test_qa_pairs):
                try:
                    ...
                    if verbose:
                        print(
                            f"       📝 Processing question {i + 1}/{len(test_qa_pairs)}: {question[:100]}..."
                        )
                    ...
                    if verbose:
                        ...
                        print(
                            f"       📊 Token scores length: {len(token_scores) if token_scores else 'None'}"
                        )
                    ...
                    if ground_truth_label == "UNKNOWN":
                        ...
                        print(
                            f"Expected Answer: {qa_pair.get('correct_answer', 'N/A')}"
                        )
                        print(f"Ground Truth Method: {ground_truth_method}")
                        print(f"Evaluator Details: {gt_result}")
                        print(
                            "💡 Ground truth evaluation returned UNKNOWN instead of TRUTHFUL/HALLUCINATION"
                        )
                        print(
                            "🛠️  This means the ground truth method cannot evaluate this type of response"
                        )
                    ...
                    if ground_truth_label not in ["TRUTHFUL", "HALLUCINATION"]:
                        ...
                        print(
                            "🛠️  This means the ground truth method is not compatible with optimization"
                        )
                        print("💥💥💥 STOPPING OPTIMIZATION IMMEDIATELY 💥💥💥\n")
                    ...
                    if ground_truth_label in ["TRUTHFUL", "HALLUCINATION"]:
                        ...
                        if verbose:
                            print(
                                f"       ⚠️ Skipping - ground truth not TRUTHFUL/HALLUCINATION: {ground_truth_label}"
                            )
            ...
            if verbose:
                print(
                    f"     📊 Total question_results for {classifier_type}: {len(question_results)}"
                )
            ...
            if len(question_results) == 0:
                if verbose:
                    print(
                        f"     ❌ CRITICAL: No question_results for {classifier_type}! Cannot test combinations."
                    )
                continue
            ...
            for aggregation in aggregation_methods:
                for threshold in thresholds:
                    if (
                        verbose and len(thresholds) <= 3
                    ):  # Only show details for small threshold sets
                        print(
                            f"       ➤ Testing {aggregation} + threshold {threshold}..."
                        )
                    ...
                    for result in question_results:
                        try:
                            ...
                            aggregated_score = aggregate_token_scores(
                                result["token_scores"], aggregation
                            )
                            classification = (
                                "HALLUCINATION"
                            ...
                            if verbose:
                                print(
                                    f"         ⚠️ Error applying {aggregation} + {threshold}: {e}"
                                )
                            continue
                    ...
                    if total_predictions > 0:
                        ...
                        if verbose:
                            print(
                                f"         ⚠️ No predictions for {aggregation} + {threshold}"
                            )
...
def run_smart_optimization(
    ...
    optimization_results = {
        ...
        "layer_results": {
            k: {"train_accuracy": v["train_accuracy"]} for k, v in layer_results.items()
        },
...
def extract_layer_activations_for_responses(
    ...
    for i, question in enumerate(questions):
        if verbose:
            print(
                f"Extracting activations for question {i + 1}/{len(questions)} (layer {layer_idx})"
            )
        ...
        try:
            ...
            generated_ids = outputs.sequences[0][
                len(inputs_on_device["input_ids"][0]) :
            ]
            ...
            results.append(
                {"question": question, "response": "[ERROR]", "layer_activations": []}
            )
...
def compute_aggregated_scores(
    layer_activations: List, aggregation_method: str
) -> List[float]:
    """
...
def run_interactive_optimization(
    ...
    if verbose:
        ...
        print(
            f"   • Training {len(layer_range)} classifiers using {len(training_pairs)} training pairs"
        )
    ...
    for q_idx, question in enumerate(questions):
        try:
            ...
            result = evaluator.evaluate_response(
                response=response_text, expected_answer=None
            )
        ...
        for layer_idx, classifier in layer_classifiers.items():
            if layer_idx < len(response_data["layer_activations"]):
                try:
                    ...
                    if verbose:
                        print(
                            f"   ⚠️ Error getting prediction from layer {layer_idx}: {e}"
                        )
        ...
        for layer_idx in layer_predictions:
            for agg_method in aggregation_methods:
                ...
                if agg_method == "average":
                    ...
                    final_score = min(
                        prediction_score * 1.1, 1.0
                    )  # Slight boost for max
                elif agg_method == "min":
                    final_score = max(
                        prediction_score * 0.9, 0.0
                    )  # Slight reduction for min
    ...
    if verbose:
        ...
        for i, ((layer, agg), perf) in enumerate(sorted_combos[:5]):
            ...
            print(
                f"   {i + 1}. Layer {layer}, {agg}: {acc:.1%} ({perf['correct']}/{perf['total']})"
            )
