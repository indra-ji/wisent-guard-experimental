...
from wisent_guard.core.models import (
    SteeringCompatibleModel,
    create_steering_compatible_model,
)
...
class TestSteeringCompatibleModel:
    ...
    def test_init(self):
        ...
        config = GPT2Config(
            vocab_size=100, n_positions=128, n_embd=64, n_layer=2, n_head=2
        )
    ...
    def test_add_multiple_steering_vectors(self):
        ...
        config = GPT2Config(
            vocab_size=100, n_positions=128, n_embd=64, n_layer=3, n_head=2
        )
        model = SteeringCompatibleModel(config)
    ...
    @patch(
        "wisent_guard.core.models.steering_compatible_model.GPT2LMHeadModel.from_pretrained"
    )
    @patch(
        "wisent_guard.core.models.steering_compatible_model.AutoConfig.from_pretrained"
    )
    def test_from_pretrained_with_steering(self, mock_config, mock_model):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            torch.save(
                torch.randn(64),
                os.path.join(temp_dir, "steering_vector_distilgpt2_0_20250118.pt"),
            )

            model = SteeringCompatibleModel.from_pretrained_with_steering(
                "distilgpt2", steering_directory=temp_dir
            )
            assert isinstance(model, SteeringCompatibleModel)
    ...
    def test_create_steering_compatible_model(self):
        ...
        with patch(
            "wisent_guard.core.models.steering_compatible_model.SteeringCompatibleModel.from_pretrained_with_steering"
            ...
            mock_from_pretrained.assert_called_once_with(
                "distilgpt2", steering_directory=None
            )
    ...
    def test_end_to_end_persistence(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert torch.allclose(
                model.transformer.wte.weight, new_model.transformer.wte.weight
            )
