...
from wisent_guard.core.pipelines import (
    ActivationCollector,
...
from wisent_guard.core.pipelines.experiment_runner import (
    ExperimentConfig,
    ExperimentResults,
)
...
class TestActivationCollector:
    ...
    @patch("wisent_guard.core.pipelines.activation_collector.AutoModel")
    @patch("wisent_guard.core.pipelines.activation_collector.AutoTokenizer")
    def test_collect_contrastive_activations(
        ...
        with patch.object(
            collector, "_collect_batch_activations"
        ) as mock_collect_batch:
...
class TestSteeringVectorTrainer:
    ...
    def test_initialize_components(self, mock_caa_class, mock_collector_class):
        ...
        mock_collector_class.assert_called_once_with(
            model_name="distilgpt2", device="auto", max_length=512
        )
        mock_caa_class.assert_called_once_with(
            device="auto", normalization_method="none", target_norm=None
        )
    ...
    def test_train_workflow(self, mock_caa_class, mock_collector_class):
        ...
        mock_collector.collect_contrastive_activations.return_value = (
            mock_activation_data
        )
...
class TestExperimentRunner:
    ...
    def test_run_experiment_workflow(
        self,
        mock_trainer_class,
        ...
        # Mock model creation
        with patch(
            "wisent_guard.core.pipelines.experiment_runner.SteeringCompatibleModel"
        ) as mock_model_class:
            mock_model = MagicMock()
...
class TestSteeringTrainingIntegration:
    ...
    def test_end_to_end_pipeline(self, mock_tokenizer_class, mock_model_class):
        ...
        # Configure training
        config = TrainingConfig(
            model_name="distilgpt2", target_layers=[0], batch_size=1, device="cpu"
        )

        # Mock activation collection to return proper tensors
        with patch.object(
            ActivationCollector, "_get_activations"
        ) as mock_get_activations:
            mock_get_activations.return_value = torch.randn(1, 768)

            # Mock steering method training
            with patch(
                "wisent_guard.core.pipelines.steering_trainer.CAA"
            ) as mock_caa_class:
                mock_caa = MagicMock()
                mock_caa.steering_vector = torch.randn(768)
                mock_caa.train.return_value = {"loss": 0.5}
    ...
    def test_quick_experiment_integration(self):
        ...
        with patch(
            "wisent_guard.core.pipelines.experiment_runner.LiveCodeBenchVersionManager"
        ) as mock_vm:
            with patch(
                "wisent_guard.core.pipelines.experiment_runner.SteeringDataExtractor"
            ) as mock_extractor:
                with patch(
                    "wisent_guard.core.pipelines.experiment_runner.SteeringVectorTrainer"
                    ...
                    with patch(
                        ...
                        mock_extractor.return_value.extract_contrastive_pairs.return_value = [
                            MagicMock()
                        ]
                        ...
                        mock_trainer_class.return_value.__enter__.return_value = (
                            mock_trainer
                        )

                        mock_model_class.from_pretrained_with_steering.return_value = (
                            MagicMock()
                        )
