...
class TestOptimizedDockerExecutor:
    ...
    def mock_docker_available(self):
        ...
        with patch(
            "wisent_guard.core.docker.optimized_docker_executor.subprocess.run"
        ) as mock_run:
    ...
    @patch(
        "wisent_guard.core.docker.optimized_docker_executor.tempfile.NamedTemporaryFile"
    )
    def test_execute_single_with_resource_optimization(
        self, mock_tempfile, mock_run, executor
    ):
    ...
    def test_execute_batch_with_complexity_grouping(
        self, mock_tempfile, mock_run, executor
    ):
...
"""
        ...
        complex_complexity = TaskComplexityAnalyzer.analyze_code_complexity(
            complex_code
        )
