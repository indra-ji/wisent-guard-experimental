...
class ContrastiveVectors:
    ...
    def create_pair_set(
        self, name: str, harmful_texts: List[str], harmless_texts: List[str]
    ) -> ContrastivePairSet:
        """
    ...
    def compute_vectors(
        self, pair_set_name: str, layer_idx: Optional[int] = None
    ) -> Dict[int, Any]:
        """
        ...
        layers_to_compute = (
            [layer_idx] if layer_idx is not None else [l.index for l in self.layers]
        )
        ...
        logger.info(
            f"Computed vectors for '{pair_set_name}' on layers {list(computed_vectors.keys())}"
        )
        return computed_vectors

    def get_similarity(
        self, text: str, category: str, layer_idx: Optional[int] = None
    ) -> float:
        """
    ...
    def load_vectors(
        self, categories: Optional[List[str]] = None, layers: Optional[List[int]] = None
    ) -> bool:
        """
        ...
        try:
            ...
            for vector_file in vector_files:
                ...
                if layers:
                    loaded_vectors = {
                        k: v for k, v in loaded_vectors.items() if k in layers
                    }
...
def create_vectors(
    ...
    return ContrastiveVectors(
        model_name=model_name, layers=layers, device=device, save_dir=save_dir
    )
