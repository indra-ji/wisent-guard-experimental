...
class SafeInference:
    ...
    def train_safety_filter(
        self, harmful_texts: List[str], harmless_texts: List[str]
    ) -> Dict[str, Any]:
        """
    ...
    def generate(
        ...
        if not self.is_trained:
            ...
            response, _ = self.model.generate(
                prompt, self.layer, max_new_tokens, **generation_kwargs
            )
        ...
        try:
            ...
            with torch.no_grad():
                outputs = self.model.hf_model(
                    **inputs_on_device, output_hidden_states=True
                )
            ...
            for token_idx in range(prompt_length, len(tokens)):
                if token_idx < hidden_states.shape[1]:
                    ...
                    if (
                        hasattr(self.steering_method, "classifier")
                        and self.steering_method.classifier
                    ):
                        try:
                            # Predict probability of being harmful (class 1)
                            # Our classifier returns a single float, not an array like sklearn
                            prob = self.steering_method.classifier.predict_proba(
                                [features.numpy()]
                            )
                            # Handle both single float and array returns
                            if isinstance(prob, (list, tuple)) or hasattr(
                                prob, "__getitem__"
                            ):
                                if len(prob) > 0:
                            ...
                            if not (0.0 <= prob <= 1.0):
                                raise ValueError(
                                    f"Classifier probability {prob} is out of valid range [0.0, 1.0]"
                                )

                            token_scores.append(float(prob))
                        except Exception as e:
                            logger.warning(
                                f"Classifier error for token {token_idx}: {e}"
                            )
            ...
            response_tokens = self.model.tokenizer(response, return_tensors="pt")[
                "input_ids"
            ][0]
...
def create_safe_inference(
    ...
    inference = SafeInference(
        model_name=model_name, layer=layer, steering_type=steering_type, device=device
    )
...
def generate_with_classification_and_handling(
    ...
    if verbose:
        print(
            f"      🚨 Detected problematic content, applying {detection_handler.action.value} handling..."
        )
    ...
    if detection_handler.action == DetectionAction.REGENERATE_UNTIL_SAFE:
        ...
        def regenerate():
            ...
            new_response, new_token_scores, new_classification = (
            ...
            if new_classification != "HALLUCINATION":
                ...
                raise Exception(
                    f"Generated response still classified as {new_classification}"
                )
    ...
    if (
        ...
        final_response_clean, final_token_scores, final_classification = (
            generate_with_classification(
                model,
                prompt,
                layer,
            ...
            if token_idx < hidden_states.shape[1]:
                ...
                if (
                    ...
                    try:
                        ...
                        if verbose:
                            print(
                                f"      ⚠️  Classifier error for token {token_idx}: {e}"
                            )
...
def generate_with_multi_layer_classification(
    ...
    for layer in layers:
        ...
        try:
            ...
            for token_idx in range(prompt_length, len(tokens)):
                if token_idx < hidden_states.shape[1]:
                    ...
                    if (
                        steering_method
                        and hasattr(steering_method, "classifier")
                        and steering_method.classifier
                    ):
                        try:
                            prob = steering_method.classifier.predict_proba(
                            ...
                            if verbose:
                                print(
                                    f"      ⚠️  Layer {layer} classifier error for token {token_idx}: {e}"
                                )
                            token_scores.append(0.5)
                    else:
                        token_scores.append(0.5)
        ...
        if token_scores:
            aggregated_score = aggregate_token_scores(token_scores, token_aggregation)
            classification = (
                "HALLUCINATION" if aggregated_score > threshold else "TRUTHFUL"
            )
        else:
            aggregated_score = 0.5
            classification = "UNKNOWN"
        ...
        if verbose:
            print(
                f"      Layer {layer}: {classification} (score: {aggregated_score:.3f})"
            )
...
def generate_with_multi_layer_classification_and_handling(
    ...
    if (
        ...
        final_response_clean, final_layer_results = (
