...
def load_truthfulqa_data() -> pd.DataFrame:
    ...
    if df is None:
        raise FileNotFoundError(
            f"Could not find TruthfulQA.csv in any of: {possible_paths}"
        )
...
def create_training_data_file(
    pairs_df: pd.DataFrame, sample_size: float, output_path: str
) -> str:
    ...
    # Sample the data
    sampled_df = pairs_df.sample(frac=sample_size, random_state=42).reset_index(
        drop=True
    )
...
def train_classifier_with_sample_size(
    layer: int, sample_size: float, training_data_path: str, model_name: str
) -> str:
    """Train a classifier with specific sample size."""
    print(
        f"🎓 Training classifier for layer {layer} with {sample_size * 100:.1f}% sample size..."
    )
...
def extract_activation_scores(
    model: Model, text: str, layer: int, classifier: Any
) -> List[float]:
...
def evaluate_classifier(
    ...
    predictions = [
        1 if score <= threshold else 0 for score in aggregated_scores
    ]  # Lower score = more truthful
...
def create_performance_curve(
    ...
    output_path: str = "sample_size_performance_curve.png",
):
...
def parse_args():
    ...
    parser.add_argument(
        "--layer", type=int, default=15, help="Target layer to analyze (default: 15)"
    )
...
def main():
    ...
    try:
        ...
        print(
            "   Sample sizes should be comma-separated values between 0 and 1, e.g., '0.1,0.2,0.5,1.0'"
        )
        return
    ...
    try:
        ...
        train_df, test_df = train_test_split(
            pairs_df, test_size=TEST_SIZE, random_state=42, stratify=pairs_df["label"]
        )
        ...
        for sample_size in SAMPLE_SIZES:
            ...
            if classifier_path and os.path.exists(classifier_path):
                ...
                if metrics:
                    ...
                    print(
                        f"   ✅ Sample size {sample_size * 100:.1f}%: Accuracy={metrics['accuracy']:.3f}, F1={metrics['f1']:.3f}"
                    )
                else:
                    print(
                        f"   ❌ Evaluation failed for sample size {sample_size * 100:.1f}%"
                    )
            else:
        ...
        if results:
            ...
            base_name = (
                f"{OUTPUT_PREFIX}sample_size_layer_{TARGET_LAYER}_{AGGREGATION_METHOD}"
            )
            ...
            for sample_size in sorted(results.keys()):
                ...
                print(
                    f"Sample {sample_size * 100:3.0f}%: Acc={metrics['accuracy']:.3f}, F1={metrics['f1']:.3f}, Prec={metrics['precision']:.3f}, Rec={metrics['recall']:.3f}"
                )
