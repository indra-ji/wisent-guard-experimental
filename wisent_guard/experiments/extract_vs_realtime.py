...
def parse_cli_output(output: str) -> List[Dict]:
    ...
    for line in lines:
        ...
        # Look for token scores - this ends response collection
            ...
            if scores_match:
                try:
                    ...
                    scores = [
                        float(s.strip().strip("'\"")) for s in scores_str.split(",")
                    ]
        ...
        if current_question and current_response and current_scores:
            ...
            print(
                f"      ✅ Captured: Question='{current_question[:30]}...', Response='{current_response[:30]}...', Scores={len(current_scores)} tokens"
            )
            ...
            if current_response:
                ...
                temp_tokens = temp_model.tokenizer.encode(
                    current_response, return_tensors="pt"
                )
                print(
                    f"      📊 Manual tokenization: {len(temp_tokens[0])} tokens from response text"
                )
                print(
                    f"      📊 CLI claims: {len(current_scores)} scores from 109 total tokens"
                )
                print(
                    "      🔍 ISSUE: CLI display shows truncated response but scored full 109-token generation!"
                )
                del temp_model
...
def extract_post_generation_scores(cli_results: List[Dict]) -> List[Dict]:
    ...
    steering_method = SteeringMethod(
        method_type=SteeringType.LOGISTIC, device=model.device
    )
    ...
    for i, result in enumerate(cli_results):
        ...
        try:
            ...
            for token_idx in range(
                prompt_length, min(len(tokens), hidden_states.shape[1])
            ):
                # Extract activation
                ...
                if token_idx - prompt_length < 3:
                    print(
                        f"         🔍 Token {token_idx - prompt_length}: activation_shape={token_activation.shape}, norm={torch.norm(token_activation):.3f}"
                    )
                    print(
                        f"         🔍 Features shape={features.shape}, norm={torch.norm(features):.3f}, mean={features.mean():.3f}"
                    )

                # Handle the loaded classifier correctly (it's a dict, not SteeringMethod)
                if (
                    isinstance(steering_method, dict)
                    and "classifier" in steering_method
                ):
                    classifier = steering_method["classifier"]
                    try:
                        prob = classifier.predict_proba([features.numpy()])
                        if (
                            isinstance(prob, (list, tuple, np.ndarray))
                            and len(prob) > 0
                        ):
                    ...
                    hasattr(steering_method, "classifier")
                    and steering_method.classifier
                ):
                    # Handle SteeringMethod object
                    try:
                        prob = steering_method.classifier.predict_proba(
                            [features.numpy()]
                        )
                        if (
                            isinstance(prob, (list, tuple, np.ndarray))
            ...
            post_gen_results.append(
                {
                    ...
                    "avg_post_gen_score": np.mean(post_token_scores)
                    if post_token_scores
                    else 0,
                }
            )

            print(
                f"      ✅ Extracted {len(post_token_scores)} token scores using SAME classifier"
            )
            print(
                f"      📊 First 5 post-gen scores: {[f'{s:.3f}' for s in post_token_scores[:5]]}"
            )
            print(
                f"      📊 Token positions: prompt_length={prompt_length}, total_tokens={len(tokens)}, response_tokens={len(tokens) - prompt_length}"
            )

            # DEBUG: Check if we're using the exact same classifier object
            print(f"      🔍 Classifier type: {type(steering_method.classifier)}")
            print(
                f"      🔍 Classifier trained: {hasattr(steering_method.classifier, 'predict_proba')}"
            )
            ...
            if len(post_token_scores) >= 3:
                ...
                for debug_idx in range(min(3, len(tokens) - prompt_length)):
                    ...
                    if token_idx < hidden_states.shape[1]:
                        ...
                        features = activation_obj.extract_features_for_classifier()
                        print(
                            f"         Token {debug_idx}: shape={features.shape}, norm={torch.norm(features):.3f}, mean={features.mean():.3f}"
                        )
            ...
            print(
                f"      🔍 Tokenization: {len(tokens)} tokens, prompt={prompt_length}, response={len(tokens) - prompt_length}"
            )
    ...
    for i, (cli_result, pg_result) in enumerate(zip(cli_results, post_gen_results)):
        ...
        if rt_scores and pg_scores:
            ...
            if min_len > 0:
                ...
                correlation = (
                    np.corrcoef(rt_subset, pg_subset)[0, 1] if min_len > 1 else 1.0
                )

                print(f"   📊 Token scores comparison ({min_len} tokens):")
                print(
                    f"      • Real-time: {[f'{s:.3f}' for s in rt_subset[:5]]}{'...' if len(rt_subset) > 5 else ''}"
                )
                print(
                    f"      • Post-gen:  {[f'{s:.3f}' for s in pg_subset[:5]]}{'...' if len(pg_subset) > 5 else ''}"
                )
                print(
                    f"      • Raw differences: {[f'{abs(r - p):.3f}' for r, p in zip(rt_subset[:5], pg_subset[:5])]}"
                )
