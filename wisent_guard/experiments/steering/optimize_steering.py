...
class SteeringOptimizer:
    ...
    def _get_model_layer_count(self) -> int:
        ...
        try:
            ...
            if self.verbose:
                logger.info(
                    f"Loading model {self.model_name} to determine layer count..."
                )
            ...
            raise RuntimeError(
                f"Could not determine layer count for model {self.model_name}. "
                f"Use --layer-range to specify layers manually."
            ) from e
    ...
    def _define_search_spaces(self) -> Dict[str, Any]:
        ...
        if self.custom_layer_range:
            ...
            if self.verbose:
                logger.info(
                    f"Searching all layers 1-{self.num_layers} ({self.num_layers} total layers)"
                )
        ...
        return {
            ...
            "method_params": {
                "HPR": {"hpr_beta": [0.1, 0.5, 1.0, 1.5, 2.0]},
                "KSteering": {
    ...
    def _parse_layer_range(self, layer_range: str) -> List[int]:
        ...
        try:
            ...
            logger.error(
                f"Invalid layer range format: {layer_range}. Use format like '8-24' or '10,15,20'"
            )
            raise e
                    ...
                    for split_ratio in self.search_spaces["split_ratios"]:
                        for seed in self.search_spaces["seeds"]:
                            ...
                            if method in self.search_spaces["method_params"]:
                                method_params = self.search_spaces["method_params"][
                                    method
                                ]
    ...
    def generate_random_search_configs(self, num_configs: int) -> List[SteeringConfig]:
        ...
        for _ in range(num_configs):
            ...
            if method in self.search_spaces["method_params"]:
                if method == "HPR":
                    config.hpr_beta = random.choice(
                        self.search_spaces["method_params"]["HPR"]["hpr_beta"]
                    )
                elif method == "KSteering":
                    params = self.search_spaces["method_params"]["KSteering"]
                    config.ksteering_alpha = random.choice(params["ksteering_alpha"])
                    config.ksteering_target_labels = random.choice(
                        params["ksteering_target_labels"]
                    )
                    config.ksteering_avoid_labels = random.choice(
                        params["ksteering_avoid_labels"]
                    )
    ...
    def evaluate_config(self, config: SteeringConfig) -> OptimizationResult:
        ...
        try:
            ...
            if (
                ...
                and "baseline_likelihoods" in result["evaluation_results"]
                and "steered_likelihoods" in result["evaluation_results"]
            ):
                ...
                if baseline and steered and len(baseline) == len(steered):
                    likelihood_change = sum(
                        s - b for s, b in zip(steered, baseline)
                    ) / len(baseline)
    ...
    def _optimize_parameters_for_runtime(
        self, target_minutes: float
    ) -> Tuple[int, int]:
        ...
        test_config = SteeringConfig(
            layer=15, method="CAA", strength=1.0, split_ratio=0.8, seed=42
        )
        ...
        try:
            ...
            processing_time_per_5_examples = max(
                20.0, base_time - model_loading_overhead
            )  # Remove loading overhead

            if self.verbose:
                print(f"   Total time (5 examples): {base_time:.1f}s")
                print(f"   Model loading overhead: {model_loading_overhead:.1f}s")
                print(
                    f"   Processing time (5 examples): {processing_time_per_5_examples:.1f}s"
                )
            ...
            def estimate_total_time(budget, limit):
                ...
                processing_time = (
                    budget * (limit / 5.0) * processing_time_per_5_examples
                )
            ...
            max_possible_limit = int(
                (max_processing_time_for_1 / processing_time_per_5_examples) * 5
            )
            min_useful_limit = max(
                5, min(15, max_possible_limit // 3)
            )  # More conservative
            ...
            for budget in [3, 5, 8, 10, 15, 20]:
                ...
                available_processing_time = target_seconds - (
                    budget * model_loading_overhead
                )
                if available_processing_time <= 0:  # No time left after loading
                    continue

                max_limit_for_budget = int(
                    (available_processing_time / budget)
                    / processing_time_per_5_examples
                    * 5
                )
            ...
            if best_score == 0:
                if self.verbose:
                    print(
                        "   No optimal combination found within time limit, using conservative fallback"
                    )
                best_budget = 1
                available_time = target_seconds - model_loading_overhead
                best_limit = max(
                    5, int((available_time / processing_time_per_5_examples * 5) * 0.8)
                )  # 80% safety margin
    ...
    def optimize(
        self,
        search_strategy: str = "smart",
        budget: int = 50,
        confirm_runtime: bool = True,
        ...
        if search_strategy == "grid":
            ...
            if len(configs) > budget:
                ...
                if self.verbose:
                    logger.info(
                        f"Sampled {budget} configurations from {len(configs)} total grid configs"
                    )
        ...
        for i, config in enumerate(configs):
            ...
            if self.verbose and result.error is None:
                logger.info(
                    f"Config {i + 1}: Layer {config.layer}, {config.method}, "
    ...
    def optimize_with_runtime_target(
        self, target_minutes: float, search_strategy: str = "smart"
    ) -> List[OptimizationResult]:
        """Run optimization with automatic parameter selection for target runtime."""

        # Determine optimal budget and limit
        optimal_budget, optimal_limit = self._optimize_parameters_for_runtime(
            target_minutes
        )
        ...
        if self.verbose:
            logger.info(
                f"Auto-configured: {optimal_budget} configs × {optimal_limit} examples for {target_minutes:.1f}min target"
            )
    ...
    def analyze_results(self) -> Dict[str, Any]:
        ...
        successful_results = [
            r for r in self.results if r.error is None and r.accuracy > 0
        ]
        ...
        for method in self.search_spaces["methods"]:
            method_results = [
                r for r in successful_results if r.config.method == method
            ]
            if method_results:
    ...
    def _generate_cli_command(self, config: SteeringConfig) -> str:
        ...
        if config.ksteering_target_labels is not None:
            cmd_parts.append(
                f"--ksteering-target-labels {config.ksteering_target_labels}"
            )
        if config.ksteering_avoid_labels is not None:
            cmd_parts.append(
                f"--ksteering-avoid-labels {config.ksteering_avoid_labels}"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Optimize steering parameters for maximum accuracy"
    )
    parser.add_argument(
        "--task", required=True, help="Task name (e.g., truthfulqa_mc1)"
    )
    parser.add_argument(
        "--model", default="meta-llama/Llama-3.1-8B-Instruct", help="Model name"
    )
    parser.add_argument(
        "--limit", type=int, default=100, help="Number of samples per evaluation"
    )
    parser.add_argument(
        "--budget", type=int, default=50, help="Number of configurations to evaluate"
    )
    parser.add_argument(
        "--strategy",
    ...
    parser.add_argument(
        "--layer-range", help='Custom layer range (e.g., "8-24" or "10,15,20")'
    )
    parser.add_argument(
        "--output-dir", help="Output directory path (default: optimization_results)"
    )
    parser.add_argument(
        "--device",
        default=None,
        help="Device to use (auto-detects best available if not specified)",
    )
    ...
    parser.add_argument(
        "--target-runtime", type=float, help="Target runtime in minutes"
    )
    ...
    if getattr(args, "target_runtime", None):
        results = optimizer.optimize_with_runtime_target(
            args.target_runtime, search_strategy=args.strategy
        )
    else:
        results = optimizer.optimize(
            search_strategy=args.strategy,
            budget=args.budget,
            confirm_runtime=not skip_confirmation,
        )
    ...
    if best_result:
        ...
        for i, result in enumerate(optimizer.get_top_configs(5)):
            if result.error is None:
                print(
                    f"   {i + 1}. Layer {result.config.layer}, {result.config.method}, "
    ...
    if "method_performance" in analysis:
        ...
        for method, perf in analysis["method_performance"].items():
            print(
                f"   {method}: Best {perf['best_accuracy']:.3f}, "
