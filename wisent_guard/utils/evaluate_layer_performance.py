...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate all layer performance for hallucination detection"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        "--half-precision", action="store_true", help="Use half precision (fp16)"
    )
...
def collect_activations_for_all_layers(
    model, tokenizer, training_pairs, layers, device
):
...
def get_user_evaluation(prompt, response):
    ...
    while True:
        user_input = (
            input("\nDoes this response contain hallucinations? (y/n/s/q): ")
            .lower()
            .strip()
        )
...
def evaluate_response_with_all_classifiers(
...
def evaluate_all_layers_performance(
    model, tokenizer, classifiers, test_prompts, max_tokens, device
):
        ...
        try:
            ...
            for layer in sorted(layer_predictions.keys()):
                ...
                if "error" not in pred:
                    print(
                        f"  Layer {layer}: {'HARMFUL' if pred['is_harmful'] else 'SAFE'} (score: {pred['score']:.3f})"
                    )
                else:
            ...
            if user_evaluation is not None:
                print(
                    f"User evaluation: {'Hallucination' if user_evaluation else 'Factual'}"
                )
...
def calculate_layer_metrics(results):
    ...
    for layer in sorted(all_layers):
        ...
        for result in results:
            if (
                result.get("user_evaluation") is not None
                and layer in result["layer_predictions"]
                and "error" not in result["layer_predictions"][layer]
            ):
        ...
        tn = sum(
            1
            for r in evaluated_results
            if not r["layer_predictions"][layer]["is_harmful"]
            and not r["user_evaluation"]
        )
        fn = sum(
            1
            for r in evaluated_results
        ...
        total_harmful_predictions = sum(
            1 for r in evaluated_results if r["layer_predictions"][layer]["is_harmful"]
        )
        avg_score = (
            sum(r["layer_predictions"][layer]["score"] for r in evaluated_results)
            / total_evaluated
        )
...
def print_all_layers_report(layer_metrics, classifier_threshold):
    ...
    for layer in sorted(layer_metrics.keys()):
        ...
        if "error" in metrics:
            ...
            print(
                f"{layer:<6} {metrics['f1']:<6.3f} {metrics['precision']:<6.3f} "
...
def save_results_to_model_params(
    model_name, layer_metrics, classifier_threshold, max_tokens, layers_evaluated
):
...
def main():
    ...
    classifiers = train_classifiers_for_all_layers(
        all_activations, args.classifier_threshold
    )
