...
def get_relevant_benchmarks_for_prompt(
    prompt: str, max_benchmarks: int = 1, existing_model=None
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        benchmark_list = "\n".join(
            [f"- {name}: {desc}" for name, desc in benchmark_descriptions.items()]
        )
                        ...
                        relevant_benchmarks.append(
                            {
                                ...
                                "relevance_score": len(relevant_benchmarks)
                                + 1,  # Higher score for earlier mentions
                            }
                        )
