...
from populate_tasks import (
    get_task_samples_for_analysis as _get_task_samples_for_analysis,
)
                ...
                with patch("builtins.input", side_effect=mock_input):
                    with patch("sys.stdin", io.StringIO("y\n")):
                        ...
                        if task_name in task_dict:
                            ...
                            return get_task_samples_direct(
                                task, num_samples=num_samples
                            )
                        else:
                            return {
                                "error": f"Task {task_name} not found even with trust_remote_code handling"
                            }
            except Exception as e:
                print(f"Trust remote code handling failed: {e}")
                return {
                    "error": f"Failed to load task with trust_remote_code handling: {e}"
                }

        # If we still have issues, try fallback approach
        return get_task_samples_fallback(
...
def extract_readme_info(benchmark_name: str) -> Dict[str, any]:
    ...
    try:
        ...
        print(
            f"   ✅ {benchmark_name}: Groups={result['groups']}, Tags={result['tags']}, Tasks={result['tasks'][:3]}{'...' if len(result['tasks']) > 3 else ''}"
        )
...
def determine_skill_risk_tags(
    benchmark_name: str, readme_content: str = ""
) -> List[str]:
    """
...
def update_benchmark_from_readme(benchmark_name: str, current_config: Dict) -> Dict:
    ...
    if not benchmark_dir:
        print(
            f"   ⚠️  Directory not found for {benchmark_name}, using auto-generated tags"
        )
    ...
    if readme_info["tags"]:
        ...
        updated_config = {
            "task": current_config.get("task", benchmark_name),
            "tags": determine_skill_risk_tags(
                benchmark_name
            ),  # Still use skill/risk tags
...
def test_single_benchmark_direct(benchmark_name: str, benchmark_config: dict) -> bool:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if (
                result.stderr
                and "trust_remote_code" in result.stderr.lower()
                and not trust_remote_code
            ):
                print(
                    "🔄 Detected trust_remote_code error, but benchmark not configured for it"
                )
                print(
                    "💡 Consider adding 'trust_remote_code': True to benchmark config"
                )
...
def test_benchmark_creation(
    ...
    try:
        ...
        if "error" in result:
            ...
            if "trust_remote_code" in error_msg and not trust_remote_code:
                ...
                if "error" not in result:
                    ...
                    print(
                        "❌ Still failed with trust_remote_code=True - WILL CAUSE SCRIPT TO EXIT"
                    )
                    return False, tags
                ...
                if alternative_results:
                    ...
                    print(
                        "❌ No alternative task names worked - WILL CAUSE SCRIPT TO EXIT"
                    )
                    return False, tags
            else:
        ...
        if not result.get("samples"):
            ...
            if not use_subtasks:
                ...
                if result.get("samples"):
                    ...
                    print(
                        "❌ No samples found with subtask approach - WILL CAUSE SCRIPT TO EXIT"
                    )
                    return False, tags
            else:
                print(
                    "❌ No samples found even with subtask approach - WILL CAUSE SCRIPT TO EXIT"
                )
                return False, tags
        ...
        try:
            ...
            result = get_task_samples_fallback(
                task_name, num_samples=5, trust_remote_code=trust_remote_code
            )
            if result.get("samples"):
        ...
        try:
            ...
            if task_name in task_dict:
                ...
                if hasattr(task, "get_task_names") or hasattr(task, "get_tasks"):
                    ...
                    print(
                        f"📦 Found {len(subtask_names)} subtasks, trying first few..."
                    )
                    ...
                    return {
                        "error": f"No samples could be retrieved from any subtasks of {task_name}"
                    }
                else:
...
def get_task_samples_fallback(
    ...
    try:
        ...
        approaches = [
            ...
            lambda: try_alternative_task_names(
                task_name, task_name, num_samples, trust_remote_code
            ),
            # Try with subtasks
            lambda: get_task_samples_with_subtasks(
                task_name, num_samples, trust_remote_code
            ),
        ]
...
def find_most_relevant_benchmarks(
    ...
    if priority != "all" or fast_only or time_budget_minutes is not None:
        benchmarks = apply_priority_filtering(
    ...
    for benchmark_name, benchmark_config in benchmarks.items():
        ...
        benchmark_list.append(
            {"name": benchmark_name, "description": description, "tags": tags}
        )
    ...
    try:
        ...
        if prefer_fast:
            benchmark_results.sort(
                key=lambda x: (x["score"], -x["loading_time"]), reverse=True
            )
        else:
        ...
        for benchmark_name, benchmark_config in benchmarks.items():
            ...
            if any(
                ...
                for word in ["what is", "who is", "where is", "capital"]
            ):
                if any(
                    word in description_lower
                    for word in ["knowledge", "factual", "trivia"]
                ):
                    score += 3
            ...
            if any(
                word in prompt_lower
                for word in ["code", "program", "python", "function"]
            ):
                if any(
                    word in description_lower
                    for word in ["code", "programming", "python"]
                ):
                    score += 3
            ...
            if any(
                word in prompt_lower
                for word in ["math", "calculate", "solve", "number"]
            ):
                if any(
                    word in description_lower
                    for word in ["math", "arithmetic", "calculation"]
                ):
                    score += 3
            ...
            if any(
                word in prompt_lower
                for word in ["doctor", "medicine", "health", "symptom"]
            ):
                if any(
                    word in description_lower
                    for word in ["medical", "health", "clinical"]
                ):
                    score += 3
            ...
            # Tag matching
            for tag in tags:
                if tag == "general knowledge" and any(
                    word in prompt_lower for word in ["what", "who", "where"]
                ):
                    score += 2
                    reasons.append("general knowledge tag")
                elif tag == "coding" and any(
                    word in prompt_lower for word in ["code", "program"]
                ):
                    score += 2
                    reasons.append("coding tag")
                elif tag == "mathematics" and any(
                    word in prompt_lower for word in ["math", "calculate"]
                ):
                    score += 2
                    reasons.append("mathematics tag")
                elif tag == "medical" and any(
                    word in prompt_lower for word in ["health", "medicine"]
                ):
                    score += 2
...
def print_priority_summary():
    ...
    print(
        f"   Count: {priority_counts['high']} ({priority_counts['high'] / total * 100:.1f}%)"
    )
    ...
    print(
        f"   Count: {priority_counts['medium']} ({priority_counts['medium'] / total * 100:.1f}%)"
    )
    ...
    print(
        f"   Count: {priority_counts['low']} ({priority_counts['low'] / total * 100:.1f}%)"
    )
    ...
    print(
        f"   • HIGH + MEDIUM = {priority_counts['high'] + priority_counts['medium']} benchmarks ({(priority_counts['high'] + priority_counts['medium']) / total * 100:.1f}%) suitable for agentic use"
    )
...
def main():
    ...
    print(
        f"📋 Processing {len(updated_benchmarks)} comprehensive benchmarks across all categories..."
    )
    print(
        "🎯 Categories: Benchmark Suites, Hallucination, Reasoning, QA/Reading, Knowledge, Math, Coding"
    )
    print(
        "🎯           Bias/Toxicity, Adversarial, Multilingual, Medical, Language Modeling, Long Context"
    )
    ...
    for benchmark_name, benchmark_config in updated_benchmarks.items():
        ...
        dataset_success, actual_tags = test_benchmark_creation(
        ...
        print(f"{'=' * 80}")
        print(
            f"✅ Dataset creation successful: {len(results['dataset_creation']['successful'])}"
        )
        print(
            f"❌ Dataset creation failed: {len(results['dataset_creation']['failed'])}"
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "test_readme":
            ...
            print(
                "Usage: python only_benchmarks.py [test_readme|test_matching|priorities|high|medium|low|find <prompt>]"
            )
    else:
        main()
