...
def find_working_task_from_group(
    group_dict: Dict, depth: int = 0, max_depth: int = 3
) -> Any:
    """
    ...
    for item_name, item in items[:3]:  # Try up to 3 items at each level
        ...
        try:
            ...
            if has_docs:
                print(
                    f"{indent}✅ Found working task '{item_name}' with {len(test_docs)} documents"
                )
                return item
            else:
...
def get_benchmark_tags_with_llama(
    task_name: str, readme_content: str = ""
) -> List[str]:
    """
    ...
    try:
        ...
        description = (
            readme_content[:1500]
            if readme_content
            else f"A benchmark called '{task_name}' for evaluating language models."
        )
...
"""
        ...
        for line in lines[:5]:  # Look at first 5 lines
            ...
            for tag in all_approved_tags:
                if (
                    tag.lower() == clean_line.lower()
                    or clean_line.lower() in tag.lower()
                ):
        ...
        if readme_content:
            ...
            if any(
                word in content_lower for word in ["math", "arithmetic", "calculation"]
            ):
            ...
            if any(
                ...
                for word in ["adversarial", "robust", "challenging"]
            ):
                determined_tags.append("adversarial robustness")
            if any(
                word in content_lower for word in ["bias", "fairness", "stereotype"]
            ):
                determined_tags.append("bias")
            if any(
                word in content_lower
                for word in ["truthful", "hallucination", "factual"]
            ):
...
def get_benchmark_groups_from_readme(task_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        for line in lines:
            ...
            if (
                line.lower().startswith("groups")
                or line.lower().startswith("## groups")
            ...
            if (
                in_target_section
                and line.startswith("##")
                and not any(x in line.lower() for x in ["groups", "tags"])
            ):
                break
            ...
            if in_target_section and line:
                ...
                if line.startswith("* `") and "`" in line and ":" in line:
                    ...
                    if start > 0 and end > start:
                        ...
                        if (
                            group_name and "-" in group_name
                        ):  # Looks like a group task name
        ...
        return {"groups": valid_groups, "tags": determined_tags}
...
def get_samples_from_group_task(
    group_name: str, subtasks: List[str], num_samples: int = 5
) -> Dict[str, Any]:
    """
    ...
    """
    print(
        f"🎯 Getting samples from group task '{group_name}' with {len(subtasks)} subtasks..."
    )

    all_samples = []
    samples_per_task = max(
        1, num_samples // len(subtasks)
    )  # At least 1 sample per task
    ...
    for i, subtask in enumerate(
        subtasks[:num_samples]
    ):  # Limit to prevent too many subtasks
        try:
            print(
                f"   📊 Getting samples from subtask {i + 1}/{min(len(subtasks), num_samples)}: '{subtask}'..."
            )
            ...
            sample_docs = (
                docs[:samples_per_task] if len(docs) >= samples_per_task else docs
            )
    ...
    print(
        f"✅ Successfully retrieved {len(all_samples)} samples from {group_name} group task"
    )

    # Get description from the group context
    subtask_names = list(set([sample["subtask"] for sample in all_samples]))
    description = (
        f"Group task '{group_name}' containing subtasks: {', '.join(subtask_names)}"
    )
...
def get_task_samples_for_analysis(
    task_name: str, num_samples: int = 5
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            ...
            if len(expanded_tasks) == 1:
                ...
                # Group task
                print(
                    f"✅ Found group task '{task_name}' with {len(expanded_tasks)} subtasks: {expanded_tasks[:5]}{'...' if len(expanded_tasks) > 5 else ''}"
                )
            ...
            try:
                ...
                if subtasks:
                    print(
                        f"✅ Found group expansion with {len(subtasks)} subtasks: {subtasks[:3]}{'...' if len(subtasks) > 3 else ''}"
                    )
                ...
                try:
                    ...
                    if task_name in all_groups:
                        print(
                            "✅ Found in groups registry, attempting large expansion..."
                        )
                        ...
                        if expanded_dict:
                            ...
                            if len(all_subtasks) > 1:
                                print(
                                    f"✅ Large group expansion: {len(all_subtasks)} subtasks"
                                )
                                return get_samples_from_group_task(
                    ...
                    print(
                        f"⚠️  Large group failed, trying to read README for '{task_name}'..."
                    )
                    try:
                        ...
                        if benchmark_groups:
                            print(
                                f"✅ Found {len(benchmark_groups)} groups from README: {benchmark_groups}"
                            )
                            ...
                            for group_name in benchmark_groups:
                                ...
                                try:
                                    group_task_dict = evaluator.get_task_dict(
                                        [group_name]
                                    )
        ...
        description = getattr(
            task, "DESCRIPTION", getattr(task, "__doc__", f"Task: {task_name}")
        )
            ...
            if len(docs) > 100:
                ...
                for i in range(num_samples):
                    ...
                    random_idx = random.randint(
                        start_idx, min(end_idx - 1, len(docs) - 1)
                    )
                ...
                # Special handling for TruthfulQA format
                    ...
                    if "mc1_targets" in doc:
                        ...
                        if "choices" in mc1_targets:
                            sample["choices"] = [
                                str(choice) for choice in mc1_targets["choices"]
                            ]
                ...
                if key not in [
                    ...
                    try:
                        ...
                        if isinstance(value, dict):
                            ...
                            sample["additional_info"][key] = str(value)[
                                :200
...
def load_lm_eval():
    ...
    try:
        ...
        print(
            "Error: lm-evaluation-harness is required. Install with: pip install lm-eval"
        )
...
def get_task_info(
    task_name: str, get_task_dict, task_registry
    ...
    try:
        ...
        if task_name not in task_dict:
            ...
            print(
                f"  Task {task_name} not found directly, checking if it's a group task..."
            )
            ...
            print(
                f"  Found group task with {len(sub_tasks)} sub-tasks: {sub_tasks[:3]}{'...' if len(sub_tasks) > 3 else ''}"
            )
        ...
        if docs:
            ...
            if not info["example_bad_response"]:
                if "bool" in task_name.lower():
                    info["example_bad_response"] = (
                        "No"
                        if info["example_good_response"]
                        .strip()
                        .lower()
                        .startswith("yes")
                        else "Yes"
...
def process_group_task(
    group_name: str, sub_tasks: List[str], get_task_dict
) -> Dict[str, Any]:
    ...
    print(
        f"  Analyzing all {len(sub_tasks)} sub-tasks for evaluation methods and categories..."
    )
    ...
    info = {
        ...
        "evaluation_method": list(all_eval_methods)
        if all_eval_methods
...
def get_relevant_benchmarks_for_prompt(
    prompt: str,
    ...
    try:
        ...
        try:
            ...
            if priority != "all" or fast_only or time_budget_minutes is not None:
                filtered_benchmarks = apply_priority_filtering(
                    BENCHMARKS, priority, fast_only, time_budget_minutes
                )
            else:
            ...
            benchmark_descriptions = {
                ...
                for name, info in filtered_benchmarks.items()
            }
        except ImportError:
    ...
    try:
        ...
        for name, desc in benchmark_descriptions.items():
            if name in filtered_benchmarks:
                ...
                benchmark_list_with_timing.append(
                    f"- {name}: {desc} (loading time: {loading_time:.1f}s, priority: {priority})"
                )
            else:
                benchmark_list_with_timing.append(
                    f"- {name}: {desc} (loading time: ~60s, priority: unknown)"
                )
...
"""
                ...
                if len(parts) >= 2:
                    ...
                    for available in available_benchmarks:
                        if available.lower() == benchmark_name.lower():
                            ...
                            benchmark_name.lower() in available.lower()
                            or available.lower() in benchmark_name.lower()
                        ):
                            matched_benchmark = available
                            break
        ...
        if len(relevant_benchmarks) < max_benchmarks:
            ...
            for fallback in fallback_benchmarks:
                if fallback in available_benchmarks and not any(
                    rb["benchmark"] == fallback for rb in relevant_benchmarks
                ):
        ...
        for i, rb in enumerate(relevant_benchmarks, 1):
            priority_str = (
                f" (priority: {rb.get('priority', 'unknown')})"
                if rb.get("priority") != "unknown"
                else ""
            )
            loading_time_str = (
                f" (loading time: {rb.get('loading_time', 60.0):.1f}s)"
                if rb.get("loading_time")
                else ""
            )
            print(
                f"   {i}. {rb['benchmark']}: {rb['explanation']}{priority_str}{loading_time_str}"
            )
        ...
        if any(
            ...
            for word in ["food", "eat", "cook", "recipe", "restaurant"]
        ):
        ...
        if any(
            word in prompt_lower for word in ["math", "calculate", "number", "equation"]
        ):
            fallback_results.append(
                {
        ...
        if any(
            word in prompt_lower for word in ["code", "program", "python", "function"]
        ):
            fallback_results.append(
                {
                    "benchmark": "humaneval"
                    if "humaneval" in available_benchmarks
                    else "mmlu",
        ...
        for gb in general_benchmarks:
            if gb in available_benchmarks and not any(
                fr["benchmark"] == gb for fr in fallback_results
            ):
                fallback_results.append(
                    {
                        "benchmark": gb,
...
def main():
    ...
    try:
        ...
        result = subprocess.run(
            ["lm_eval", "--tasks", "list"], capture_output=True, text=True, timeout=60
        )
    ...
    print(
        f"Tasks with details: {sum(1 for task in current_data['tasks'].values() if task)}"
    )
