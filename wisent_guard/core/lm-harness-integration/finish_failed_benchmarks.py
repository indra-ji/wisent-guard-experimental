...
def test_failed_benchmark(benchmark_name: str, benchmark_config: Dict) -> Dict:
    ...
    try:
        ...
        result = get_task_samples_for_analysis(
            task_name=task_name, num_samples=5, trust_remote_code=trust_remote_code
        )
        ...
        if result.get("samples"):
            print(
                f"   ✅ SUCCESS: {len(result['samples'])} samples loaded in {loading_time:.2f}s"
            )
...
def main():
    ...
    results_file = Path(
        "benchmark_timing_results/benchmark_timing_20250705_142530.json"
    )
    ...
    output_file = Path(
        "benchmark_timing_results/benchmark_timing_20250705_142530_fixed.json"
    )
    ...
    print(
        f"   Improvement: {successful_loads - original_results['summary']['successful_loads']} benchmarks fixed"
    )
    print(
        "   Fixed benchmarks: math_qa, crows_pairs, hendrycks_ethics, paws_x, mmmlu, pubmedqa"
    )
    print(
        "   Remaining issues: storycloze (manual download), narrativeqa (large dataset)"
    )
