...
def time_benchmark_loading(
    benchmark_name: str, benchmark_config: dict, num_samples: int = 5
) -> Dict:
    """
    ...
    try:
        ...
        samples_result = get_task_samples_for_analysis(
            task_name, num_samples=num_samples
        )
        ...
        if "error" in samples_result:
            ...
            result["loading_time_seconds"] = (
                loading_time  # Still record time even if failed
            )
            print(
                f"❌ Failed in {loading_time:.2f}s - {samples_result['error'][:100]}..."
            )
        else:
            result["success"] = True
            ...
            if "description" in samples_result:
                result["metadata"]["description"] = (
                    samples_result["description"][:200] + "..."
                    if len(str(samples_result.get("description", ""))) > 200
                    else samples_result.get("description", "")
                )
...
def analyze_timing_results(results: List[Dict]) -> Dict:
    ...
    if successful_results:
        ...
        sorted_by_time = sorted(
            successful_results, key=lambda x: x["loading_time_seconds"]
        )
...
def print_timing_summary(analysis: Dict):
    ...
    if analysis["timing_stats"]:
        ...
        for i, benchmark in enumerate(analysis["fastest_benchmarks"], 1):
            print(
                f"  {i}. {benchmark['name']} - {benchmark['time']:.2f}s ({benchmark['samples']} samples)"
            )
    ...
    if analysis["category_stats"]:
        ...
        sorted_categories = sorted(
            analysis["category_stats"].items(), key=lambda x: x[1]["mean_time"]
        )
        for category, stats in sorted_categories:
            print(
                f"  {category}: {stats['mean_time']:.2f}s avg ({stats['count']} benchmarks)"
            )
