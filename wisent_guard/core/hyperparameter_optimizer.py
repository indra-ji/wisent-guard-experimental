...
class OptimizationConfig:
    ...
    aggregation_methods: List[str] = field(
        default_factory=lambda: ["average", "final", "first", "max", "min"]
    )
...
class HyperparameterOptimizer:
    ...
    def optimize(
        ...
        if layer_range is None:
            ...
            if verbose:
                ...
                print(
                    f"   • Using all layers for optimization: {layer_range[0]}-{layer_range[-1]}"
                )

        if verbose:
            print("\n🔍 Starting hyperparameter optimization...")
            print(
                f"   • Layers to test: {len(layer_range)} (range: {layer_range[0]}-{layer_range[-1]})"
            )
        ...
        if len(combinations) > self.config.max_combinations:
            if verbose:
                print(
                    f"   • Too many combinations ({len(combinations)}), sampling {self.config.max_combinations}"
                )
            combinations = np.random.choice(
                combinations, size=self.config.max_combinations, replace=False
            ).tolist()
        ...
        for i, (layer, aggregation, threshold, classifier_type) in enumerate(
            combinations
        ):
            try:
                if verbose and (i + 1) % 10 == 0:
                    print(
                        f"   • Progress: {i + 1}/{len(combinations)} combinations tested"
                    )
                ...
                if score > best_score:
                    ...
                    if verbose:
                        print(
                            f"   • New best: layer={layer}, agg={aggregation}, thresh={threshold:.2f}, {self.config.metric}={score:.3f}"
                        )

            except Exception as e:
                logger.warning(
                    f"Failed to evaluate combination (layer={layer}, agg={aggregation}, thresh={threshold}, type={classifier_type}): {e}"
                )
                continue
        ...
        if verbose:
            ...
            print(
                f"   • Best {self.config.metric}: {optimization_result.best_score:.3f}"
            )
    ...
    def _evaluate_combination(
        ...
        steering_type = (
            SteeringType.LOGISTIC if classifier_type == "logistic" else SteeringType.MLP
        )
        ...
        for pair in test_pair_set.pairs:
            if hasattr(pair.positive_response, "activations") and hasattr(
                pair.negative_response, "activations"
            ):
                # Get classifier predictions for both responses
                pos_features = (
                    pair.positive_response.activations.extract_features_for_classifier()
                )
                neg_features = (
                    pair.negative_response.activations.extract_features_for_classifier()
                )

                # Predict probabilities
                pos_prob = steering_method.classifier.predict_proba(
                    [pos_features.numpy()]
                )[0]
                neg_prob = steering_method.classifier.predict_proba(
                    [neg_features.numpy()]
                )[0]
        ...
        try:
            ...
            auc = (
                roc_auc_score(true_labels, prob_scores)
