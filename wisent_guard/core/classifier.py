...
class Classifier:
    def __init__(
        self, model_type="logistic", device=None, threshold=0.5, model_path=None
    ):
    ...
    def _extract_features(self, activation_tensor: torch.Tensor) -> torch.Tensor:
        ...
        if not isinstance(activation_tensor, torch.Tensor):
            activation_tensor = torch.tensor(
                activation_tensor, dtype=torch.float32, device=self.device
            )
        else:
            ...
            activation_tensor = activation_tensor.to(
    ...
    def fit(
        ...
        **model_params,
        ...
        for epoch in range(num_epochs):
            ...
            true_positives = sum(
                1 for p, l in zip(all_preds, all_labels) if p == 1 and l == 1
            )
            false_positives = sum(
                1 for p, l in zip(all_preds, all_labels) if p == 1 and l == 0
            )
            false_negatives = sum(
            ...
            if early_stopping_counter >= early_stopping_patience:
                print(
                    f"Early stopping at epoch {epoch + 1} due to no improvement in accuracy"
                )
                break
        ...
        # Final metrics
        test_accuracy = (
            sum(1 for p, l in zip(y_pred, y_true) if p == l) / len(y_pred)
            if len(y_pred) > 0
            else 0.0
        )
    ...
    def predict(self, X):
        ...
        if self.model is None:
            raise ValueError(
                "No model has been trained or loaded. Call fit() or load_model() first."
            )
...
class ActivationClassifier:
    ...
    def __init__(
        ...
        self.classifier = Classifier(
            model_type=model_type, threshold=threshold, device=device
        )
