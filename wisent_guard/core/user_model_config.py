...
class UserModelConfig:
    ...
    def prompt_and_save_config(self, model_name: str) -> Dict[str, Any]:
        ...
        print(
            "   These are the special tokens your model uses to distinguish user and assistant messages."
        )
        print("   Examples:")
        print(
            "   - Llama 3: <|start_header_id|>user<|end_header_id|> and <|start_header_id|>assistant<|end_header_id|>"
        )
        ...
        config["assistant_token"] = input(
            "   Enter the assistant token/prefix: "
        ).strip()

        # Optional system token
        system_token = input(
            "   Enter the system token/prefix (press Enter to skip): "
        ).strip()
        if system_token:
        ...
        while True:
            ...
            if choice == "1":
                ...
                template = input(
                    "   Enter the layer path template (use {idx} for layer index): "
                ).strip()
        ...
        print(
            "   If your model requires a specific prompt format beyond simple token prefixes,"
        )
        print(
            "   you can provide a template. Use {user_message} and {assistant_message} as placeholders."
        )
        print(
        ...
        custom_template = input(
            "\n   Enter custom template (press Enter to skip): "
        ).strip()
        if custom_template:
