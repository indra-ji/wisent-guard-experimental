...
def save_classification_results_csv(results: Dict[str, Any], output_path: str) -> None:
    ...
    try:
        ...
        for task_name, task_results in results.items():
            if (
                not isinstance(task_results, dict)
                or "sample_responses" not in task_results
            ):
                continue
            ...
            for response_data in sample_responses:
                ...
                if is_multi_layer and layer_results:
                    ...
                    for layer in sorted_layers:
                        ...
                        if layer_data.get("token_scores"):
                            token_scores_formatted = [
                                f"{score:.6f}" for score in layer_data["token_scores"]
                            ]
                        ...
                        csv_row[f"aggregated_score_layer_{layer}"] = (
                            f"{layer_data.get('aggregated_score', 0.0):.6f}"
                        )
                        csv_row[f"overall_prediction_layer_{layer}"] = layer_data.get(
                    ...
                    if response_data.get("token_scores"):
                        token_scores_formatted = [
                            f"{score:.6f}" for score in response_data["token_scores"]
                        ]
                        token_scores_str = "|".join(token_scores_formatted)

                    csv_row["token_scores"] = token_scores_str
                    csv_row["aggregated_score"] = (
                        f"{response_data.get('aggregated_score', 0.0):.6f}"
                    )
                    csv_row["overall_prediction"] = response_data.get(
                        "classification", "UNKNOWN"
                    )
        ...
        if csv_rows:
            ...
            if is_multi_layer:
                print(
                    f"   • Multi-layer format with columns for layers: {sorted_layers}"
                )
                print(
                    "   • Token scores, aggregated scores, and predictions saved per layer"
                )
            else:
                print("   • Single-layer format")
            print(
                "   • Fill in the 'ground_truth' column with: 'TRUTHFUL' or 'HALLUCINATION'"
            )
            print("   • Use for manual evaluation and classifier optimization")
        else:
            logger.info(
                "No classification results to export (steering mode or empty results)"
            )
...
def create_evaluation_report(results: Dict[str, Any], output_path: str) -> None:
    ...
    try:
        with open(output_path, "w") as f:
            ...
            f.write("## Summary\n\n")
            f.write(
                "| Task | Training Accuracy | Evaluation Accuracy | Optimization |\n"
            )
            f.write("|------|------------------|--------------------|--------------|\n")
            ...
            for task_name, task_results in results.items():
                if task_results is None:
                    ...
                    train_acc = task_results.get("training_results", {}).get(
                        "accuracy", "N/A"
                    )
                    eval_acc = task_results.get("evaluation_results", {}).get(
                        "accuracy", "N/A"
                    )
                    optimized = (
                        "Yes"
                    ...
                    f.write(
                        f"| {task_name} | {train_acc} | {eval_acc} | {optimized} |\n"
                    )
            ...
            for task_name, task_results in results.items():
                ...
                if task_results is None:
                    ...
                    f.write(
                        f"- **Model**: {task_results.get('model_name', 'Unknown')}\n"
                    )
                    f.write(f"- **Layer**: {task_results.get('layer', 'Unknown')}\n")
                    f.write(
                        f"- **Classifier**: {task_results.get('classifier_type', 'Unknown')}\n"
                    )
                    f.write(
                        f"- **Token Aggregation**: {task_results.get('token_aggregation', 'Unknown')}\n"
                    )
                    f.write(
                        f"- **Ground Truth Method**: {task_results.get('ground_truth_method', 'Unknown')}\n"
                    )
                    ...
                    if "evaluation_results" in task_results:
                        ...
                        f.write(
                            f"- **Total Predictions**: {eval_results.get('total_predictions', 'N/A')}\n"
                        )
                        f.write(
                            f"- **Correct Predictions**: {eval_results.get('correct_predictions', 'N/A')}\n"
                        )
                    ...
                    if task_results.get("optimization_performed", False):
                        f.write("\n### Optimization Results\n")
                        f.write(
                            f"- **Best Layer**: {task_results.get('best_layer', 'Unknown')}\n"
                        )
                        f.write(
                            f"- **Best Aggregation**: {task_results.get('best_aggregation', 'Unknown')}\n"
                        )
