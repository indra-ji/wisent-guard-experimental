...
class HPR(SteeringMethod):
    ...
    def train(
        ...
        if vector_norm > 1e-8:
            ...
            identity = torch.eye(
                len(v_normalized), device=self.device, dtype=v_normalized.dtype
            )
            ...
            self.householder_matrix = torch.eye(
                len(self.steering_vector),
                device=self.device,
                dtype=self.steering_vector.dtype,
            )
    ...
    def apply_steering(
        ...
        if strength != 0:
            ...
            identity = torch.eye(
                hidden_dim, device=activations.device, dtype=activations.dtype
            )
            effective_matrix = (
                1 - alpha
            ) * identity + alpha * self.householder_matrix.to(activations.device)
    ...
    def get_steering_vector(self) -> torch.Tensor:
        ...
        if not self.is_trained:
            raise ValueError(
                "HPR method must be trained before getting steering vector"
            )
