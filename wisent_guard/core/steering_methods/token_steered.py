...
class TokenSteeringStrategy(Enum):
    ...
    SECOND_TO_LAST = (
        "second_to_last"  # Only steer the second-to-last token (reference behavior)
    )
    FIRST_ONLY = "first_only"  # Only steer the first token
    ALL_EQUAL = "all_equal"  # Apply equal steering to all tokens
    EXPONENTIAL_DECAY = (
        "exponential_decay"  # Exponentially decreasing from first to last
    )
    EXPONENTIAL_GROWTH = (
        "exponential_growth"  # Exponentially increasing from first to last
    )
...
class TokenSteeringConfig:
    ...
    def __init__(
        ...
        if strategy == TokenSteeringStrategy.CUSTOM and custom_function is None:
            raise ValueError(
                "custom_function must be provided when using CUSTOM strategy"
            )
...
class TokenSteeringMixin:
    ...
    def compute_token_strengths(
        self, sequence_length: int, base_strength: float = 1.0, prompt_length: int = 0
    ) -> torch.Tensor:
        """
        ...
        strengths[start_idx : start_idx + generation_length] = (
            generation_strengths * base_strength
        )

        # Apply prompt token strengths if enabled
        if config.apply_to_prompt and prompt_length > 0:
            prompt_strengths = torch.full(
                (prompt_length,), base_strength * config.prompt_strength_multiplier
            )
    ...
    def _compute_strategy_strengths(
        ...
        if strategy == TokenSteeringStrategy.LAST_ONLY:
            ...
            strengths = max_strength - normalized_positions * (
            ...
            normalized_positions = positions / (length - 1)
            strengths = min_strength + normalized_positions * (
                max_strength - min_strength
            )
            ...
            strengths = torch.tensor(
                [custom_function(i, length) for i in range(length)], dtype=torch.float32
            )
    ...
    def apply_token_steering(
        ...
        if len(activations.shape) == 2:
            ...
            token_strengths = self.compute_token_strengths(
                seq_length, base_strength, prompt_length
            ...
            for token_idx in range(seq_length):
                if token_strengths[token_idx] > 0:
                    steered[:, token_idx : token_idx + 1, :] = steered[
                        :, token_idx : token_idx + 1, :
                    ] + token_strengths[token_idx] * steering_vector.unsqueeze(
                        0
                    ).unsqueeze(0)
...
class TokenSteeringWrapper(SteeringMethod):
    ...
    def train(
        self, contrastive_pair_set: ContrastivePairSet, layer_index: int
    ) -> Dict[str, Any]:
    ...
    def apply_steering(
        self, activations: torch.Tensor, strength: float = 1.0, **kwargs
    ) -> torch.Tensor:
        """Apply steering with token-level control."""
        ...
        return self.apply_token_steering(
            activations, steering_vector, strength, prompt_length
        )
    ...
    def load_steering_vector(self, path: str) -> bool:
        ...
        try:
            ...
            self.token_steering_config = TokenSteeringConfig(
                ...
                prompt_strength_multiplier=config_data.get(
                    "prompt_strength_multiplier", 0.1
                ),
            )
        except Exception:
...
def attention_weighted_strength(
...
def gaussian_strength(
    center: float = 0.5, width: float = 0.3
) -> Callable[[int, int], float]:
    """
...
def step_function_strength(
    step_position: float = 0.5,
    before_strength: float = 1.0,
    after_strength: float = 0.1,
) -> Callable[[int, int], float]:
    """
