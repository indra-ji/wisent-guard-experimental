...
class BiPO(SteeringMethod):
    ...
    def train(
        ...
        self.steering_vector = torch.zeros(
            vector_dim, device=self.device, requires_grad=True
        )
        ...
        for epoch in range(self.num_epochs):  # Use full epoch count
            ...
            if pos_scores and neg_scores:
                ...
                loss = -torch.mean(
                    torch.log(torch.sigmoid(self.beta * (pos_scores - neg_scores)))
                )
    ...
    def get_steering_vector(self) -> torch.Tensor:
        ...
        if not self.is_trained:
            raise ValueError(
                "BiPO method must be trained before getting steering vector"
            )
