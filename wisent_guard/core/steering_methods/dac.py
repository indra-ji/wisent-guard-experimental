...
class DAC(SteeringMethod):
    ...
    def apply_steering(
        ...
        if self.dynamic_control:
            if token_probs is not None:
                ...
                entropy = -torch.sum(
                    token_probs * torch.log(token_probs + 1e-10), dim=-1
                )
                entropy_factor = torch.clamp(
                    self.entropy_threshold / (entropy + 1e-10), 0.1, 2.0
                )
        ...
        if len(activations.shape) == 3:  # [batch, seq, hidden]
            ...
            if activations.shape[1] > 1:
                ...
                steered[:, -2:-1, :] = steered[
                    :, -2:-1, :
                ] + dynamic_strength * steering_vector.unsqueeze(0).unsqueeze(0)
            else:
    ...
    def get_steering_vector(self) -> torch.Tensor:
        ...
        if not self.is_trained:
            raise ValueError(
                "DAC method must be trained before getting steering vector"
            )
