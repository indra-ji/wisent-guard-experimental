...
class CAA(SteeringMethod):
    ...
    def train(
        ...
        self.steering_vector, training_stats = (
            create_control_vector_from_contrastive_pairs(
        ...
        if self.normalizer and self.normalization_method != "none":
            if self.normalization_method == "cross_behavior":
                ...
                normalized_vectors = self.normalizer.normalize_cross_behavior(
                    vectors, target_norm=self.target_norm
                )
                self.steering_vector = normalized_vectors[0]
            elif self.normalization_method == "l2_unit":
                self.steering_vector = self.normalizer.normalize_l2_unit(
    ...
    def apply_steering(
        self, activations: torch.Tensor, strength: float = 1.0, verbose: bool = False
    ) -> torch.Tensor:
        """
        ...
        if verbose:
            ...
            print(
                f"   Input norm (mean): {torch.norm(activations, dim=-1).mean().item():.4f}"
            )
        ...
        if len(activations.shape) == 3:  # [batch, seq, hidden]
            ...
            if activations.shape[1] > 1:
                ...
                print(f"      Vector norm: {torch.norm(steering_vector).item():.4f}")
                print(
                    f"      Effective addition norm: {torch.norm(strength * steering_vector).item():.4f}"
                )

                steered[:, -2:-1, :] = steered[
                    :, -2:-1, :
                ] + strength * steering_vector.unsqueeze(0).unsqueeze(0)
    ...
    def get_steering_vector(self) -> torch.Tensor:
        ...
        if not self.is_trained:
            raise ValueError(
                "CAA method must be trained before getting steering vector"
            )
    ...
    def load_steering_vector(self, path: str) -> bool:
        ...
        try:
            ...
            self.aggregation_method = ControlVectorAggregationMethod(
                data.get("aggregation_method", "CAA")
            )
