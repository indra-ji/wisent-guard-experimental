...
class KSteering(SteeringMethod):
    ...
    def train(
        ...
        try:
            pos_activations, neg_activations = (
                contrastive_pair_set.get_activation_pairs()
            )
        ...
        activation_tensor = torch.stack(
            [act.to(self.device) for act in all_activations]
        )
        ...
        self.training_stats = {
            ...
            "final_classifier_loss": classifier_losses[-1]
            ...
            // 2,  # Approximate since we don't have true pairs
        }
    ...
    def apply_steering(
        ...
        if not self.is_trained:
            raise ValueError(
                "K-steering method must be trained before applying steering"
            )
    ...
    def get_steering_vector(self) -> torch.Tensor:
        ...
        if not self.is_trained:
            raise ValueError(
                "K-steering method must be trained before getting steering vector"
            )
    ...
    def set_targets(
        self, target_labels: List[int], avoid_labels: Optional[List[int]] = None
    ):
        """
    ...
    def get_classifier_probabilities(self, activations: torch.Tensor) -> torch.Tensor:
        ...
        if not self.is_trained:
            raise ValueError(
                "K-steering method must be trained before getting probabilities"
            )
