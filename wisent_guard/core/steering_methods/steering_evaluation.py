...
def run_lm_harness_evaluation(
    ...
    try:
        ...
        class SteeredModelWrapper(LM):
            ...
            def __init__(
                self, wisent_model, steering_methods, layers, steering_strength=1.0
            ):
            ...
            def tok_encode(self, string: str):
                return self.wisent_model.tokenizer.encode(
            ...
            def generate_until(self, requests, disable_tqdm: bool = False):
                ...
                for request in requests:
                    try:
                        ...
                        inputs = self.wisent_model.tokenizer(
                            context, return_tensors="pt"
                        ).to(self.wisent_model.device)
            ...
            def loglikelihood(self, requests, disable_tqdm: bool = False):
                ...
                print(
                    f"   Steering methods: {len(self.steering_methods) if self.steering_methods else 0}"
                )
                ...
                for i, request in enumerate(_requests):
                    ...
                    try:
                        ...
                        if len(request) >= 2:
                            ...
                            print(
                                f"      Context: '{context[:50] if context else '(empty)'}...'"
                            )
                            ...
                            print(
                                f"      Fallback parsing - continuation only: '{continuation}'"
                            )
                        ...
                        print(
                            f"         Context: '{context[:50]}...' -> {len(context_tokens)} tokens"
                        )
                        print(
                            f"         Continuation: '{continuation[:50]}...' -> expected {len(continuation_tokens)} tokens"
                        )
                        print(
                            f"         Full text: '{full_text[:50]}...' -> {len(full_tokens)} tokens"
                        )
                        ...
                        input_ids = torch.tensor(
                            [full_tokens], device=self.wisent_model.device
                        )
                        ...
                        if self.steering_methods and self.layers:
                            ...
                            print(
                                f"      Methods: {len(self.steering_methods)}, layers: {self.layers}, strength: {self.steering_strength}"
                            )
                            for i, method in enumerate(self.steering_methods):
                                print(
                                    f"      Method {i}: {type(method).__name__}, trained: {getattr(method, 'is_trained', 'unknown')}"
                                )
                                if (
                                    hasattr(method, "steering_vector")
                                    and method.steering_vector is not None
                                ):
                                    print(
                                        f"      Vector shape: {method.steering_vector.shape}, norm: {torch.norm(method.steering_vector).item():.4f}"
                                    )

                            hooks = []
                            try:
                                print(
                                    f"      🔧 Registering {len(self.layers)} hooks for layers: {self.layers}"
                                )
                                ...
                                def create_steering_hook(steering_method):
                                    def steering_hook(module, input, output):
                                        try:
                                            ...
                                            if verbose:
                                                print(
                                                    f"   🔍 Hook called at layer {layer_idx}, output type: {type(output)}, hidden_states shape: {hidden_states.shape if hasattr(hidden_states, 'shape') else 'no shape'}"
                                                )

                                            if (
                                                hasattr(hidden_states, "shape")
                                                ...
                                                if verbose:
                                                    print(
                                                        f"   🔍 Hidden states shape valid: [{batch_size}, {seq_len}, {hidden_dim}]"
                                                    )

                                                # Handle KSteering differently from vector-based methods
                                                if (
                                                    steering_method.__class__.__name__
                                                    == "KSteering"
                                                ):
                                                    if verbose:
                                                        print(
                                                            "   🔍 KSteering detected, applying gradient-based steering"
                                                        )

                                                    # KSteering uses apply_steering method directly
                                                    steered_hidden_states = steering_method.apply_steering(
                                                        hidden_states,
                                                    ...
                                                    if verbose:
                                                        print(
                                                            f"   🎯 Successfully applied KSteering at layer {layer_idx}, strength={self.steering_strength}"
                                                        )
                                                ...
                                                # Handle vector-based steering methods (CAA, HPR, DAC, BiPO)
                                                    ...
                                                    and steering_method.steering_vector
                                                    is not None
                                                ):
                                                    steering_vector = (
                                                        steering_method.steering_vector
                                                    )
                                                    if verbose:
                                                        print(
                                                            f"   🔍 Steering vector found, shape: {steering_vector.shape}"
                                                        )
                                                    ...
                                                    if (
                                                        ...
                                                        print(
                                                            f"         🔄 HOOK CALLED at layer {layer_idx}"
                                                        )
                                                        print(
                                                            f"         Hidden states shape: {hidden_states.shape}"
                                                        )
                                                        print(
                                                            f"         Hidden norm before: {torch.norm(hidden_states, dim=-1).mean().item():.4f}"
                                                        )
                                                        ...
                                                        print(
                                                            f"         Hidden norm after: {torch.norm(steered_hidden_states, dim=-1).mean().item():.4f}"
                                                        )
                                                        print(
                                                            f"         Applied strength: {self.steering_strength}"
                                                        )
                                                        ...
                                                        if torch.any(
                                                            ...
                                                            print(
                                                                "         ⚠️ WARNING: Steered hidden states contain NaN or Inf values!"
                                                            )
                                                            print(
                                                                f"         Max value: {steered_hidden_states.max().item()}"
                                                            )
                                                            print(
                                                                f"         Min value: {steered_hidden_states.min().item()}"
                                                            )
                                                        ...
                                                        if verbose:
                                                            print(
                                                                f"   ⚠️ Dimension mismatch: vector {steering_vector.shape[-1]} vs hidden {hidden_dim}"
                                                            )
                                                else:
                                                    if verbose:
                                                        print(
                                                            f"   ⚠️ No steering vector found on method {steering_method.__class__.__name__}"
                                                        )
                                            else:
                                                if verbose:
                                                    print(
                                                        "   ⚠️ Invalid hidden states shape for steering"
                                                    )
                                ...
                                steering_hook_fn = create_steering_hook(
                                ...
                                for layer_idx in self.layers:
                                    ...
                                    if hasattr(self.wisent_model.model, "transformer"):
                                        # GPT2 style
                                        layer = self.wisent_model.model.transformer.h[
                                            layer_idx
                                        ]
                                    elif hasattr(self.wisent_model.model, "model"):
                                        # LLaMA style
                                        layer = self.wisent_model.model.model.layers[
                                            layer_idx
                                        ]
                                    else:
                                        raise ValueError(
                                            f"Unknown model architecture for {type(self.wisent_model.model)}"
                                        )
                                ...
                                print(
                                    f"         Contains inf: {torch.any(torch.isinf(logits)).item()}"
                                )
                                print(
                                    f"         Contains nan: {torch.any(torch.isnan(logits)).item()}"
                                )
                        ...
                        if continuation_start >= logits.shape[1]:
                            print(
                                f"      ❌ Continuation start {continuation_start} >= logits seq length {logits.shape[1]}"
                            )
                            print("      Returning -inf")
                            results.append((float("-inf"), False))
                            continue
                        ...
                        target_logits = logits[
                            0, continuation_start - 1 : continuation_end - 1
                        ]  # Shape: [cont_len, vocab_size]
                        target_tokens = torch.tensor(
                            continuation_tokens, device=self.wisent_model.device
                        )
                        ...
                        if torch.any(torch.isinf(target_logits)) or torch.any(
                            torch.isnan(target_logits)
                        ):
                            print(
                                "      ⚠️ CRITICAL: Target logits already contain inf/nan!"
                            )
                            print(
                                f"      Inf count in logits: {torch.isinf(target_logits).sum().item()}"
                            )
                            print(
                                f"      NaN count in logits: {torch.isnan(target_logits).sum().item()}"
                            )
                        ...
                        if torch.any(torch.isinf(log_probs)) or torch.any(
                            torch.isnan(log_probs)
                        ):
                            print("      ⚠️ Log probs contain inf/nan!")
                            print(
                                f"      Inf count: {torch.isinf(log_probs).sum().item()}"
                            )
                            print(
                                f"      NaN count: {torch.isnan(log_probs).sum().item()}"
                            )

                        # Get log probability for each target token
                        print(
                            f"      Extracting token log probs for tokens: {target_tokens[:5].tolist()}"
                        )
                        token_log_probs = log_probs[
                            range(len(continuation_tokens)), target_tokens
                        ]

                        print(f"      Token log probs: {token_log_probs[:5].tolist()}")

                        # Check each token probability individually
                        for idx, (token_id, log_prob) in enumerate(
                            zip(target_tokens[:5], token_log_probs[:5])
                        ):
                            print(
                                f"      Token {idx}: id={token_id}, log_prob={log_prob:.4f}, finite={torch.isfinite(log_prob).item()}"
                            )
        ...
        print(
            f"   Created unsteered model with {len(unsteered_model.steering_methods)} methods, strength={unsteered_model.steering_strength}"
        )
        task_dict = {task_data.config.task: task_data}

        baseline_results = evaluate(
            unsteered_model, task_dict, limit=len(test_qa_pairs), bootstrap_iters=0
        )

        # Extract baseline log-likelihoods
        baseline_samples = baseline_results.get("samples", {}).get(
        ...
        if baseline_samples:
            ...
            if verbose:
                ...
                print(
                    f"   First baseline likelihoods: {baseline_likelihoods[:3] if baseline_likelihoods else 'None'}"
                )
                print(
                    f"   Sample ID/hash: {id(sample)}"
        ...
        if verbose:
            ...
            print(
                f"   Steering methods: {[m.__class__.__name__ for m in steering_methods] if steering_methods else 'None'}"
            )
            print(f"   Steering layers: {layers}")
            print(f"   Steering strength: {steering_strength}")
            if steering_methods and len(steering_methods) > 0:
                print(
                    f"   First method has vector: {hasattr(steering_methods[0], 'steering_vector') and steering_methods[0].steering_vector is not None}"
                )
                if (
                    hasattr(steering_methods[0], "steering_vector")
                    and steering_methods[0].steering_vector is not None
                ):
                    print(
                        f"   Vector shape: {steering_methods[0].steering_vector.shape}"
                    )
                    print(
                        f"   Vector norm: {torch.norm(steering_methods[0].steering_vector).item():.4f}"
                    )

        steered_model = SteeredModelWrapper(
            model, steering_methods, layers, steering_strength
        )

        steered_results = evaluate(
            steered_model, task_dict, limit=len(test_qa_pairs), bootstrap_iters=0
        )

        # Extract steered log-likelihoods
        steered_samples = steered_results.get("samples", {}).get(
            task_data.config.task, []
        )
        steered_likelihoods = []
        if steered_samples:
        ...
        if (
            verbose
            and baseline_likelihoods
            and steered_likelihoods
            and output_mode in ["likelihoods", "both"]
        ):
            print("\n📊 LOG-LIKELIHOOD COMPARISON:")
            print(f"   Steering Strength: {steering_strength}")
            print(
                f"   Method: {steering_methods[0].__class__.__name__ if steering_methods else 'None'}"
            )
            ...
            if baseline_samples:
                ...
                for i, (baseline, steered) in enumerate(
                    zip(baseline_likelihoods, steered_likelihoods)
                ):
                    ...
                    choice_text = (
                        choices[i][:60] + "..."
                        if i < len(choices) and len(choices[i]) > 60
                        else choices[i]
                        if i < len(choices)
                        else f"Choice {i}"
                    )
                    ...
                    print(
                        f"         Change:    {change:8.2f} ({'+' if change > 0 else ''}{change:6.2f}) {status}"
                    )
                    print()
                ...
                print(
                    f"      Unsteered model prefers: {chr(65 + unsteered_best)} (likelihood: {max(baseline_likelihoods):.2f})"
                )
                print(
                    f"      Steered model prefers:   {chr(65 + steered_best)} (likelihood: {max(steered_likelihoods):.2f})"
                )

                if unsteered_best != steered_best:
                    print(
                        f"      🔄 Steering changed preference from {chr(65 + unsteered_best)} to {chr(65 + steered_best)}"
                    )
                else:
                    print(
                        f"      ➡️ Steering maintained preference for {chr(65 + steered_best)}"
                    )
        ...
        if verbose and baseline_samples and output_mode in ["responses", "both"]:
            ...
            try:
                ...
                inputs = model.tokenizer(generation_prompt, return_tensors="pt").to(
                    model.device
                )
                ...
                print(
                    f"   🎯 Generating steered response (strength {steering_strength})..."
                )
                    ...
                    if steering_methods and layers:
                                ...
                                try:
                                    ...
                                    if (
                                        ...
                                        if (
                                            ...
                                            # KSteering uses apply_steering method directly
                                            steered_hidden_states = (
                                                steering_method.apply_steering(
                                                    hidden_states,
                                        ...
                                        # Handle vector-based steering methods (CAA, HPR, DAC, BiPO)
                                            ...
                                            hasattr(steering_method, "steering_vector")
                                            and steering_method.steering_vector
                                            is not None
                                        ):
                                            steering_vector = (
                                    ...
                                    if verbose:
                                        print(
                                            f"      ⚠️ Steering hook failed during generation: {e}"
                                        )
                                    return output
                                return steering_hook
                        ...
                        for layer_idx in layers:
                            ...
                            if hasattr(model.model, "transformer"):
                                ...
                                raise ValueError(
                                    f"Unknown model architecture for {type(model.model)}"
                                )
                ...
                # Analyze differences
                if (
                    unsteered_response.lower().strip()
                    == steered_response.lower().strip()
                ):
                    print(
                    ...
                    unsteered_has_correct = (
                        correct_answer.lower() in unsteered_response.lower()
                    )
                    steered_has_correct = (
                        correct_answer.lower() in steered_response.lower()
                    )

                    if unsteered_has_correct and not steered_has_correct:
                        print(
                            f"   📉 Steering made response WORSE: removed correct answer '{correct_answer}'"
                        )
                    elif not unsteered_has_correct and steered_has_correct:
                        print(
                            f"   📈 Steering made response BETTER: added correct answer '{correct_answer}'"
                        )
                    elif unsteered_has_correct and steered_has_correct:
                        print(
                            f"   ➡️ Both responses mention correct answer '{correct_answer}'"
                        )
                    else:
                        print(
                            f"   ❓ Neither response mentions correct answer '{correct_answer}'"
                        )
                        ...
                        for incorrect in incorrect_answers:
                            if incorrect.lower() in unsteered_response.lower():
                                print(
                                    f"   ❌ Unsteered response mentions incorrect answer: '{incorrect}'"
                                )
                            if incorrect.lower() in steered_response.lower():
                                print(
                                    f"   ❌ Steered response mentions incorrect answer: '{incorrect}'"
                                )
        ...
        evaluation_results = {
            ...
            "task_name": task_name,
            "steering_applied": len(steering_methods) > 0
            if steering_methods
            else False,
