...
class ControlVectorSteering(SteeringMethod):
    ...
    def __init__(
        ...
        self.control_vector = (
            control_vector.to(self.device) if control_vector is not None else None
        )
        self.layer = layer
    ...
    def train(
        ...
        if self.control_vector is None:
            return {"success": False, "error": "Failed to compute control vector"}
    ...
    def apply_steering(
        ...
        if activations.dim() == 2:
            ...
            steered = activations - strength * self.control_vector.unsqueeze(
                0
            ).unsqueeze(0)
        else:
