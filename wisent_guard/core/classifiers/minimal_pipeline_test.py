...
from wisent_guard.core.classifiers.pipeline_steps.download_full_benchmarks import (
...
class MinimalPipelineTester:
    ...
    def extract_single_layer_activations(
        self, response_generator, texts: List[str]
    ) -> torch.Tensor:
        """Extract activations from only the specified layer for a batch of texts."""
        from wisent_guard.core.layer import Layer

        print(
            f"   🧠 Extracting activations for layer {self.layer} from {len(texts)} texts..."
        )
        ...
        for i, text in enumerate(texts):
            if not text or not text.strip():
                raise ValueError(
                    f"Empty text provided for activation extraction at index {i}"
                )

            layer_activations = response_generator.model.extract_activations(
                text, layer
            )

            if layer_activations is None or not isinstance(
                layer_activations, torch.Tensor
            ):
                raise ValueError(
                    f"Failed to extract activations for layer {self.layer} at text index {i}"
                )
    ...
    def test_benchmark(self, benchmark: str) -> Dict:
        ...
        response_generator = TestingResponseGenerator(
            self.model_name, str(self.output_dir)
        )
            ...
            benchmark_result["training_pairs"].append(
                {
                    ...
                    "good_activation_text": resp["evaluation_result"][
                        "good_activation_text"
                    ],
                    "bad_activation_text": resp["evaluation_result"][
                        "bad_activation_text"
                    ],
                }
            )
        ...
        train_activations = self.extract_single_layer_activations(
            response_generator, train_texts
        )
        test_activations = self.extract_single_layer_activations(
            response_generator, test_texts
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Minimal pipeline test: 4 train + 1 test per benchmark"
    )
