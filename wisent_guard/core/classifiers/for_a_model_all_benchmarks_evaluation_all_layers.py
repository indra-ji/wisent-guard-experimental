...
class ComprehensiveModelEvaluator:
    ...
    def __init__(self, model_name: str, base_dir: Optional[str] = None):
        ...
        self.classifier_generator = CLIBatchClassifierGenerator(
            model_name, str(self.base_dir)
        )
    ...
    def ensure_benchmarks_downloaded(
        ...
        try:
            ...
            downloader = FullBenchmarkDownloader(
                download_dir=str(self.script_dir / "full_benchmarks")
            )

            # Download all benchmarks
            print(
                "🔄 Downloading all available benchmarks (this may take several minutes)..."
            )
            ...
            if results["successful"]:
                print(
                    f"✅ Successfully downloaded {len(results['successful'])} benchmarks!"
                )
                print(f"   📁 Location: {benchmarks_dir}")
                return {
    ...
    def check_classifier_coverage(
        self, benchmarks: List[str], layers: List[int]
    ) -> Dict[str, Any]:
        """
        ...
        print(
            f"   ✅ Existing: {coverage['total_existing']}/{coverage['total_expected']}"
        )
        print(
            f"   ❌ Missing: {coverage['total_missing']}/{coverage['total_expected']}"
        )
        print(
            f"   📊 Coverage: {coverage['total_existing'] / coverage['total_expected'] * 100:.1f}%"
        )

        return coverage

    def train_missing_classifiers(
        self,
        missing_classifiers: List[Tuple[str, int]],
        ...
        training_results = {"trained": 0, "failed": 0, "details": []}

        for benchmark, layers in benchmarks_to_train.items():
            print(f"\n🎯 Training classifiers for {benchmark}...")

            benchmark_results = (
                self.classifier_generator.generate_classifiers_for_benchmark(
                    benchmark, layers, split_ratio, limit
                )
            )

            training_results["trained"] += len(benchmark_results["successful"])
            training_results["failed"] += len(benchmark_results["failed"]) + len(
    ...
    def ensure_test_responses_exist(
        self, benchmarks: List[str], limit: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        if self.response_generator is None:
            ...
            self.response_generator = TestingResponseGenerator(
                self.model_name, str(self.test_responses_dir)
            )

        # Generate missing test responses
        results = self.response_generator.generate_all_testing_responses(
            benchmarks=missing_responses, limit_per_benchmark=limit
        )
    ...
    def load_classifier(self, benchmark: str, layer: int) -> Optional[Any]:
        ...
        classifier_file = (
            self.classifiers_dir / benchmark / f"layer_{layer}_pkl_layer_{layer}.pkl"
        )
    ...
    def load_test_data(self, benchmark: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {"responses": responses_data, "activations": activations}
        except Exception as e:
            print(f"      ⚠️ Failed to load test data for {benchmark}: {e}")
            return None

    def evaluate_classifier(
        self, classifier: Any, activations: torch.Tensor, labels: List[str]
    ) -> Dict[str, Any]:
        """
    ...
    def evaluate_all_classifiers(
        self, benchmarks: List[str], layers: List[int], test_limit: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        for benchmark in benchmarks:
            ...
            for layer in layers:
                ...
                metrics = self.evaluate_classifier(
                    classifier, layer_activations, labels
                )

                if "error" not in metrics:
                    print(
                        f"      ✅ Layer {layer}: Acc={metrics['accuracy']:.3f}, F1={metrics['f1_score']:.3f}"
                    )
            ...
            layer_metrics = [
                m for m in benchmark_results["layers"].values() if "error" not in m
            ]
            if layer_metrics:
        ...
        for layer in layers:
            ...
            for benchmark in benchmarks:
                if benchmark in evaluation_results["benchmark_results"]:
                    layer_result = evaluation_results["benchmark_results"][benchmark][
                        "layers"
                    ].get(layer)
    ...
    def print_evaluation_summary(self, results: Dict[str, Any]):
        ...
        for benchmark, result in results["benchmark_results"].items():
            if "summary" in result and result["summary"]:
                ...
                print(
                    f"   • {benchmark}: Acc={summary['avg_accuracy']:.3f}, F1={summary['avg_f1_score']:.3f} ({summary['num_layers_evaluated']} layers)"
                )

        print("\n🧠 Layer Performance:")
        for layer, result in results["layer_results"].items():
            print(
                f"   • Layer {layer}: Acc={result['avg_accuracy']:.3f}, F1={result['avg_f1_score']:.3f} ({result['num_benchmarks']} benchmarks)"
            )
                ...
                for layer, layer_result in result["layers"].items():
                    if "error" not in layer_result:
                        best_performances.append(
                            (benchmark, layer, layer_result["f1_score"])
                        )
    ...
    def run_comprehensive_evaluation(
        ...
        if download_results["already_existed"]:
            print(
                f"   📁 Benchmark data: Available ({download_results['total_files']} files)"
            )
        else:
            print(
                f"   📁 Benchmark data: Downloaded {download_results['downloaded']} benchmarks"
            )
        ...
        evaluation_results = self.evaluate_all_classifiers(
            benchmarks, layers, test_limit
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Comprehensive model evaluation across all benchmarks and layers"
    )
