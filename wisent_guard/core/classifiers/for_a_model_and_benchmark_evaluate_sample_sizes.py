...
class SampleSizeEvaluator:
    ...
    def ensure_benchmark_downloaded(self) -> bool:
        ...
        try:
            ...
            results = downloader.download_all_benchmarks(
                benchmarks=[self.benchmark_name], force=False
            )
    ...
    def ensure_test_responses_exist(self) -> bool:
        ...
        test_responses_file = (
            self.test_responses_dir
            / f"{self.model_safe}_{self.benchmark_name}_responses.pkl"
        )
        ...
        try:
            ...
            results = response_generator.generate_responses_for_benchmark(
                self.benchmark_name, limit=self.test_limit
            )

            # Check if we have results and successful responses
            if results and results.get("num_successful_responses", 0) > 0:
                print(
                    f"✅ Successfully generated {results['num_successful_responses']} test responses"
                )
    ...
    def train_classifier_with_sample_size(self, sample_size: int) -> Dict[str, Any]:
        ...
        classifier_path = (
            classifier_subdir / f"layer_{self.layer}_samples_{sample_size}.pkl"
        )
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                actual_path = (
                    classifier_subdir
                    / f"layer_{self.layer}_samples_{sample_size}_pkl_layer_{self.layer}.pkl"
                )
            ...
            return {"status": "error", "error": str(e), "sample_size": sample_size}
    ...
    def load_test_data(self) -> Tuple[List[str], Dict[int, torch.Tensor], List[str]]:
        ...
        if self.test_limit and self.test_limit < len(test_responses):
            print(
                f"   📏 Limiting test data to {self.test_limit} samples (from {len(test_responses)})"
            )
    ...
    def run_sample_size_evaluation(self) -> Dict[str, Any]:
        ...
        print(
            f"   Labels: {len(test_labels)} ({sum(1 for l in test_labels if l == 'GOOD')} GOOD, {sum(1 for l in test_labels if l == 'BAD')} BAD)"
        )
    ...
    def create_visualization(self, results: Dict[str, Any]):
        ...
        plt.subplot(2, 2, 1)
        plt.semilogx(
            sample_sizes, accuracies, "o-", label="Accuracy", linewidth=2, markersize=8
        )
        plt.semilogx(
            sample_sizes, f1_scores, "s-", label="F1 Score", linewidth=2, markersize=8
        )
        ...
        plt.subplot(2, 2, 2)
        plt.semilogx(
            sample_sizes, accuracies, "o-", color="blue", linewidth=2, markersize=8
        )
        plt.xlabel("Training Sample Size")
        plt.ylabel("Accuracy")
        ...
        plt.subplot(2, 2, 3)
        plt.semilogx(
            sample_sizes, f1_scores, "s-", color="red", linewidth=2, markersize=8
        )
        plt.xlabel("Training Sample Size")
        plt.ylabel("F1 Score")
    ...
    def print_summary(self, results: Dict[str, Any]):
        ...
        print(
            f"   Total evaluated: {summary['total_evaluated']}/{len(self.sample_sizes)}"
        )
        ...
        if results["evaluation_results"]:
            ...
            print(
                f"   {'Size':>6} {'Accuracy':>9} {'F1 Score':>9} {'Precision':>9} {'Recall':>9}"
            )
            print(f"   {'-' * 50}")
            ...
            for sample_size in self.sample_sizes:
                if sample_size in results["evaluation_results"]:
                    ...
                    if "error" not in eval_result:
                        ...
                        print(
                            f"   {sample_size:>6} {accuracy:>9.3f} {f1:>9.3f} {precision:>9.3f} {recall:>9.3f}"
                        )
                    else:
                        print(
                            f"   {sample_size:>6} {'ERROR':>9} {'ERROR':>9} {'ERROR':>9} {'ERROR':>9}"
                        )
        ...
        if best_sample_size:
            ...
            for i in range(1, len(accuracies)):
                ...
                ):  # Less than 0.01 improvement per 100 samples
                    diminishing_returns_point = prev_size
                    break

            if diminishing_returns_point:
                print(
                    f"   Diminishing returns after: {diminishing_returns_point} samples"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate classifier performance vs sample size"
    )
