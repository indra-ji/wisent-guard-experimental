...
class FullBenchmarkDownloader:
    ...
    def download_complete_benchmark(
        ...
        try:
            ...
            for split in splits_to_try:
                try:
                    if hasattr(task, f"{split}_docs"):
                        ...
                        if docs:
                            print(
                                f"      📊 Found {len(docs)} samples in {split} split"
                            )
            ...
            contrastive_data = self.convert_to_contrastive_pairs(
                benchmark_name, complete_data
            )
            ...
            print(
                f"         📊 Contrastive pairs: {len(contrastive_data['contrastive_pairs'])}"
            )
    ...
    def download_all_benchmarks(
        self, benchmarks: Optional[List[str]] = None, force: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        if benchmarks is None:
            ...
            if excluded_count > 0:
                ...
                print(
                    f"   📋 Available benchmarks: {len(available_benchmarks)}/{len(CORE_BENCHMARKS)}"
                )
        else:
            benchmarks_to_download = {
                name: CORE_BENCHMARKS[name]
            ...
            unavailable_requested = [
                name for name in benchmarks if name in self.UNAVAILABLE_BENCHMARKS
            ]
            if unavailable_requested:
                print(
                    f"⚠️  Requested benchmarks are known to be unavailable: {unavailable_requested}"
                )
        ...
        for i, (benchmark_name, benchmark_config) in enumerate(
            benchmarks_to_download.items(), 1
        ):
            print(
            ...
            try:
                result_path = self.download_complete_benchmark(
                    benchmark_name, benchmark_config, force
                )
    ...
    def convert_to_contrastive_pairs(
        self, benchmark_name: str, complete_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _convert_sample_to_pairs(
        self, sample: Dict[str, Any], benchmark_name: str
    ) -> List[Dict[str, Any]]:
        ...
        elif "label" in sample and str(sample["label"]).lower() in [
            "true",
            "false",
            "0",
            "1",
    ...
    def _convert_mmmlu_format(self, sample: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        correct_answer = options.get(
            answer, option_a
        )  # Default to A if answer not found
    ...
    def _convert_multiple_choice_numeric(
        ...
        incorrect_answers = [
            choices[i] for i in range(len(choices)) if i != correct_idx
        ]
    ...
    def _convert_multiple_choice_letter(
        self, sample: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        incorrect_answers = [
            choices_text[i] for i in range(len(choices_text)) if i != correct_idx
        ]
    ...
    def _convert_textual_entailment(
        self, sample: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def _convert_text_generation(self, sample: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if any(
            math_keyword in question.lower()
            for math_keyword in ["dollars", "cost", "price", "how much", "how many"]
        ):
    ...
    def _convert_reading_comprehension(
        self, sample: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        if "questions" in sample and "answers" in sample:
            ...
            # Handle other formats where questions/answers might be lists directly
                ...
                for i, (q, a) in enumerate(zip(questions_data, answers_data)):
                    question_text = (
                        q.get("input_text", q.get("text", ""))
                        if isinstance(q, dict)
                        else str(q)
                    )
                    answer_text = (
                        a.get("input_text", a.get("text", ""))
    ...
    def _convert_generic_multiple_choice(
        self, sample: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Download complete benchmarks from lm-eval-harness"
    )

    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument(
        "--benchmarks", nargs="+", help="Specific benchmarks to download"
    )
    group.add_argument(
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force redownload even if exists"
    )
    parser.add_argument(
        "--download-dir", default="full_benchmarks", help="Directory to save downloads"
    )
    ...
    try:
        if args.all:
            ...
            print(
                f"📋 Downloading {len(args.benchmarks)} specified benchmarks: {args.benchmarks}"
            )

        results = downloader.download_all_benchmarks(
            benchmarks=benchmarks_to_download, force=args.force
        )
