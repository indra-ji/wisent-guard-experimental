...
class TestingResponseGenerator:
    ...
    def __init__(self, model_name: str, output_dir: Optional[str] = None):
        ...
        methods_file = (
            ...
            / "parameters"
            / "benchmarks"
            / "benchmark_evaluation_methods.json"
        )
    ...
    def get_available_benchmarks(self) -> List[str]:
        ...
        available_benchmarks = [
            b for b in all_benchmarks if b in self.evaluation_methods
        ]
    ...
    def extract_activations_from_all_layers(self, text: str) -> Dict[int, torch.Tensor]:
        ...
        for layer_idx in range(self.num_layers):
            try:
                ...
                if layer_activations is not None and isinstance(
                ...
                print(
                    f"      ⚠️ Failed to extract activations from layer {layer_idx}: {e}"
                )
                activations[layer_idx] = None
        ...
        print(
            f"      ✅ Successfully extracted activations from {len(successful_layers)}/{self.num_layers} layers"
        )

        return activations

    def generate_log_likelihood_response(
        self, sample: Dict, benchmark_name: str
    ) -> Dict[str, Any]:
        """
        ...
        question = sample.get(
            ...
            sample.get(
                "premise",
                sample.get("text", sample.get("query", sample.get("ctx", ""))),
            ),
        )
        ...
        for choice in choices:
            ...
            try:
                ...
                context_tokens = (
                    self.model.tokenizer.encode(context, add_special_tokens=False)
                    if context
                    else []
                )
                full_tokens = self.model.tokenizer.encode(
                ...
                target_tokens = torch.tensor(
                    continuation_tokens, device=self.model.device
                )
    ...
    def generate_text_generation_response(
        self, sample: Dict, benchmark_name: str
    ) -> Dict[str, Any]:
        """
        ...
        question = sample.get(
            "question",
            sample.get(
                "input", sample.get("text", sample.get("query", sample.get("ctx", "")))
            ),
        )
    ...
    def generate_perplexity_response(
        self, sample: Dict, benchmark_name: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            token_log_probs = (
                log_probs[0, :-1, :]
                .gather(dim=-1, index=input_ids[0, 1:].unsqueeze(-1))
                .squeeze(-1)
            )
    ...
    def compute_evaluation_result(
        ...
        if evaluation_method == "log-likelihoods":
            ...
            is_correct = any(
                generated == correct.strip().lower() for correct in correct_answers
            )
    ...
    def generate_responses_for_benchmark(
        self, benchmark_name: str, limit: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        for i, sample in enumerate(test_data):
            ...
            try:
                ...
                if eval_method == "log-likelihoods":
                    response = self.generate_log_likelihood_response(
                        sample, benchmark_name
                    )
                elif eval_method == "text-generation":
                    response = self.generate_text_generation_response(
                        sample, benchmark_name
                    )
                ...
                if activation_text:
                    print(
                        f"      🧠 Extracting activations for sample {i + 1} from text: '{activation_text[:50]}...'"
                    )
                    sample_activations = self.extract_activations_from_all_layers(
                        activation_text
                    )
                    ...
                    response["activations"] = {
                        layer: None for layer in range(self.num_layers)
                    }
        ...
        for layer_idx in range(self.num_layers):
            valid_activations = [
                act for act in all_activations[layer_idx] if act is not None
            ]
        ...
        print(
            f"   ✅ Generated {len(responses)}/{len(test_data)} responses successfully"
        )
    ...
    def save_benchmark_results(self, benchmark_name: str, results: Dict[str, Any]):
        ...
        responses_only = {
            ...
            "responses": [
                {k: v for k, v in r.items() if k != "activations"}
    ...
    def generate_all_testing_responses(
        self,
        ...
        for i, benchmark in enumerate(benchmarks):
            ...
            try:
                ...
                results = self.generate_responses_for_benchmark(
                    benchmark, limit_per_benchmark
                )
                ...
                if results:
                    ...
                    overall_results["total_responses"] += results[
                    ...
                    print(
                        f"   ❌ Benchmark {benchmark} failed to generate any responses"
                    )
        ...
        print(
            f"📊 Benchmarks processed: {len(overall_results['benchmarks_processed'])}/{len(benchmarks)}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate testing responses and save layer activations"
    )
    parser.add_argument(
        "--model",
    ...
    try:
        results = generator.generate_all_testing_responses(
            benchmarks=args.benchmarks, limit_per_benchmark=args.limit
        )
