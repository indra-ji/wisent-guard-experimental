...
class ModelPersistence:
    ...
    @staticmethod
    def save_classifier(
        classifier, layer: int, save_path: str, metadata: Dict[str, Any] = None
    ) -> str:
        """
    ...
    @staticmethod
    def save_multi_layer_classifiers(
        classifiers: Dict[int, Any], save_path: str, metadata: Dict[str, Any] = None
    ) -> List[str]:
        """
        ...
        for layer, classifier in classifiers.items():
            path = ModelPersistence.save_classifier(
                classifier, layer, save_path, metadata
            )
            saved_paths.append(path)
        return saved_paths

    @staticmethod
    def load_multi_layer_classifiers(
        load_path: str, layers: List[int]
    ) -> Dict[int, tuple]:
        """
        ...
        for layer in layers:
            try:
                ...
                print(
                    f"⚠️  Warning: Classifier for layer {layer} not found at {load_path}"
                )
                continue
        return classifiers
    ...
    def list_available_models(
        model_dir: str, model_type: str = "classifier"
    ) -> Dict[str, List[int]]:
        """
