...
class BigCodeTaskLoader:
    ...
    def load_task(self, task_name: str, limit: Optional[int] = None) -> "BigCodeTask":
        ...
        if not self._bigcode_available:
            raise ImportError(
                "bigcode-evaluation-harness not installed. Run: pip install bigcode-evaluation-harness"
            )
...
class BigCodeTask:
    ...
    def __init__(
        self, task_name: str, bigcode_task_name: str, limit: Optional[int] = None
    ):
        """
...
class BigCodeEvaluator:
    ...
    def evaluate(
        ...
        if self._is_code_execution_task(task.task_name):
            results["execution_results"] = self._evaluate_code_execution(
                task, generations
            )
            ...
            for k in k_values:
                if k <= len(generations):
                    pass_rate = self._calculate_pass_at_k(
                        results["execution_results"], k
                    )
    ...
    def _evaluate_code_execution(
        self, task: BigCodeTask, generations: List[str]
    ) -> List[Dict]:
        ...
        for i, sample in enumerate(task.get_samples()):
            ...
            for j, generation in enumerate(
                generations[i] if i < len(generations) else []
            ):
    ...
    def _execute_in_subprocess(
        self, sample: Dict, generation: str, task_name: str
    ) -> Dict:
        ...
        try:
            ...
            try:
                ...
                if proc.returncode == 0:
                    ...
                    logger.info(
                        f"✅ Code execution PASSED. Output: {proc.stdout[:200]}"
                    )
                else:
                    result["error"] = proc.stderr or proc.stdout
                    logger.info(
                        f"❌ Code execution FAILED. Error: {result['error'][:500]}"
                    )
...
def evaluate_bigcode_task(
    task: BigCodeTask, generations: List[str], docker_executor=None
) -> Dict[str, Any]:
    """Evaluate generations on a BigCode task."""
