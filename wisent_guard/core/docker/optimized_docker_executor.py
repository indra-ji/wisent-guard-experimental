    ...
    def for_complexity(cls, complexity: TaskComplexity) -> "ResourceProfile":
        ...
        profiles = {
            TaskComplexity.SIMPLE: cls(
                memory_limit="64m", cpu_limit=0.25, timeout=5, batch_size=10
            ),
            TaskComplexity.MEDIUM: cls(
                memory_limit="128m", cpu_limit=0.5, timeout=10, batch_size=5
            ),
            TaskComplexity.COMPLEX: cls(
                memory_limit="256m", cpu_limit=1.0, timeout=30, batch_size=2
            ),
        }
...
class TaskComplexityAnalyzer:
    ...
    def analyze_code_complexity(code: str) -> TaskComplexity:
        ...
        complexity_indicators = {
            "nested_loops": len(
                [line for line in lines if "    for " in line or "    while " in line]
            ),
            "recursion": "def " in code
            and any(
                func_name in code
                for func_name in ["fibonacci", "factorial", "recursive"]
            ),
            "data_structures": any(
                ds in code_lower for ds in ["dict", "list", "set", "tuple", "class"]
            ),
            "algorithms": any(
                alg in code_lower
                for alg in ["sort", "search", "binary", "graph", "tree"]
            ),
            "imports": len(
...
class OptimizedDockerExecutor:
    ...
    def execute_batch(self, tasks: List[Tuple[str, str]]) -> List[Dict[str, Any]]:
        ...
        logger.info(
            f"Batched execution: {len(tasks)} tasks in {len(complexity_groups)} complexity groups"
        )

        return results

    def _execute_with_profile(
        self, code: str, input_data: str, profile: ResourceProfile
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            docker_cmd = [
                ...
                "1001:1001",
                "-v",
                f"{code_file}:/home/coderunner/workspace/code.py:ro",
                self.image_name,
                "python",
                "/home/coderunner/workspace/code.py",
            ]
    ...
    def _execute_batch_with_profile(
        self, batch: List[Tuple[int, str, str]], profile: ResourceProfile
    ) -> List[Dict[str, Any]]:
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        if stats["total_tasks"] > 0:
            ...
            stats["container_efficiency"] = (
                stats["total_tasks"] / stats["containers_created"]
            )
        else:
    ...
    def cleanup(self):
        ...
        try:
            ...
            subprocess.run(
                ["docker", "builder", "prune", "-f"], capture_output=True, timeout=30
            )
