...
class SyntheticContrastivePairGenerator:
    ...
    def generate_scenarios(
        ...
        selected_scenarios = self._select_diverse_scenarios(
    ...
    def _parse_scenarios_from_response(self, response: str) -> List[str]:
        ...
        for line in lines:
            ...
            if (
                ...
                and (
                    "?" in cleaned
                    or "what" in cleaned_lower
                    or "how" in cleaned_lower
                    or "why" in cleaned_lower
                    or "when" in cleaned_lower
                    or "can you" in cleaned_lower
                    or "should" in cleaned_lower
                )
            ):
...
Brief answer showing non-{trait_description}:"""
        ...
        if scenario.strip().endswith("?") or any(
            word in scenario.lower()
            for word in ["what", "how", "why", "when", "explain", "tell me"]
        ):
    ...
    def generate_contrastive_pair_set(
        self, trait_description: str, num_pairs: int = 30, name: Optional[str] = None
    ) -> ContrastivePairSet:
        """
        ...
        """
        print(
            f"🎯 Generating {num_pairs} contrastive pairs for trait: '{trait_description}'"
        )
        ...
        pair_set = ContrastivePairSet(
            name=name or f"synthetic_{trait_description[:30]}", task_type="synthetic"
        )

        for i, scenario in enumerate(scenarios):
            try:
                print(
                    f"   Generating pair {i + 1}/{len(scenarios)}: {scenario[:50]}..."
                )
    ...
    def save_to_json(self, pair_set: ContrastivePairSet, filepath: str) -> None:
        ...
        data = {"name": pair_set.name, "task_type": pair_set.task_type, "pairs": []}
