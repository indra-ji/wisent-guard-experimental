...
class ContrastivePairQualityChecker:
    ...
    def check_scenario_quality(self, scenario: str, trait_description: str) -> Dict[str, Any]:
        """
        ...
        if self.strict_mode:
            ...
            if not trait_related and len(trait_keywords) > 1:
                ...
                trait_related = any(keyword in scenario_lower for keyword in semantic_keywords)
    ...
    def check_response_quality(self, response_text: str, trait_description: str, is_positive: bool) -> Dict[str, Any]:
        """
    ...
    def check_pair_quality(self, pair: ContrastivePair, trait_description: str) -> Dict[str, Any]:
        """
        ...
        overall_score = (scenario_check["score"] * 0.4 + 
                        pos_check["score"] * 0.3 + 
                        neg_check["score"] * 0.3)
        ...
        if scenario_check["issues"]:
            all_issues.extend([f"Scenario: {issue}" for issue in scenario_check["issues"]])
        if pos_check["issues"]:
    ...
    def filter_pair_set(self, pair_set: ContrastivePairSet, trait_description: str) -> ContrastivePairSet:
        """
        ...
        filtered_set = ContrastivePairSet(
            name=f"{pair_set.name}_filtered",
