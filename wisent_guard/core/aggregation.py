...
def create_control_vector_from_contrastive_pairs(
    ...
    if len(pos_activations) != len(neg_activations):
        raise ValueError(
            f"Mismatch in activation pairs: {len(pos_activations)} vs {len(neg_activations)}"
        )
    ...
    for pos_act, neg_act in zip(pos_activations, neg_activations):
        ...
        if pos_tensor.shape != neg_tensor.shape:
            raise ValueError(
                f"Shape mismatch in pair: {pos_tensor.shape} vs {neg_tensor.shape}"
            )
...
def create_control_vector_from_representations(
    ...
    if positive_representation.shape != negative_representation.shape:
        raise ValueError(
            f"Shape mismatch: {positive_representation.shape} vs {negative_representation.shape}"
        )
    ...
    if method == ControlVectorAggregationMethod.CAA:
        ...
        raise ValueError(
            f"Method {method} not supported for pre-aggregated representations"
        )
