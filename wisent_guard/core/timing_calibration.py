...
class TimingCalibrator:
    ...
    def run_calibration(self, model_name: str) -> Dict[str, float]:
        ...
        cmd = [
            ...
            "10",
            "--layer-range",
            f"{calibration_layer},{calibration_layer}",
            "--no-save",
    ...
    def estimate_optimization_time(
        ...
        base_training = self.timings[
            "training_time"
        ]  # Time for 1 task, 1 layer, 10 samples
        base_steering = self.timings[
            "steering_time"
        ]  # Time for 1 task, 1 layer, 10 samples
        ...
        classification_time = (
            base_training * num_tasks * num_layers * (samples_per_task / 10)
        )
        ...
        if include_sample_size_opt and sample_sizes:
            ...
            sample_size_time = (
                ...
                * num_tasks
                * len(sample_sizes)
                * (min(avg_sample_size, sample_size_limit) / 10)
            )
        ...
        if include_control_vectors and base_steering is not None:
            ...
            control_vectors_time = (
                base_steering * num_tasks * cv_layers * (samples_per_task / 10)
            )
