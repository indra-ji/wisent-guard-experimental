...
class GroundTruthEvaluator:
    ...
    def __init__(
        self,
        method: GroundTruthMethod = GroundTruthMethod.NONE,
    ...
    def evaluate_response(
        self, response: str, expected_answer, user_label: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _evaluate_user_specified(
        self, response: str, user_label: str
    ) -> Dict[str, Any]:
        ...
        if user_label_lower in ["truthful", "true", "correct", "1"]:
            ...
            logger.warning(
                f"Unrecognized user label: {user_label}. Defaulting to UNKNOWN."
            )
    ...
    def _evaluate_exact_match(
        self, response: str, expected_answer: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if isinstance(is_actually_correct, dict):
                ...
                isinstance(is_actually_correct, (list, tuple))
                and len(is_actually_correct) > 0
            ):
            ...
            logger.warning(
                f"lm-eval exact match failed: {e}. Using simple string comparison."
            )
    ...
    def _evaluate_interactive(
        ...
        while True:
            user_input = (
                input("Is this response truthful? (y/n/q to quit): ").strip().lower()
            )
            ...
            if user_input in ["y", "yes", "true", "t", "1"]:
                ...
                print(
                    "Please enter 'y' for truthful, 'n' for hallucination, or 'q' to quit."
                )
                continue
    ...
    def _evaluate_lm_eval_harness(
        self, response: str, expected_answer
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            is_correct = self._evaluate_using_extractor(
                response, expected_answer, extractor, task_name
            )
    ...
    def _evaluate_using_extractor(
        self, response: str, expected_answer, extractor, task_name: str
    ) -> bool:
        ...
        try:
            ...
            if extractor_name == "WinograndeExtractor":
                ...
                logger.warning(
                    f"Unknown extractor type: {extractor_name}. Using flexible matching."
                )
                return self._evaluate_flexible_logic(response, expected_answer)
    ...
    def _evaluate_numerical_logic(self, response: str, expected_answer) -> bool:
        ...
        try:
            ...
            if isinstance(expected_answer, (int, float)):
                ...
                return (
                    abs(response_num - expected_num) < 0.01
    ...
    def evaluate_batch(
        ...
        for i, (response, expected, user_label) in enumerate(
            zip(responses, expected_answers, user_labels)
        ):
            try:
    ...
    @classmethod
    def from_string(
        cls, method_str: str, task_name: Optional[str] = None
