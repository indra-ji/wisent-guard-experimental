...
class Activations:
    def __init__(self, tensor, layer, aggregation_method=None):
        ...
        self.aggregation_method = (
            aggregation_method or ActivationAggregationMethod.LAST_TOKEN
        )
    ...
    def calculate_similarity(
        self, other_tensor: torch.Tensor, method: str = "cosine"
    ) -> float:
        """
        ...
        try:
            if method == "cosine":
                ...
                cos_sim = F.cosine_similarity(
                    activation.unsqueeze(0), other_tensor.unsqueeze(0), dim=1
                )
    ...
    def compare_with_vectors(
        self, vector_dict: Dict[str, torch.Tensor], threshold: float = 0.7
    ) -> Dict[str, Any]:
        """
    ...
    def from_model_output(
        cls, model_outputs, layer: Layer, aggregation_method=None
    ) -> "Activations":
        """
        ...
        if (
            ...
            if layer.index + 1 < len(hidden_states):
                ...
                raise ValueError(
                    f"Layer {layer.index} not found in model with {len(hidden_states)} layers"
                )
        else:
    ...
    def from_tensor_dict(
        ...
        if "activations" in tensor_dict:
            ...
            raise ValueError(
                f"No activation tensor found for layer {layer.index} in dictionary"
            )
...
class ActivationMonitor:
    ...
    def store_activations(
        self, activations: Dict[int, Activations], text: Optional[str] = None
    ) -> None:
    ...
    def analyze_activations(
        self, activations: Optional[Dict[int, Activations]] = None
    ...
    def detect_anomalies(
        ...
        for layer_idx in activations:
            ...
            similarities = [
                current.cosine_similarity(hist) for hist in historical_activations
            ]
...
class TestActivationCache:
    ...
    def add_activation(
        self, question: str, response: str, activations: "Activations", layer: int
    ):
    ...
    def load_from_file(cls, filepath: str) -> "TestActivationCache":
        ...
        print(
            f"📁 Loading {metadata.get('num_samples', 0)} test activations from {filepath}"
        )
