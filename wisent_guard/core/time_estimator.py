...
class OptimizationTimeEstimator:
    ...
    def __init__(
        ...
        if hasattr(model, "model") and hasattr(model.model, "config"):
            if hasattr(model.model.config, "num_hidden_layers"):
                ...
                raise RuntimeError(
                    f"Cannot determine number of layers for model {model_name}"
                )
        else:
        ...
        if skip_calibration:
            raise RuntimeError(
                "Calibration cannot be skipped. Accurate timing requires calibration."
            )
        ...
        if calibration_file and calibration_file.exists():
            ...
            if not self.calibrator.load_from_file(calibration_file):
                raise RuntimeError(
                    f"Failed to load calibration from {calibration_file}"
                )
            self.timing = self.calibrator.timings
            if (
                self.timing["training_time"] is None
                or self.timing["steering_time"] is None
            ):
                raise RuntimeError(
                    f"Calibration file {calibration_file} contains invalid data"
                )
        else:
            # Run calibration
            if verbose:
    ...
    def estimate_classification_time(
        self, num_tasks: int, sample_limit: int = 200, layers: Optional[list] = None
    ...
    def estimate_full_optimization_time(
        ...
        if sample_sizes is None:
            raise RuntimeError(
                "sample_sizes must be provided for full optimization time estimation"
            )
    ...
    def print_time_breakdown(self, total_time: float, breakdown: Dict[str, float]):
        ...
        if len(breakdown) > 1:
            ...
            for phase, time_sec in breakdown.items():
                if time_sec > 0:
                    print(
                        f"   - {phase.replace('_', ' ').title()}: {self.format_time(time_sec)}"
                    )
