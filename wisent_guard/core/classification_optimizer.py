...
class DetailedOptimizationLogs:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "task_logs": {
                task_name: [asdict(entry) for entry in entries]
                for task_name, entries in self.task_logs.items()
            },
...
class TaskOptimizationResult:
    ...
    classifier_metadata: Optional[Dict[str, Any]] = (
        None  # Metadata about saved classifier
    )
...
class ClassificationOptimizationSummary:
    ...
    layer_frequency_analysis: Dict[
        int, int
    ]  # layer -> count of tasks where it was optimal
...
class ClassificationOptimizer:
        ...
        try:
            ...
            if tasks:
                self.available_tasks = [
                    task for task in tasks if task in all_available_tasks
                ]
                ...
                if len(self.available_tasks) != len(tasks):
                    invalid_tasks = [
                        task for task in tasks if task not in all_available_tasks
                    ]
                    logger.warning(
                        f"Some specified tasks are not available: {invalid_tasks}"
                    )
                logger.info(
                    f"✅ Using {len(self.available_tasks)} specified tasks for optimization"
                )
            else:
                self.available_tasks = all_available_tasks
                logger.info(
                    f"✅ Found {len(self.available_tasks)} available tasks for optimization"
                )
        except ImportError:
            logger.error("❌ Could not import available tasks from CLI")
            self.available_tasks = []

    def _get_classifier_save_directory(
        self, save_classifiers_dir: Optional[str] = None
    ) -> str:
        """
    ...
    def run_comprehensive_optimization(
        ...
        if max_time_per_task_minutes is not None:
            logger.info(
                f"   ⏱️  Max time per task: {max_time_per_task_minutes:.1f} minutes"
            )
        else:
        ...
        for i, task_name in enumerate(self.available_tasks, 1):
            logger.info(f"\n{'=' * 60}")
            logger.info(
                f"📊 OPTIMIZING TASK {i}/{len(self.available_tasks)}: {task_name.upper()}"
            )
            logger.info(f"{'=' * 60}")
            ...
            try:
                ...
                if detailed_logger:
                    ...
                    detailed_logger.set_task_progress(
                        task_name, {"status": "completed", "result": asdict(result)}
                    )
                ...
                if detailed_logger:
                    ...
                    detailed_logger.set_task_progress(
                        task_name, {"status": "failed_skipped", "error": str(e)}
                    )
        ...
        if detailed_logger:
            ...
            detailed_logger.set_timing(
                "average_time_per_task", total_time / len(self.available_tasks)
            )
            detailed_logger.set_timing(
                "average_time_per_successful_task",
    ...
    def _optimize_single_task(
        ...
        try:
            ...
            optimize_layers_str = (
                layer_range if layer_range else "all"
            )  # Default: test all layers
            ...
            logger.debug(
                f"Single task optimization failed for {task_name}, re-raising to outer handler"
            )
            raise e
        ...
        if save_classifier and classifier_save_dir:
            try:
                ...
                classifier_save_path = os.path.join(
                    classifier_save_dir, classifier_filename
                )
                ...
                classifier_metadata = {
                    ...
                    "configurations_tested": optimization_result.get(
                ...
                logger.info(
                    f"💾 Task {task_name} optimization completed with layer {best_layer}"
                )

            except Exception as e:
                logger.warning(
                    f"⚠️ Failed to process classifier metadata for {task_name}: {e}"
                )
                if detailed_logger:
                    detailed_logger.log_task(
                        task_name,
                        f"Failed to process classifier metadata: {str(e)}",
                        "warning",
                        {"error": str(e)},
                    )
        ...
        result = TaskOptimizationResult(
            ...
            optimization_time_seconds=optimization_time,
            total_configurations_tested=optimization_result.get(
                "total_combinations_tested", 0
            ),
    ...
    def _analyze_optimization_results(
        ...
        """
        logger.info(
            f"📊 Analyzing optimization results across {len(task_results)} tasks"
        )
        ...
        for result in successful_results:
            ...
            aggregation_frequency[aggregation] = (
                aggregation_frequency.get(aggregation, 0) + 1
            )

        # Find most frequent (likely best overall) parameters
        most_common_layer = max(layer_frequency.items(), key=lambda x: x[1])[0]
        most_common_aggregation = max(
            aggregation_frequency.items(), key=lambda x: x[1]
        )[0]
        ...
        if total_weighted_f1 > 0:
            weighted_avg_threshold = (
                sum(r.best_threshold * r.best_f1 for r in successful_results)
                / total_weighted_f1
            )
        else:
    ...
    def _save_optimization_results(
        ...
        if results_file is None:
            ...
            safe_model_name = self.model_name.replace("/", "_").replace(":", "_")
            results_file = (
                f"classification_optimization_{safe_model_name}_{timestamp}.json"
            )
    ...
    def _save_optimal_parameters_to_config(
        self, summary: ClassificationOptimizationSummary
    ):
        """
        ...
        successful_results = [
            r for r in summary.task_results if r.error_message is None
        ]
        optimization_metrics = {
            "average_f1_across_tasks": sum(r.best_f1 for r in successful_results)
            / len(successful_results)
            if successful_results
            else 0.0,
            "average_accuracy_across_tasks": sum(
                r.best_accuracy for r in successful_results
            )
            / len(successful_results)
            if successful_results
            else 0.0,
        ...
        logger.info(
            f"✅ Model configuration saved with {len(task_specific_overrides)} task-specific overrides"
        )
    ...
    def _log_failed_benchmark(self, error_details: Dict[str, Any]):
        ...
        model_dir = os.path.join(
            os.path.expanduser("~/.wisent-guard/model_configs"), self.model_name
        )
    ...
    def _print_optimization_summary(self, summary: ClassificationOptimizationSummary):
        ...
        print(
            f"📊 Tasks: {summary.successful_optimizations}/{summary.total_tasks_tested} successful"
        )
        ...
        sorted_layers = sorted(
            summary.layer_frequency_analysis.items(), key=lambda x: x[1], reverse=True
        )
        ...
        sorted_agg = sorted(
            summary.aggregation_frequency_analysis.items(),
            key=lambda x: x[1],
            reverse=True,
        )
        ...
        if successful_results:
            ...
            top_tasks = sorted(
                successful_results, key=lambda x: x.best_f1, reverse=True
            )[:5]
        ...
        failed_results = [
            r for r in summary.task_results if r.error_message is not None
        ]
        if failed_results:
            print("\n❌ FAILED TASKS:")
        ...
        saved_classifiers = [
            r for r in summary.task_results if r.classifier_save_path is not None
        ]
...
def run_classification_optimization(
    ...
    **kwargs,
    ...
    optimizer = ClassificationOptimizer(
        model_name=model_name, device=device, verbose=verbose, tasks=tasks
    )
