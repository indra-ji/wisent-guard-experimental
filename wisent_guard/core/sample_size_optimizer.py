...
class SampleSizeOptimizer:
    ...
    def load_and_split_data(
    ...
    ) -> Tuple[ContrastivePairSet, ContrastivePairSet]:
        """
                    ...
                    if "good_response" in normalized and "bad_response" in normalized:
                        qa_pair = {
                            "question": normalized.get(
                                "context", normalized.get("question", "")
                            ),
        ...
        if not qa_pairs:
            ...
            # Load lm-eval task
            task_data = self.model.load_lm_eval_task(
                self.task_name, shots=0, limit=max_samples
            )

            # Split into train/test docs
            docs, _ = self.model.split_task_data(
            ...
            qa_pairs = ContrastivePairSet.extract_qa_pairs_from_task_docs(
                self.task_name, task_data, docs
            )

        if not qa_pairs:
            raise ValueError(
                f"No QA pairs could be extracted from task {self.task_name}"
            )
        ...
        all_pairs = [
            p
            for p in all_pairs
            if p.positive_activations is not None and p.negative_activations is not None
        ]
        ...
        train_set = ContrastivePairSet(
            name=f"{self.task_name}_train", pairs=train_pairs
        )
    ...
    def train_classifier_with_sample_size(
        ...
        if len(train_pairs) < 2:
            logger.warning(
                f"Not enough training samples ({len(train_pairs)}). Skipping."
            )
            return None, 0.0
    ...
    def evaluate_classifier(
        self, classifier: Classifier, test_set: ContrastivePairSet
    ) -> Dict[str, float]:
        """
    ...
    def find_optimal_sample_size(self) -> int:
        ...
        for i, gain in enumerate(gains):
            if (
                gain < threshold and accuracies[i + 1] > 0.7
    ...
    def run_optimization(self) -> Dict[str, Any]:
        ...
        if not valid_sample_sizes:
            raise ValueError(
                f"No valid sample sizes. Training set has only {max_train_size} samples."
            )
        ...
        for sample_size in valid_sample_sizes:
            ...
            # Train classifier
            classifier, training_time = self.train_classifier_with_sample_size(
            ...
            if classifier is None:
                logger.warning(
                    f"Skipping sample size {sample_size} - not enough samples for training"
                )
                continue
    ...
    def plot_results(self, save_path: Optional[str] = None, show: bool = True) -> None:
        ...
        if self.optimal_sample_size:
            ax1.axvline(
                ...
                color="r",
                linestyle=":",
                label=f"Optimal: {self.optimal_sample_size}",
            )
...
def run_sample_size_optimization(
    ...
    if save_to_config and optimizer.optimal_sample_size:
        ...
        if existing_config:
            ...
            existing_config["optimal_sample_sizes"][task_name][str(layer)] = (
                optimizer.optimal_sample_size
            )

            # Save updated config
            config_manager.update_model_config(model_name, existing_config)
            logger.info(
                f"Updated model config with optimal sample size: {optimizer.optimal_sample_size}"
            )
        else:
            logger.warning(
                "No existing model config found. Run optimize-classification first."
            )
