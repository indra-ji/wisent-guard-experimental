...
class VectorNormalizationMethod(Enum):
    ...
    L2_UNIT = "l2_unit"  # Normalize to unit length (L2 norm = 1)
    CROSS_BEHAVIOR = (
        "cross_behavior"  # Normalize across behaviors to same magnitude (CAA style)
    )
...
class VectorNormalizer:
    ...
    def normalize_vector(
        ...
        try:
            ...
            stats = {
                ...
                "scaling_factor": final_norm / original_norm
                if original_norm > 0
                else 1.0,
            }
    ...
    def normalize_cross_behavior(
        ...
        if method == VectorNormalizationMethod.CROSS_BEHAVIOR:
            ...
            raise ValueError(
                f"Method {method} not supported for cross-behavior normalization"
            )
        ...
        stats = {
            ...
            "final_norms": {
                behavior: torch.norm(vector, p=2).item()
                for behavior, vector in normalized_vectors.items()
            },
        }
    ...
    def normalize_layer_wise(
        ...
        for layer_idx, behavior_vectors in layer_behavior_vectors.items():
            if behavior_vectors:
                normalized_vectors, stats = self.normalize_cross_behavior(
                    behavior_vectors, method
                )
    ...
    def save_normalized_vectors(
        ...
        try:
            ...
            # Normalize vectors
            normalized_layers, layer_stats = self.normalize_layer_wise(
                layer_behavior_vectors, method
            )
            ...
            for layer_idx, behavior_vectors in normalized_layers.items():
                for behavior, vector in behavior_vectors.items():
                    ...
                    vector_path = os.path.join(
                        behavior_dir, f"vec_layer_{layer_idx}.pt"
                    )
                ...
                torch.save(
                    {
                        ...
                        "total_behaviors": len(
                            next(iter(normalized_layers.values()), {})
                        ),
                    },
                    stats_path,
                )
                    ...
                    if file.startswith("vec_layer_") and file.endswith(".pt"):
                        ...
                        try:
                            ...
                            if layers is None or layer_idx in layers:
                                ...
                                vector = torch.load(
                                    vector_path, map_location=self.device
                                )
...
def create_caa_normalized_vectors(
    ...
    """
    normalizer = VectorNormalizer()
    return normalizer.normalize_cross_behavior(
        behavior_vectors, VectorNormalizationMethod.CROSS_BEHAVIOR
    )
...
def integrate_normalization_with_aggregation(
    ...
    # Apply normalization
    normalized_vector, norm_stats = normalize_control_vector(
        control_vector, normalization_method, target_norm
    )
    ...
    updated_stats["final_vector_norm"] = norm_stats.get(
        "final_norm", torch.norm(normalized_vector).item()
    )
...
def get_vector_statistics(vectors: Dict[str, torch.Tensor]) -> Dict[str, Any]:
    ...
    return {
        ...
        "norm_ratio": np.max(norms) / np.min(norms)
        if np.min(norms) > 0
        else float("inf"),
    }
