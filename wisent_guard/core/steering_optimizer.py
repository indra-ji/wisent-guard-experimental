...
def get_default_steering_configs() -> List["SteeringMethodConfig"]:
    ...
    return [
        # CAA variations
        SteeringMethodConfig(name="CAA", method=SteeringMethod.CAA, params={}),
        ...
        # HPR variations
        SteeringMethodConfig(
            name="HPR", method=SteeringMethod.HPR, params={"hpr_beta": 1.0}
        ),
        SteeringMethodConfig(
            name="HPR_Beta0.5", method=SteeringMethod.HPR, params={"hpr_beta": 0.5}
        ),
        # BiPO variations
        SteeringMethodConfig(
            name="BiPO",
        ...
        SteeringMethodConfig(
            ...
            params={"dac_dynamic_control": True, "dac_entropy_threshold": 1.0},
        ),
    ]
...
class SteeringOptimizer:
    ...
    def __init__(self, model_name: str, device: str = None, verbose: bool = False):
        ...
        if self.classification_config:
            ...
            logger.info(
                f"📊 Found existing classification layer: {self.base_classification_layer}"
            )
        else:
    ...
    def optimize_steering_method_comparison(
        self,
        task_name: str,
        methods_to_test: Optional[
            Union[List[SteeringMethod], List[SteeringMethodConfig]]
        ] = None,
            ...
            for item in methods_to_test:
                if isinstance(item, SteeringMethodConfig):
                    ...
                    method_configs.append(
                        SteeringMethodConfig(name=item.value, method=item, params={})
                    )
        ...
        logger.info(
            f"🎯 Comparing {len(method_configs)} steering method configurations for task: {task_name}"
        )
        ...
        if layer_range:
            ...
            max_layer = min(
                32, self.base_classification_layer + 2
            )  # Assume max 32 layers
            ...
            for layer in layers_to_test:
                for strength in strength_range:
                    ...
                    try:
                        ...
                        if self.verbose:
                            logger.info(
                                f"   {method_config.name} L{layer} S{strength}: {score:.3f}"
                            )

                    except Exception as e:
                        logger.error(
                            f"   Error testing {method_config.name} L{layer} S{strength}: {e}"
                        )
        ...
        for layer in layer_effectiveness:
            layer_effectiveness[layer] = sum(layer_effectiveness[layer]) / len(
                layer_effectiveness[layer]
            )
        ...
        summary = SteeringOptimizationSummary(
            ...
            best_overall_method=best_overall_config["method"]
            if best_overall_config
            else "none",
            best_overall_layer=best_overall_config["layer"]
            if best_overall_config
            else 0,
            best_overall_strength=best_overall_config["strength"]
            if best_overall_config
            else 0.0,
    ...
    def optimize_steering_layer(
        ...
        """
        logger.info(
            f"🔍 Optimizing steering layer for {task_name} using {steering_method.value}"
        )
    ...
    def optimize_steering_strength(
        ...
        config_path = os.path.join(
            ...
            "..",
            "..",
            "steering_optimization_parameters.json",
        )
        ...
        if strength_range is None:
            default_strengths = steering_config.get("steering_strengths", {}).get(
        ...
        if method_params is None:
            ...
            for config in method_configs:
                if (
                    config["method"] == steering_method.value
                    and config["name"] == steering_method.value
                ):
        ...
        for strength in strengths:
            try:
                ...
                if isinstance(result, dict):
                    ...
                    if self.verbose:
                        logger.debug(
                            f"   Found {len(baseline_likes)} baseline and {len(steered_likes)} steered likelihoods"
                        )
                        if baseline_likes and len(baseline_likes) > 0:
                            logger.debug(
                                f"   First few baseline likes: {baseline_likes[:3]}"
                            )
                            logger.debug(
                                f"   First few steered likes: {steered_likes[:3]}"
                            )
                        logger.debug(
                            f"   Full eval_results keys: {list(eval_results.keys())}"
                        )
                        logger.debug(
                            f"   Accuracy value: {eval_results.get('accuracy')}"
                        )
                    ...
                    if baseline_likes and steered_likes:
                        ...
                        if valid_pairs:
                            ...
                            for i in range(
                                0, len(baseline_likes), 2
                            ):  # Assuming binary choices
                                if i + 1 < len(baseline_likes):
                                    if (
                                        np.isfinite(baseline_likes[i])
                                        and np.isfinite(baseline_likes[i + 1])
                                        and np.isfinite(steered_likes[i])
                                        and np.isfinite(steered_likes[i + 1])
                                    ):
                                        baseline_pref = (
                                            0
                                            if baseline_likes[i] > baseline_likes[i + 1]
                                            else 1
                                        )
                                        steered_pref = (
                                            0
                                            if steered_likes[i] > steered_likes[i + 1]
                                            else 1
                                        )
                ...
                logger.info(
                    f"   Strength {strength:.2f}: score={score:.3f}, effect={steering_effect:.3f}, accuracy={accuracy:.3f}"
                )

            except Exception as e:
                logger.error(f"   Error testing strength {strength}: {e}")
                results.append(
                    {"strength": float(strength), "score": 0.0, "error": str(e)}
                )
    ...
    def optimize_method_specific_parameters(
        ...
        """
        logger.info(
            f"🔧 Optimizing {steering_method.value}-specific parameters for {task_name}"
        )

        if steering_method == SteeringMethod.CAA:
            return self._optimize_caa_parameters(
                task_name, base_layer, base_strength, limit
            )
        elif steering_method == SteeringMethod.HPR:
            return self._optimize_hpr_parameters(
                task_name, base_layer, base_strength, limit
            )
        elif steering_method == SteeringMethod.DAC:
            return self._optimize_dac_parameters(
                task_name, base_layer, base_strength, limit
            )
        elif steering_method == SteeringMethod.BIPO:
            return self._optimize_bipo_parameters(
                task_name, base_layer, base_strength, limit
            )
        elif steering_method == SteeringMethod.KSTEERING:
            return self._optimize_ksteering_parameters(
                task_name, base_layer, base_strength, limit
            )
        else:
    ...
    def _optimize_ksteering_parameters(
        ...
        raise NotImplementedError(
            "K-Steering parameter optimization not yet implemented"
        )
    ...
    def run_comprehensive_steering_optimization(
        ...
        if tasks is None:
            ...
            if self.classification_config:
                task_overrides = self.classification_config.get(
                    "task_specific_overrides", {}
                )
                tasks = list(task_overrides.keys())
                if not tasks:
                    logger.warning(
                        "No classification-optimized tasks found, using default task set"
                    )
    ...
    def _save_steering_optimization_results(self, summary: SteeringOptimizationSummary):
        ...
        config["steering_optimization"]["method_ranking"] = (
            summary.method_performance_ranking
        )
    ...
    **kwargs,
    ...
    if optimization_type == "auto":
        ...
        return optimizer.optimize_steering_method_comparison(
            task_name=task_name, limit=limit, **kwargs
        )
        ...
        return optimizer.run_comprehensive_steering_optimization(limit=limit, **kwargs)
    else:
...
def run_auto_steering_optimization(
    ...
    if use_classification_config:
        ...
        if not classification_config:
            logger.info(
                "ℹ️  No classification config found, proceeding with standalone steering optimization"
            )
    ...
    if task_name:
        ...
        if "task_specific_overrides" in classification_config:
            tasks_to_optimize = list(
                classification_config["task_specific_overrides"].keys()
            )
        ...
        if not tasks_to_optimize and "optimization_metrics" in classification_config:
            ...
            if "optimal_sample_sizes" in classification_config:
                tasks_to_optimize = list(
                    classification_config["optimal_sample_sizes"].keys()
                )

        if not tasks_to_optimize:
            return {
                "error": "No task specified and no classification tasks found in config"
            }
    else:
    ...
    if methods_to_test is None:
        ...
        for method in methods_to_test:
            if method == "CAA":
                ...
                method_configs.append(
                    SteeringMethodConfig("CAA", SteeringMethod.CAA, {})
                )
                method_configs.append(
                    SteeringMethodConfig(
                        "CAA_L2",
                ...
                method_configs.append(
                    SteeringMethodConfig("HPR", SteeringMethod.HPR, {"hpr_beta": 1.0})
                )
                method_configs.append(
                    SteeringMethodConfig(
                        "HPR_Beta0.5", SteeringMethod.HPR, {"hpr_beta": 0.5}
                    )
                )
            elif method == "DAC":
                method_configs.append(
                    SteeringMethodConfig(
                        "DAC",
                        SteeringMethod.DAC,
                        {"dac_dynamic_control": True, "dac_entropy_threshold": 1.0},
                    )
                )
            elif method == "BiPO":
    ...
    for task in tasks_to_optimize:
        ...
        if not task_layer_range and classification_config and use_classification_config:
            ...
            task_overrides = classification_config.get(
                "task_specific_overrides", {}
            ).get(task, {})
            ...
            if not class_layer:
                ...
                class_layer = classification_config.get("optimal_parameters", {}).get(
            ...
            if class_layer:
                ...
                if verbose:
                    logger.info(
                        f"   Using layer range around classification layer {class_layer}: {task_layer_range}"
                    )
        ...
        try:
            ...
            task_result = {
                ...
                "score": summary.task_results[0].steering_effectiveness_score
            ...
            # Update overall best
            if (
                not results["overall_best"]
                or task_result["score"] > results["overall_best"]["score"]
            ):
                results["overall_best"] = task_result

        except Exception as e:
            logger.error(f"❌ Failed to optimize task {task}: {e}")
            results["tasks_optimized"].append({"task": task, "error": str(e)})

    # Save configuration
    if results["tasks_optimized"] and not any(
        "error" in r for r in results["tasks_optimized"]
    ):
        ...
        if verbose:
            ...
            logger.info(
                f"   Overall best: {results['overall_best']['best_method']} "
...
def get_optimal_steering_params(
    model_name: str, task_name: Optional[str] = None
) -> Optional[Dict[str, Any]]:
    """
