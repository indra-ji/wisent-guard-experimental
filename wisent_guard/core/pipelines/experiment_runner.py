...
class ExperimentRunner:
    ...
    def _load_and_split_data(self) -> Tuple[List[Any], List[Any], Dict[str, Any]]:
        ...
        logger.info(
            f"Loading data: {self.config.train_version} -> {self.config.eval_version}"
        )
        ...
        logger.info(
            f"Data loaded: {len(train_data)} train, {len(eval_data)} eval problems"
        )
    ...
    def _train_steering_vectors(self, contrastive_pairs: List[Any]) -> TrainingResults:
        ...
        logger.info(
            f"Trained {len(training_results.steering_vectors)} steering vectors"
        )
        return training_results

    def _create_steering_model(
        ...
        logger.info(
            f"Created model with {len(training_results.steering_vectors)} steering vectors"
        )
        return model

    def _save_experiment_results(
        self, training_results: TrainingResults, data_info: Dict[str, Any]
    ):
