...
class ActivationCollector:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized ActivationCollector with {model_name} on {self.device}"
        )
    ...
    def collect_contrastive_activations(
        ...
        """
        logger.info(
            f"Collecting activations for {len(contrastive_pairs)} pairs at layers {target_layers}"
        )
        ...
        for i in range(0, len(contrastive_pairs), batch_size):
            ...
            for layer_idx in target_layers:
                batch_activations = self._collect_batch_activations(
                    batch_pairs, layer_idx
                )
    ...
    def _collect_batch_activations(
        self, batch_pairs: List[ContrastivePair], layer_idx: int
    ) -> List[ActivationData]:
