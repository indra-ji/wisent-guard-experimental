...
class TrainingConfig:
    ...
    def __post_init__(self):
        if self.target_layers is None:
            self.target_layers = [
                5
            ]  # Default to layer 5 for distilgpt2 (6 layers: 0-5)
...
class SteeringVectorTrainer:
    ...
    def __init__(self, config: TrainingConfig):
        ...
        logger.info(
            f"Initialized SteeringVectorTrainer with {config.steering_method} method"
        )
    ...
    def train(self, contrastive_pairs: List[ContrastivePair]) -> TrainingResults:
        ...
        logger.info(
            f"Starting steering vector training with {len(contrastive_pairs)} pairs"
        )
        ...
        try:
            ...
            steering_vectors, training_stats = self._train_steering_vectors(
                activation_data
            )
    ...
    def _initialize_components(self):
        ...
        logger.info(
            f"Initialized components: {self.config.model_name} + {self.config.steering_method}"
        )

    def _collect_activations(
        self, contrastive_pairs: List[ContrastivePair]
    ) -> List[ActivationData]:
    ...
    def _create_contrastive_set(
        self, layer_activations: List[ActivationData]
    ) -> ContrastivePairSet:
    ...
    def save_results(self, results: TrainingResults, save_directory: str):
        ...
        for layer_idx, steering_vector in results.steering_vectors.items():
            ...
            filename = (
                f"steering_vector_{results.config.model_name}_{layer_idx}_{date_str}.pt"
            )
