...
class TrainingMetrics:
    ...
    def samples_per_second(self) -> float:
        ...
        return (
            self.training_samples / self.total_training_time
...
class LatencyTracker:
    ...
    @contextmanager
    def time_generation(
        self, name: str = "response_generation", prompt_length: int = 0
    ):
        """
        ...
        generation_state = {"first_token_time": None, "token_count": 0}

        # Add methods that modify the dict
        generation_state["mark_first_token"] = lambda: generation_state.update(
            {"first_token_time": time.time()}
        )
        generation_state["update_tokens"] = lambda count: generation_state.update(
            {"token_count": count}
        )
        ...
        try:
            ...
            # Calculate TTFT
            ttft = (
                generation_state["first_token_time"] - start_time
                if generation_state["first_token_time"]
                else 0.0
            )

            # Calculate tokens per second
            tokens_per_sec = (
                generation_state["token_count"] / total_duration
                if total_duration > 0
                else 0.0
            )
    ...
    ) -> Union[LatencyStats, Dict[str, LatencyStats]]:
        """
    ...
    def _calculate_stats(
        self, operation: str, events: List[TimingEvent]
    ) -> LatencyStats:
    ...
    def get_generation_metrics(
        self, operation_name: str = "response_generation"
    ...
    def get_training_metrics(
        self, operation_name: str = "total_training_time"
    ...
    def format_user_metrics(self) -> str:
        ...
        if steered_events and unsteered_events:
            ...
            unsteered_avg = sum(e.duration for e in unsteered_events) / len(
                unsteered_events
            )
        ...
        if isinstance(stats, LatencyStats):
            ...
            for operation, op_stats in stats.items():
                ...
                lines.extend(
                    [
                        f"  {line}"
                        for line in self._format_single_stats(op_stats, detailed).split(
                            "\n"
                        )
                    ]
                )
