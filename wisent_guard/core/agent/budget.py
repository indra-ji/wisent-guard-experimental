...
class BudgetManager:
    ...
    def set_budget(
        self, resource_type: ResourceType, amount: float, unit: str = ""
    ) -> None:
        """Set a budget for any resource type."""
        self.budgets[resource_type] = ResourceBudget(
            resource_type=resource_type, total_budget=amount, unit=unit
        )
    ...
    def calculate_max_tasks_for_budget(
        self, task_type: str = "default", time_budget_minutes: float = 5.0
    ) -> int:
        """
    ...
    def track_task_execution(
        self, task_name: str, start_time: float, end_time: float
    ) -> None:
        """
        ...
        if task_name in self.task_estimates:
            ...
            self.task_estimates[task_name] = TaskEstimate(
                task_name=task_name, time_seconds=actual_time
            )
    ...
    def _get_default_cost_estimate(
        self, task_name: str, resource_type: ResourceType
    ) -> float:
        ...
        if resource_type == ResourceType.TIME:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Device benchmark estimate failed for task '{task_name}': {e}. Run device benchmark first with: python -m wisent_guard.core.agent.budget benchmark"
                )

        elif resource_type == ResourceType.MEMORY:
            raise RuntimeError(
                f"Memory estimation not implemented for task '{task_name}'"
            )

        elif resource_type == ResourceType.COMPUTE:
            raise RuntimeError(
                f"Compute estimation not implemented for task '{task_name}'"
            )

        elif resource_type == ResourceType.TOKENS:
            raise RuntimeError(
                f"Token estimation not implemented for task '{task_name}'"
            )
...
def calculate_max_tasks_for_time_budget(
    task_type: str = "benchmark_evaluation", time_budget_minutes: float = 5.0
) -> int:
    """
    Calculate maximum number of tasks that can fit within a time budget.
    ...
    try:
        ...
        raise RuntimeError(
            f"Budget calculation failed for task '{task_type}': {e}. Run device benchmark first with: python -m wisent_guard.core.agent.budget benchmark"
        )


def optimize_tasks_for_budget(
    task_candidates: List[str],
    ...
    return _budget_manager.optimize_task_allocation(
        task_candidates, ResourceType.TIME, max_tasks
    )


def optimize_benchmarks_for_budget(
    task_candidates: List[str],
    time_budget_minutes: float = 5.0,
    max_tasks: Optional[int] = None,
    ...
    try:
        ...
        sys.path.insert(
            0, os.path.join(os.path.dirname(__file__), "..", "lm-harness-integration")
        )
        ...
        if prefer_fast:
            ...
            benchmark_info.sort(
                key=lambda x: (x["priority_score"], -x["loading_time"]), reverse=True
            )
        ...
        return optimize_tasks_for_budget(
            task_candidates, time_budget_minutes, max_tasks
        )
...
def run_device_benchmark(force_rerun: bool = False) -> None:
    ...
    print(
        f"Evaluation: {benchmark.benchmark_eval_seconds_per_100_examples:.1f}s per 100 examples"
    )
    print(
        f"Classifier Training: {benchmark.classifier_training_seconds_per_100_samples:.1f}s per 100 samples"
    )
    print(f"Steering: {benchmark.steering_seconds_per_example:.1f}s per example")
    print(
        f"Data Generation: {benchmark.data_generation_seconds_per_example:.1f}s per example"
    )
    ...
    print(
        f"100 eval examples: {benchmark.benchmark_eval_seconds_per_100_examples:.1f}s"
    )
    print(
        f"Training classifier (200 samples): {(benchmark.classifier_training_seconds_per_100_samples * 2):.1f}s"
    )
...
def main():
    ...
    budget_parser.add_argument(
        "--time-minutes", "-t", type=float, default=5.0, help="Time budget in minutes"
    )
    budget_parser.add_argument(
        "--task-type", default="benchmark_evaluation", help="Task type to optimize for"
    )
    ...
    try:
        if args.command == "benchmark":
            ...
            print(
                f"⏱️ Estimated time for {args.quantity}x {args.task_type}: {estimated_seconds:.1f} seconds ({estimated_seconds / 60:.2f} minutes)"
            )

        elif args.command == "budget":
            max_tasks = calculate_max_tasks_for_time_budget(
                args.task_type, args.time_minutes
            )
            ...
            if benchmark_type in ["benchmark_eval", "classifier_training"]:
                task_time = (
                    estimate_task_time_direct(benchmark_type, 100) / 100
                )  # Per unit
            else:
            ...
            print(
                f"Time budget: {args.time_minutes:.1f} minutes ({args.time_minutes * 60:.0f} seconds)"
            )
            ...
            print(
                f"Total estimated time: {total_time:.1f} seconds ({total_time / 60:.2f} minutes)"
            )
            print(
                f"Budget utilization: {(total_time / (args.time_minutes * 60)) * 100:.1f}%"
            )
