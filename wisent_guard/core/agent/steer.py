...
class ResponseSteering:
    ...
    def __init__(
        self,
        generate_response_func: Callable[[str], Awaitable[str]],
    ...
    async def improve_response(
        self, prompt: str, response: str, analysis: AnalysisResult
    ) -> ImprovementResult:
    ...
    def choose_improvement_method(self, issues: List[str]) -> str:
        ...
        if any(
            issue in ["scientific_myth", "factual_error_population"] for issue in issues
        ):
    ...
    async def improve_by_regeneration(
        self, prompt: str, response: str, analysis: AnalysisResult
    ) -> ImprovementResult:
        ...
        try:
            ...
            for issue in issues:
                ...
                trait_description = trait_descriptions.get(
                    issue, f"avoiding {issue} issues in responses"
                )
            ...
            if not training_pairs:
                raise ValueError(
                    f"Could not generate training data for issues: {issues}"
                )
            ...
            raise ValueError(
                f"Failed to generate training data for issues {issues}: {e}"
            )
