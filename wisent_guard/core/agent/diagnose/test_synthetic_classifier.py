...
def main():
    ...
    print(
        f"⏱️ Starting synthetic classifier test with {timeout_seconds}s timeout and {budget_minutes * 60}s budget..."
    )
    ...
    try:
        ...
        print(
            f"✅ SUCCESS: Created {len(classifiers)} classifiers for {len(trait_discovery.traits_discovered)} traits"
        )
        print(
            f"⏱️ Total time: {elapsed_time:.1f}s (timeout: {timeout_seconds}s, budget: {budget_minutes * 60}s)"
        )

        if elapsed_time > timeout_seconds:
            print(
                f"⚠️ WARNING: Test completed but exceeded timeout by {elapsed_time - timeout_seconds:.1f}s"
            )
        else:
            print(
                f"🎉 Test completed within timeout with {timeout_seconds - elapsed_time:.1f}s to spare!"
            )
        ...
        print(f"❌ ERROR: {e}")
        print(
            f"❌ Test failed after {elapsed_time:.1f}s (timeout: {timeout_seconds}s, budget: {budget_minutes * 60}s)"
        )
