        ...
        if os.path.exists(tasks_json_path):
            with open(tasks_json_path, "r") as f:
                ...
                if "task_list" in tasks_data and tasks_data["task_list"]:
                    print(
                        f"Loaded {len(tasks_data['task_list'])} tasks from local tasks.json"
                    )
        ...
        try:
            ...
            for line in result.stdout.split("\n"):
                if (
                    ...
                    and "Config Location" not in line
                ):
                    parts = line.split("|")
                    if len(parts) >= 2:
        ...
        if hasattr(task, "dataset") and hasattr(task, "fewshot_split"):
            try:
                ...
                dataset = load_dataset(
                    ...
                    else task.dataset_name,
                    task.dataset_config_name
                    if hasattr(task, "dataset_config_name")
                    else None,
                ...
                raise RuntimeError(
                    f"No labelled docs available for task {task.NAME}. Error loading fewshot split: {e}"
                )
        else:
...
def find_working_task_from_group(group_dict, max_depth=3, current_depth=0):
    ...
    try:
        ...
        for key, value in items:
            ...
            if (
                hasattr(value, "has_validation_docs")
                or hasattr(value, "has_test_docs")
                or hasattr(value, "has_training_docs")
            ):
                ...
                try:
                    ...
                    if (
                        hasattr(value, "has_validation_docs")
                        and value.has_validation_docs()
                    ):
                        has_docs = True
                    elif hasattr(value, "has_test_docs") and value.has_test_docs():
                        has_docs = True
                    elif (
                        hasattr(value, "has_training_docs")
                        and value.has_training_docs()
                    ):
                        has_docs = True
        ...
        for key, value in items:
            if hasattr(value, "items") and "ConfigurableGroup" in str(type(key)):
                result_task, result_name = find_working_task_from_group(
...
def handle_configurable_group_task(task_name: str):
    ...
    try:
        ...
        try:
            ...
            print(
                f"   📊 Registry check: {len(all_tasks)} tasks, {len(all_groups)} groups available"
            )
            ...
            if task_name in all_tasks or task_name in all_groups:
                print(
                    f"   🔍 Task {task_name} exists in registry but has loading issues"
                )

                # For group tasks, try to extract individual working tasks
                if task_name in all_groups:
                    print(
                        f"   💡 Found {task_name} as a ConfigurableGroup - extracting individual tasks..."
                    )
                    result = try_extract_working_tasks_from_group(
                        task_name, task_manager
                    )
                    if result:
                        return result
                    else:
                        print(
                            f"   💥 FAILED: Group {task_name} exists but no working tasks found!"
                        )
                        return None

                # For individual tasks that fail loading, try aggressive search
                print(
                    f"   💡 Found {task_name} as individual task - trying alternatives..."
                )
                return try_find_related_working_task(task_name)

            # If not found in registry at all, try aggressive search
            print(
                f"   🔄 Task {task_name} not found in registry, trying alternatives..."
            )
            return try_find_related_working_task(task_name)
    ...
    if task_name == "flan_held_in":
        ...
        if yaml_file_path:
            ...
            try:
                ...
                individual_tasks = extract_individual_tasks_from_yaml(
                    yaml_file_path, task_name
                )
                if individual_tasks:
                    ...
                    for extracted_task_name in individual_tasks:
                        try:
                            individual_dict = load_task_with_config_dir(
                                extracted_task_name, config_dir
                            )
                            if extracted_task_name in individual_dict:
                                task = individual_dict[extracted_task_name]
                                print(
                                    f"   ✅ Successfully loaded individual task: {extracted_task_name}"
                                )
                                return task, extracted_task_name
                        except Exception as e:
                            print(
                                f"      ❌ Failed to load {extracted_task_name}: {str(e)[:50]}"
                            )
                            continue
    ...
    yaml_candidates = []
    search_dirs = ["wisent_guard/parameters/tasks", ".", "tasks", "configs"]

    for search_dir in search_dirs:
        if os.path.exists(search_dir):
            yaml_candidates.extend(
                glob.glob(os.path.join(search_dir, f"{task_name}.yaml"))
            )
            yaml_candidates.extend(
                glob.glob(os.path.join(search_dir, f"{task_name}.yml"))
            )
    ...
    try:
        ...
        if hasattr(task, "__dict__") and isinstance(
            getattr(task, "__dict__", {}), dict
        ):
...
def extract_individual_tasks_from_yaml(
    yaml_file: str, group_name: str, _visited_files=None
) -> List[str]:
    """
    ...
    try:
        ...
        if yaml_path_normalized in _visited_files:
            print(
                f"   🔄 Cycle detected: {yaml_file} - skipping to prevent infinite recursion"
            )
            return []
        ...
        potential_tasks = list(
            set([task for task in individual_tasks if task and isinstance(task, str)])
        )

        print(
            f"   📋 Found potential tasks/groups: {potential_tasks[:5]}..."
        )  # Limit output
        ...
        for i, task_name in enumerate(potential_tasks[:max_tasks_to_process]):
            ...
            if any(
                suffix in task_name
                for suffix in ["_zeroshot_", "_fewshot_", "_cot_", "_prompt-", "_task_"]
            ):
            ...
            if len(_visited_files) < 3:  # Limit recursion depth
                ...
                if os.path.exists(potential_group_file):
                    print(
                        f"   🔍 Found nested group file: {os.path.basename(potential_group_file)}"
                    )
                    # Recursively extract from this group
                    nested_tasks = extract_individual_tasks_from_yaml(
                        potential_group_file, task_name, _visited_files.copy()
                    )
                ...
                for subdir in ["zeroshot", "fewshot", "cot"]:
                    ...
                    if os.path.isdir(subdir_path):
                        subdir_yaml = os.path.join(
                            subdir_path, f"_{task_name}_{subdir}.yaml"
                        )
                        if os.path.exists(subdir_yaml):
                            print(f"   🔍 Found nested group in subdir: {subdir}")
                            nested_tasks = extract_individual_tasks_from_yaml(
                                subdir_yaml,
                                f"{task_name}_{subdir}",
                                _visited_files.copy(),
                            )
                            resolved_tasks.extend(nested_tasks[:3])  # Limit results
                            break
...
def try_find_related_working_task(task_name: str):
    ...
    try:
        ...
        print(
            f"   🔄 AGGRESSIVE SEARCH for working alternatives to '{task_name}' ({len(all_available_tasks)} tasks available)..."
        )
        ...
        matching_tasks = [
            t
            for t in all_available_tasks
            if t.startswith(prefix + "_") and t != task_name
        ]
        ...
        for keyword in keywords:
            ...
            keyword_tasks = [
                t for t in all_available_tasks if keyword in t and t != task_name
            ]
        ...
        print(
            f"   💥 FAILED TO FIND CORRECT TASK: {task_name} - NO RANDOM FALLBACKS ALLOWED!"
        )
        return None
...
def try_extract_working_tasks_from_group(group_name: str, task_manager):
    ...
    try:
        ...
        if (
            hasattr(task_manager, "task_index")
            and group_name in task_manager.task_index
        ):
            ...
            if yaml_path and os.path.exists(yaml_path):
                ...
                try:
                    ...
                    if initial_tasks:
                        print(
                            f"   📋 Found {len(initial_tasks)} initial tasks from main YAML: {initial_tasks[:5]}..."
                        )
                        ...
                        for task_name in initial_tasks[:15]:  # Try more tasks
                            try:
                                ...
                                result = get_task_dict(
                                    [task_name], task_manager=task_manager
                                )
                                if task_name in result:
                                    task = result[task_name]
                                    print(
                                        f"   ✅ SUCCESS: Found working initial task {task_name}"
                                    )
                                    return task, task_name
                            except Exception as e:
                                print(
                                    f"      ❌ Initial task {task_name} failed: {str(e)[:50]}"
                                )
                                continue
                    else:
                    ...
                    print(
                        f"   ⚠️  Main YAML parsing failed: {str(yaml_parse_error)[:100]}"
                    )

                # Fallback: try the recursive extraction method
                try:
                    individual_tasks = extract_individual_tasks_from_yaml(
                        yaml_path, group_name
                    )

                    if individual_tasks:
                        print(
                            f"   📋 Found {len(individual_tasks)} individual tasks in group"
                        )
                        ...
                        for base_task in base_tasks_to_try:
                            try:
                                ...
                                result = get_task_dict(
                                    [base_task], task_manager=task_manager
                                )
                                if base_task in result:
                                    task = result[base_task]
                                    print(
                                        f"   ✅ SUCCESS: Found working base task {base_task}"
                                    )
                                    return task, base_task
                            except Exception as e:
                                print(
                                    f"      ❌ Base task {base_task} failed: {str(e)[:50]}"
                                )
                                continue
                        ...
                        valid_tasks = [
                            t
                            for t in individual_tasks
                            if not any(
                                x in t
                                for x in ["{{", "}}", "_common_yaml", "sentence:"]
                            )
                        ]
                        for individual_task in valid_tasks[
                            :5
                        ]:  # Try first 5 valid ones
                            try:
                                print(
                                    f"   🎯 Trying individual task: {individual_task}"
                                )
                                result = get_task_dict(
                                    [individual_task], task_manager=task_manager
                                )
                                if individual_task in result:
                                    task = result[individual_task]
                                    print(
                                        f"   ✅ SUCCESS: Found working individual task {individual_task}"
                                    )
                                    return task, individual_task
                            except Exception as e:
                                print(
                                    f"      ❌ Individual task {individual_task} failed: {str(e)[:50]}"
                                )
                                continue

                except Exception as yaml_error:
                    print(
                        f"   ⚠️  YAML extraction failed (likely !function constructor): {str(yaml_error)[:100]}"
                    )
        ...
        print(
            "   🔍 FINAL CATCH-ALL: Searching registry for tasks matching group pattern..."
        )
        ...
        for part in group_parts:
            ...
            matching_tasks.sort(
                key=lambda x: (part in x.split("_"), len(x)), reverse=True
            )
        ...
        print(
            f"   💥 FAILED: Group {group_name} has no working tasks - exhausted all generic approaches"
        )
...
def create_task_yaml_from_user_content(
    task_name: str, user_yaml_content: str
) -> Optional[str]:
    """
...
def load_task_with_config_dir(task_name: str, config_dir: str):
    ...
    try:
        ...
        try:
            ...
            task_manager = LMTaskManager()
            if hasattr(task_manager, "initialize_tasks") or hasattr(
                task_manager, "load_config"
            ):
...
class TaskManager:
    ...
    def resolve_task_name(self, task_name: str) -> str:
        ...
        suggestions = [
            ...
            if any(word.lower() in task.lower() for word in task_name.split("_"))
        ][:5]
    ...
    def split_task_data(
        self, task_data, split_ratio: float = 0.8, random_seed: int = 42
