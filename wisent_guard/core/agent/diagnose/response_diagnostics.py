...
class ResponseDiagnostics:
    ...
    def __init__(self, model: Model, classifier_configs: List[Dict[str, Any]]):
        ...
        if not classifier_configs:
            raise ValueError(
                "classifier_configs is required - no fallback mode available"
            )
        ...
        for config in classifier_configs:
            try:
                ...
                print(
                    f"✅ Loaded classifier for {config['issue_type']} at layer {config['layer']}"
                )
        ...
        if not self.classifiers:
            raise RuntimeError(
                "Failed to load any classifiers - system cannot operate without them"
            )
    ...
    async def analyze_response(self, response: str, prompt: str) -> AnalysisResult:
        ...
        if not confidence_scores:
            ...
            confidence = (
                sum(result["confidence"] for result in classifier_results)
                / len(classifier_results)
    ...
    def _analyze_with_classifiers(self, response: str) -> List[Dict[str, Any]]:
        ...
        for classifier_config in self.classifiers:
            ...
            activations_tensor = self.model.extract_activations(
                response, classifier_config["layer"]
            )
            ...
            probability = (
                prob_result[0]
                if isinstance(prob_result, (list, tuple))
                else prob_result
            )
