...
class ClassifierCreator:
    ...
    def create_classifier_for_issue_type(
        ...
        if config is None:
            config = TrainingConfig(
                issue_type=issue_type, layer=layer, model_name=self.model.name
            )
        ...
        training_data = self._generate_training_data(
            issue_type, config.training_samples
        )
        ...
        classifier = self._train_classifier(
            harmful_activations, harmless_activations, config
        )

        # Evaluate performance
        print("   📈 Evaluating performance...")
        metrics = self._evaluate_classifier(
            classifier, harmful_activations, harmless_activations
        )
    ...
    def create_multi_layer_classifiers(
        ...
        for layer in layers:
            config = TrainingConfig(
                ...
                save_path=f"{save_base_path}_layer_{layer}.pkl"
                if save_base_path
                else None,
            )
    ...
    def optimize_classifier_for_performance(
        ...
        if layer_range is None:
            ...
            print(
                f"   📊 Auto-detected {total_layers} layers, testing range {layer_range[0]}-{layer_range[1]}"
            )

        best_result = None
        best_score = 0.0

        layers_to_test = range(
            layer_range[0], layer_range[1] + 1, 2
        )  # Test every 2nd layer
        ...
        for layer in layers_to_test:
            for classifier_type in classifier_types:
                ...
                try:
                    result = self.create_classifier_for_issue_type(
                        issue_type, layer, config
                    )
                    score = result.performance_metrics.get(target_metric, 0.0)

                    print(
                        f"      Layer {layer}, {classifier_type}: {target_metric}={score:.3f}"
                    )
        ...
        if best_result is None:
            raise RuntimeError(
                f"Failed to create any working classifier for {issue_type}"
            )
    ...
    async def create_classifier_for_issue_with_benchmarks(
        ...
        """
        print(
            f"🎯 Creating {issue_type} classifier using benchmarks: {relevant_benchmarks}"
        )
    ...
    async def create_combined_benchmark_classifier(
        ...
        """
        print(
            f"🏗️ Creating combined classifier from {len(benchmark_names)} benchmarks..."
        )
        print(f"   📊 Benchmarks: {benchmark_names}")
        print(
            f"   🧠 Using layer {classifier_params.optimal_layer}, {classifier_params.training_samples} samples"
        )
    ...
    async def _load_combined_benchmark_data(
        ...
        for benchmark_name in benchmark_names:
            try:
                ...
                benchmark_data = self._load_benchmark_data(
                    [benchmark_name], samples_per_benchmark
                )
                combined_data.extend(benchmark_data)
                print(
                    f"         ✅ Loaded {len(benchmark_data)} samples from {benchmark_name}"
                )
        ...
        if len(combined_data) < total_samples:
            ...
            print(
                f"      🔧 Generating {remaining_samples} synthetic samples to reach target"
            )
            synthetic_data = self._generate_synthetic_training_data(
                "quality", remaining_samples
            )
    ...
    async def create_classifier_for_issue(
        self, issue_type: str, layer: int = 15
    ) -> TrainingResult:
        """
    ...
    def _generate_training_data(
        self, issue_type: str, num_samples: int
    ) -> List[Dict[str, Any]]:
        """
        ...
        if relevant_benchmarks:
            print(
                f"   🎯 Found {len(relevant_benchmarks)} relevant benchmarks: {relevant_benchmarks[:3]}..."
            )
    ...
    def _find_relevant_benchmarks(
        self, issue_type: str, time_budget_minutes: float = 5.0
    ) -> List[str]:
        ...
        try:
            ...
            print(
                f"   🕐 Time budget: {time_budget_minutes:.1f}min → max {max_tasks} tasks"
            )
            ...
            try:
                ...
                sys.path.insert(
                    0,
                    os.path.join(
                        os.path.dirname(__file__), "..", "..", "lm-harness-integration"
                    ),
                )
                ...
                relevant_benchmarks = [
                    result["benchmark"] for result in relevant_results
                ]

                if relevant_benchmarks:
                    print(
                        f"   📊 Found {len(relevant_benchmarks)} priority-aware benchmarks for '{issue_type}':"
                    )
                    for i, result in enumerate(relevant_results[:3]):
                        ...
                        loading_time_str = (
                            f" (loading time: {result.get('loading_time', 60.0):.1f}s)"
                        )
                        print(
                            f"      {i + 1}. {result['benchmark']}{priority_str}{loading_time_str}"
                        )
                ...
                candidate_benchmarks = [
                    task_name for task_name, score in relevant_task_results
                ]
                ...
                if relevant_benchmarks:
                    print(
                        f"   📊 Found {len(relevant_benchmarks)} relevant benchmarks for '{issue_type}':"
                    )
                    ...
                    for i, (task_name, score) in enumerate(relevant_task_results[:3]):
                        if task_name in relevant_benchmarks:
                            print(
                                f"      {i + 1}. {task_name} (relevance: {score:.3f})"
                            )
                    if len(relevant_benchmarks) > 3:
                        print(f"      ... and {len(relevant_benchmarks) - 3} more")
    ...
    def _extract_benchmark_concepts(
    ...
    def _extract_domain_concepts(
        self, benchmark_name: str, tokens: List[str]
    ) -> List[str]:
    ...
    def _calculate_benchmark_relevance(
        self, issue_type: str, benchmark_concepts: Dict[str, List[str]]
    ) -> List[str]:
        ...
        for benchmark_name, concepts in benchmark_concepts.items():
            score = self._calculate_semantic_similarity(
                issue_type, benchmark_name, concepts
            )
    ...
    def _calculate_semantic_similarity(
        self, issue_type: str, benchmark_name: str, concepts: List[str]
    ) -> float:
            ...
            for benchmark_token in benchmark_tokens:
                if len(issue_token) > 2 and len(benchmark_token) > 2:
                    if issue_token == benchmark_token:
                        ...
                        issue_token in benchmark_token or benchmark_token in issue_token
                    ):
                        score += 1.0
    ...
    def _load_benchmark_data(
        self, benchmarks: List[str], num_samples: int
    ) -> List[Dict[str, Any]]:
        ...
        for benchmark in benchmarks:
            try:
                ...
                task_data = task_manager.load_task(
                    benchmark, limit=samples_per_benchmark * 3
                )
                ...
                for pair in qa_pairs[:samples_per_benchmark]:
                    if self._is_valid_pair(pair):
                        training_data.append(
                            {
                                "prompt": pair.get(
                                    "question", f"Context from {benchmark}"
                                ),
                ...
                print(
                    f"     ✅ Loaded {len(qa_pairs[:samples_per_benchmark])} examples from {benchmark}"
                )
        ...
        if not training_data:
            print(
                "   ❌ Failed to load from any benchmarks, falling back to synthetic..."
            )
    ...
    def _generate_synthetic_training_data(
        ...
        from ...contrastive_pairs.generate_synthetically import (
        ...
        try:
            ...
            trait_description = trait_descriptions.get(
                issue_type, f"avoiding {issue_type} issues in responses"
            )
            ...
            print(
                f"   ✅ Generated {len(training_data)} synthetic examples for {issue_type}"
            )
            ...
            raise ValueError(
                f"Cannot generate training data for issue type: {issue_type}"
            )
    ...
    def _extract_activations_from_data(
        ...
        for example in training_data:
            ...
            harmful_tensor = self.model.extract_activations(
                example["harmful_response"], layer_obj
            )
            ...
            harmless_tensor = self.model.extract_activations(
                example["harmless_response"], layer_obj
            )
    ...
    def _save_classifier(
        ...
        save_path = ModelPersistence.save_classifier(
            classifier.classifier, config.layer, config.save_path, metadata
        )
