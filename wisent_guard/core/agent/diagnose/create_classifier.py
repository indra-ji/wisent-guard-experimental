...
class ClassifierCreator:
    ...
    def create_classifier_for_issue_type(
        ...
        print(f"   ✅ Classifier created in {training_time:.2f}s "
    ...
    def optimize_classifier_for_performance(
        ...
        print(f"   ✅ Best configuration: Layer {best_result.config.layer}, "
    ...
    async def create_classifier_for_issue_with_benchmarks(
        ...
        print(f"   ✅ Benchmark-based classifier created in {training_time:.2f}s "
              f"(F1: {metrics.get('f1', 0):.3f}, Accuracy: {metrics.get('accuracy', 0):.3f})")
    ...
    async def create_combined_benchmark_classifier(
        ...
        print(f"   ✅ Combined classifier created in {training_time:.2f}s "
              f"(F1: {metrics.get('f1', 0):.3f}, Accuracy: {metrics.get('accuracy', 0):.3f})")
        
        return result

    async def _load_combined_benchmark_data(self, benchmark_names: List[str], total_samples: int) -> List[Dict[str, Any]]:
        """
    ...
    def _extract_benchmark_concepts(self, benchmark_names: List[str]) -> Dict[str, List[str]]:
        ...
        for name in benchmark_names:
            ...
            semantic_tokens = []
            skip_tokens = {'the', 'and', 'or', 'of', 'in', 'on', 'at', 'to', 'for', 'with', 
                          'by', 'from', 'as', 'is', 'are', 'was', 'were', 'be', 'been', 'being',
                          'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'could',
                          'should', 'may', 'might', 'can', 'light', 'full', 'val', 'test', 'dev',
                          'mc1', 'mc2', 'mt', 'cot', 'fewshot', 'zeroshot', 'generate', 'until',
                          'multiple', 'choice', 'group', 'subset'}
    ...
    def _extract_semantic_roots(self, word: str) -> List[str]:
        ...
        suffixes = ['ing', 'tion', 'sion', 'ness', 'ment', 'able', 'ible', 'ful', 'less', 'ly', 'al', 'ic', 'ous', 'ive']
    ...
    def _calculate_benchmark_relevance(self, issue_type: str, 
                                     benchmark_concepts: Dict[str, List[str]]) -> List[str]:
    ...
    def _calculate_semantic_similarity(self, issue_type: str, benchmark_name: str, 
    ...
    def _are_semantically_similar(self, term1: str, term2: str) -> bool:
        ...
        return (char_similarity > 0.6 or 
                substring_match or 
                affix_similarity > 0.4 or
                prefix_len >= 3 or 
                suffix_len >= 3)
    ...
    def _is_valid_pair(self, pair: Dict[str, Any]) -> bool:
        ...
        return (
            pair.get("correct_answer") and 
            pair.get("incorrect_answer") and
            len(pair.get("correct_answer", "")) > 5 and
            len(pair.get("incorrect_answer", "")) > 5 and
            pair.get("correct_answer") != pair.get("incorrect_answer")
        )
    ...
    def _generate_synthetic_training_data(self, issue_type: str, num_samples: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            trait_description = trait_descriptions.get(
                issue_type, 
                f"avoiding {issue_type} issues in responses"
            )
    ...
    def _extract_activations_from_data(
        ...
        for example in training_data:
            ...
            harmful_activation = Activations(
            ...
            harmless_activation = Activations(
