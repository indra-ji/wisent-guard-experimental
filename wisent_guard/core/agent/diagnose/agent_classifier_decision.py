...
sys.path.append(
    os.path.join(os.path.dirname(__file__), "..", "..", "lm-harness-integration")
)
...
class TaskAnalysis:
    ...
    prompt_content: str
    relevant_benchmarks: List[Dict[str, Any]] = (
        None  # Selected benchmarks for training and steering
    )
...
class ClassifierParams:
    ...
    aggregation_method: str = (
        "last_token"  # last_token/mean/max for activation aggregation
    )
    token_aggregation: str = (
        "average"  # average/final/first/max/min for token score aggregation
    )
    num_epochs: int = 50
    batch_size: int = 32
...
class AgentClassifierDecisionSystem:
    ...
    def analyze_task_requirements(
        ...
        return TaskAnalysis(
            prompt_content=prompt, relevant_benchmarks=relevant_benchmarks
        )

    def _get_relevant_benchmarks_for_prompt(
        self,
        prompt: str,
        existing_model=None,
        priority: str = "all",
        ...
        try:
            ...
            return [
                {
                    "benchmark": "mmlu",
                    "explanation": "General knowledge benchmark",
                    "relevance_score": 1,
                    "priority": "high",
                    "loading_time": 9.5,
                },
                {
                    ...
                    "explanation": "Truthfulness benchmark",
                    "relevance_score": 2,
                    "priority": "high",
                    "loading_time": 11.2,
                },
                {
                    "benchmark": "hellaswag",
                    "explanation": "Commonsense reasoning benchmark",
                    "relevance_score": 3,
                    "priority": "high",
                    "loading_time": 12.8,
                },
    ...
    async def create_single_quality_classifier(
        self,
        task_analysis: TaskAnalysis,
        classifier_params: "ClassifierParams",
        quality_threshold: float = 0.3,
        time_budget_minutes: float = 10.0,
    ) -> SingleClassifierDecision:
        """
        ...
        """
        print(
            f"🔍 Creating single quality classifier from {len(task_analysis.relevant_benchmarks)} benchmark(s)..."
        )
        ...
        model_name = (
            ...
            if hasattr(classifier_params, "model_name")
            else "unknown"
        )
        ...
        for classifier in available_classifiers:
            ...
            if (
                benchmark_name.lower() in classifier.path.lower()
                and str(layer) in classifier.path
                and classifier.layer == layer
            ):
        ...
        if (
            existing_classifier
            and existing_classifier.quality_score >= quality_threshold
        ):
            return SingleClassifierDecision(
    ...
    async def execute_single_classifier_decision(
        self, decision: SingleClassifierDecision, classifier_params: "ClassifierParams"
    ) -> Optional[Any]:
        """
        ...
        if decision.action == "skip":
            ...
            print(
                f"   📦 Using existing classifier: {decision.selected_classifier.path}"
            )
            ...
            benchmark_name = (
                decision.benchmark_names[0] if decision.benchmark_names else "unknown"
            )
            ...
            try:
                ...
                new_classifier = await self._create_single_benchmark_classifier(
                    benchmark_name=benchmark_name, classifier_params=classifier_params
                )

                creation_time = time.time() - start_time
                print(
                    f"      ✅ Classifier created successfully in {creation_time:.1f}s"
                )
    ...
    async def _create_single_benchmark_classifier(
        self, benchmark_name: str, classifier_params: "ClassifierParams"
    ) -> Optional[Any]:
        """
        ...
        try:
            ...
            config = TrainingConfig(
                ...
                model_name=self.marketplace.model.name
                if self.marketplace.model
                else "unknown",
            )
    ...
    async def _create_combined_classifier(
        ...
        try:
            ...
            print(
                f"      📊 Loading combined training data from benchmarks: {benchmark_names}"
            )
            classifier = await creator.create_combined_benchmark_classifier(
                benchmark_names=benchmark_names, classifier_params=classifier_params
            )
    ...
    async def make_classifier_decisions(
        self,
        task_analysis: TaskAnalysis,
        quality_threshold: float = 0.3,
        time_budget_minutes: float = 10.0,
        ...
        for benchmark_info in task_analysis.relevant_benchmarks:
            ...
            existing_options = [
                c
                for c in available_classifiers
                if benchmark_name.lower() in c.path.lower()
            ]
            best_existing = (
                max(existing_options, key=lambda x: x.quality_score)
                if existing_options
                else None
            )
            ...
            if decision.action == "create_new":
                training_time_seconds = (
                    creation_estimate.estimated_training_time_minutes * 60
                )
                budget_manager.get_budget(ResourceType.TIME).spend(
                    training_time_seconds
                )
                classifier_count += 1
                remaining_minutes = (
                    budget_manager.get_budget(ResourceType.TIME).remaining_budget / 60
                )
    ...
    def _evaluate_benchmark_classifier_options(
        self,
        benchmark_name: str,
    ...
    async def execute_decisions(
        self, decisions: List[ClassifierDecision]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for decision in decisions:
            if decision.action == "skip":
                ...
                print(
                    f"   📎 Using existing {decision.issue_type} classifier: {config['path']}"
                )

            elif decision.action == "create_new":
                print(
                    f"   🏗️ Creating new classifier for benchmark: {decision.benchmark_name}..."
                )
                start_time = time.time()
                try:
                    ...
                    classifier_configs.append(config)
                    print(
                        f"      ✅ Created: {config['path']} (took {end_time - start_time:.1f}s)"
                    )
                except Exception as e:
                    print(
                        f"      ❌ Failed to create {decision.benchmark_name} classifier: {e}"
                    )
                    continue
    ...
    async def smart_classifier_selection(
        self,
        prompt: str,
        context: str = "",
        quality_threshold: float = 0.3,
