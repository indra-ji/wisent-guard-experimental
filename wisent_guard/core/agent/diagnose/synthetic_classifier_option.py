...
from ....core.contrastive_pairs.generate_synthetically import (
...
class AutomaticTraitDiscovery:
    ...
    def discover_relevant_traits(
        self, prompt: str, time_budget_minutes: float
        ...
        max_traits = calculate_max_tasks_for_time_budget(
            "classifier_training", time_budget_minutes
        )
        max_traits = max(1, min(max_traits, 5))  # Cap between 1-5 traits
        print(
            f"   💰 Budget system: {time_budget_minutes:.1f} min budget → max {max_traits} traits"
        )
    ...
    def create_classifier_from_trait(
        ...
        try:
            ...
            if len(pair_set.pairs) < 3:
                raise ValueError(
                    f"Insufficient training pairs generated: {len(pair_set.pairs)}"
                )
            ...
            print(
                f"      🧠 Extracting activations from {len(pair_set.pairs)} pairs..."
            )
            ...
            print(
                f"      🔧 Created Layer object: index={layer_obj.index}, type={layer_obj.type}"
            )
            ...
            for i, pair in enumerate(pair_set.pairs):
                ...
                try:
                    ...
                    print(
                        f"         📊 Extracting positive activations for: {repr(pair.positive_response.text[:100])}"
                    )
                    pos_activations = self.model.extract_activations(
                    ...
                    print(
                        f"         📊 Extracting negative activations for: {repr(pair.negative_response.text[:100])}"
                    )
                    neg_activations = self.model.extract_activations(
                    ...
                    print(
                        f"         ⚠️ Error extracting activations for pair {i + 1}: {e}"
                    )
                    import traceback
            ...
            print(
                f"         Positive activations collected: {len(positive_activations)}"
            )
            print(
                f"         Negative activations collected: {len(negative_activations)}"
            )
            print(f"         Total pairs processed: {len(pair_set.pairs)}")
            print(
                f"         Success rate: {(len(positive_activations) / len(pair_set.pairs) * 100):.1f}%"
            )
            ...
            try:
                ...
                training_result = classifier.train_on_activations(
                    harmful_activations, harmless_activations
                )
                print(
                    f"      ✅ Classifier training completed successfully! Result: {training_result}"
                )
            except Exception as e:
            ...
            print(
                f"   ❌ Error creating classifier for trait '{trait_description}': {e}"
            )
            raise
...
class SyntheticClassifierSystem:
    ...
    def create_classifiers_for_prompt(
        ...
        """
        print(
            f"🔍 Creating synthetic classifiers for prompt (budget: {time_budget_minutes:.1f} minutes)..."
        )
        ...
        try:
            ...
            model_loading_cost = estimate_task_time_direct(
                "model_loading", 1
            )  # Already loaded, minimal cost
            trait_discovery_cost = 10.0  # Estimate: simple text generation ~10s
            data_generation_cost = estimate_task_time_direct(
                "data_generation", 1
            )  # Per pair
            ...
            print(
                f"      • Classifier training: ~{classifier_training_cost:.0f}s per classifier"
            )
        ...
        min_required_time = (
            trait_discovery_cost + (data_generation_cost * 3) + classifier_training_cost
        )

        if budget_seconds < min_required_time:
            print(
                f"   💰 Budget ({budget_seconds:.0f}s) too small for full classifier training"
            )
            ...
            discovery_result = self.trait_discovery.discover_relevant_traits(
                prompt, time_budget_minutes
            )
            print(
        ...
        cost_per_trait = (
            data_generation_cost * pairs_per_trait
        ) + classifier_training_cost
        ...
        print(
            f"      • Cost per trait ({pairs_per_trait} pairs): {cost_per_trait:.0f}s"
        )
        ...
        for i, trait_description in enumerate(affordable_traits):
            print(
                f"   🎯 Creating classifier {i + 1}/{len(affordable_traits)}: {trait_description}"
            )
            ...
            if total_estimated_cost > remaining_budget:
                ...
                max_affordable_pairs = max(
                    3,
                    int(
                        (remaining_budget - classifier_training_cost)
                        / data_generation_cost
                    ),
                )
                if max_affordable_pairs < 3:
                    print(
                        f"      💰 Insufficient budget ({remaining_budget:.0f}s) for training, skipping"
                    )
                    continue
                else:
                    print(
                        f"      💰 Reducing pairs from {pairs_per_trait} to {max_affordable_pairs} to fit budget"
                    )
            ...
            try:
                ...
                classifier, pairs_count = (
                    self.classifier_factory.create_classifier_from_trait(
                ...
                print(
                    f"      ✅ Classifier created with {pairs_count} training pairs ({actual_time:.0f}s)"
                )
    ...
    def apply_classifiers_to_response(
        ...
        try:
            response_activations, _ = self.model.extract_activations(
                response_text, layer=15
            )
        except Exception as e:
        ...
        for i, classifier in enumerate(classifiers):
            ...
            print(
                f"   🔬 Applying classifier {i + 1}/{len(classifiers)}: {trait_description}"
            )
            ...
            try:
                ...
                print(
                    f"      ✅ Result: prediction={prediction}, confidence={confidence_score:.3f}"
                )

            except Exception as e:
                print(
                    f"      ❌ Error applying classifier for trait '{trait_description}': {e}"
                )
                continue
...
def get_time_budget_from_manager() -> float:
    ...
    if not time_budget:
        raise ValueError(
            "No time budget set in budget manager. Call set_time_budget(minutes) first."
        )
...
def create_classifiers_for_prompt(
    ...
    return system.create_classifiers_for_prompt(
        prompt, time_budget_minutes, pairs_per_trait
    )
...
def apply_classifiers_to_response(
    ...
    """
    system = create_synthetic_classifier_system(model)
    return system.apply_classifiers_to_response(
        response_text, classifiers, trait_discovery
    )
...
def create_classifier_from_trait_description(
    ...
    log_and_print(
        f"📝 Stored metadata: trait='{trait_description}', pairs_count={len(pair_set.pairs)}"
    )
