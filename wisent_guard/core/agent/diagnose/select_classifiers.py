...
class ClassifierSelector:
    ...
    def discover_classifiers(self) -> List[ClassifierInfo]:
        ...
        self.discovered_classifiers.sort(
    ...
    def select_classifiers(self, criteria: SelectionCriteria) -> List[Dict[str, Any]]:
        ...
        for issue_type in criteria.required_issue_types:
            ...
            if best_classifier:
                ...
                print(
                    f"   ✅ Selected for {issue_type}: {os.path.basename(best_classifier.path)} "
                    f"(layer {best_classifier.layer}, score: {best_classifier.performance_score:.3f})"
                )
            else:
                print(f"   ❌ No classifier found for {issue_type}")
                raise ValueError(
                    f"No suitable classifier found for issue type: {issue_type}"
                )
    ...
    def _find_best_classifier_for_issue_type(
        self, issue_type: str, criteria: SelectionCriteria
        ...
        for classifier in self.discovered_classifiers:
            ...
            if (
                classifier.issue_type == issue_type
                or issue_type in classifier.issue_type
                or classifier.issue_type in issue_type
            ):
                # Check performance threshold
                if classifier.performance_score >= criteria.min_performance_score:
                    # Check layer preferences
                    if (
                        criteria.preferred_layers is None
                        or classifier.layer in criteria.preferred_layers
                    ):
        ...
        return (
            max(candidates, key=lambda x: x.performance_score) if candidates else None
        )

    def _is_model_compatible(
        self, classifier: ClassifierInfo, model_name: Optional[str]
    ) -> bool:
        """
    ...
    def _add_supplementary_classifiers(
        self, selected_classifiers: List[Dict[str, Any]], criteria: SelectionCriteria
    ):
        """
        ...
        for classifier in self.discovered_classifiers:
            ...
            if (
                classifier.path not in selected_paths
                and classifier.performance_score >= criteria.min_performance_score
            ):
                config = {
                ...
                print(
                    f"   ➕ Added supplementary: {os.path.basename(classifier.path)} "
    ...
    def get_classifier_summary(self) -> str:
        ...
        for issue_type, classifiers in by_issue_type.items():
            ...
            for classifier in sorted(
                classifiers, key=lambda x: x.performance_score, reverse=True
            )[:3]:
...
def auto_select_classifiers_for_agent(
    ...
    if required_issue_types is None:
        required_issue_types = ["hallucination", "quality", "harmful", "bias"]
