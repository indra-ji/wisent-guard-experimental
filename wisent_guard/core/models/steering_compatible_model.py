...
class SteeringCompatibleModel(GPT2LMHeadModel):
    ...
    def add_steering_vector(
        ...
        if layer_idx >= len(self.transformer.h):
            raise ValueError(
                f"Layer index {layer_idx} out of range for model with {len(self.transformer.h)} layers"
            )
        ...
        if layer_idx not in indices:
            ...
            self._steering_layer_indices = torch.tensor(
                sorted(indices), dtype=torch.long
            )
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        if indices_key in state_dict:
            ...
            if new_size > 0:
                self.register_buffer(
                    "_steering_layer_indices",
                    torch.zeros(new_size, dtype=torch.long),
                    persistent=True,
                )
        ...
        for name, buffer in self.named_buffers():
            if name.startswith("_steering_vector_"):
                try:
                    ...
                    logger.warning(
                        f"Could not parse layer index from buffer name: {name}"
                    )

        logger.info(
            f"Restored {len(self.steering_vectors)} steering vectors from model state"
        )
        ...
        def create_steering_hook(layer_idx, steering_vector):
            def hook_fn(module, input, output):
                ...
                hidden_states[:, -1, :] = hidden_states[
                    :, -1, :
                ] + steering_vector_expanded.squeeze(0).squeeze(0)
    ...
    def save_steering_vectors(self, save_directory: str):
        ...
        logger.info(
            f"Saved {len(self.steering_vectors)} steering vectors to {save_directory}"
        )
                ...
                if len(parts) >= 4:
                    try:
                        ...
                        metadata_filepath = os.path.join(
                            load_directory, metadata_filename
                        )
                        metadata = {}
                        ...
                        logger.warning(
                            f"Could not parse layer index from filename: {filename}"
                        )
    ...
    def from_pretrained_with_steering(
        cls, model_name_or_path: str, steering_directory: Optional[str] = None, **kwargs
    ):
        """
...
def create_steering_compatible_model(
    ...
    return SteeringCompatibleModel.from_pretrained_with_steering(
        base_model_name, steering_directory=steering_directory
    )
