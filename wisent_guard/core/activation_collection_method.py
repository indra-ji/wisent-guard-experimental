...
class PromptConstructionStrategy(Enum):
    ...
    MULTIPLE_CHOICE = (
        "multiple_choice"  # Original: "Which is better: Q A. bad B. good" -> "A"/"B"
    )
    ROLE_PLAYING = (
        "role_playing"  # "Behave like person who would answer Q with good_resp" -> "I"
    )
    DIRECT_COMPLETION = "direct_completion"  # "Q" -> "good_resp"/"bad_resp"
    INSTRUCTION_FOLLOWING = (
        "instruction_following"  # "[INST] Q [/INST]" -> "good_resp"/"bad_resp"
    )
...
class TokenTargetingStrategy(Enum):
    ...
    CONTINUATION_TOKEN = (
        "continuation_token"  # Target first token of continuation ("I", etc.)
    )
...
class PromptPair:
    ...
    def __init__(
        self, positive_prompt: str, negative_prompt: str, target_token: str = None
    ):
...
class ActivationCollectionLogic:
    ...
    def _construct_multiple_choice_prompts(
        ...
        mc_question = (
            f"Which is better: {question} A. {incorrect_answer} B. {correct_answer}"
        )
    ...
    def _construct_role_playing_prompts(
        self, question: str, correct_answer: str, incorrect_answer: str
    ) -> PromptPair:
    ...
    def construct_prompt_pair(
        ...
        if prompt_strategy == PromptConstructionStrategy.MULTIPLE_CHOICE:
            return self._construct_multiple_choice_prompts(
                question, correct_answer, incorrect_answer
            )
        elif prompt_strategy == PromptConstructionStrategy.ROLE_PLAYING:
            return self._construct_role_playing_prompts(
                question, correct_answer, incorrect_answer
            )
        elif prompt_strategy == PromptConstructionStrategy.DIRECT_COMPLETION:
            return self._construct_direct_completion_prompts(
                question, correct_answer, incorrect_answer
            )
        elif prompt_strategy == PromptConstructionStrategy.INSTRUCTION_FOLLOWING:
            return self._construct_instruction_following_prompts(
                question, correct_answer, incorrect_answer
            )
        else:
    ...
    def create_contrastive_pair(
        ...
        """
        prompt_pair = self.construct_prompt_pair(
            question, correct_answer, incorrect_answer, prompt_strategy
        )
    ...
    def create_batch_contrastive_pairs(
        ...
        for qa_pair in qa_pairs:
            ...
            incorrect = qa_pair.get("incorrect_answer") or qa_pair.get(
                "incorrect_choice"
            )
            if not incorrect:
                raise KeyError(
                    f"Missing 'incorrect_answer' field in qa_pair: {qa_pair.keys()}"
                )
    ...
    def _get_token_position_choice_token(
        self, tokens: List[str], target_token: str
    ) -> int:
        ...
        for i in range(len(tokens) - 1, -1, -1):
            ...
            if (
                target_token
                and target_token.lower() == token_str
                or target_token.lower() in token_str
            ):
                target_position = i
                break
    ...
    def _get_token_position_continuation_token(
        self, tokens: List[str], target_token: str
    ) -> int:
    ...
    def _get_token_position_last_token(
        self, tokens: List[str], target_token: str
    ) -> int:
        """Always use the last token position."""
        return len(tokens) - 1

    def _get_token_position_first_token(
        self, tokens: List[str], target_token: str
    ) -> int:
    ...
    def _get_activation_with_strategy(
        ...
        tokens: List[str],
        target_token: str,
        strategy: TokenTargetingStrategy,
    ) -> torch.Tensor:
        """
        ...
        if strategy == TokenTargetingStrategy.CHOICE_TOKEN:
            ...
            print(
                f"   🎯 MEAN_POOLING: Using mean across all {hidden_states.shape[1]} tokens"
            )
            ...
            print(
                f"   🎯 MAX_POOLING: Using max across all {hidden_states.shape[1]} tokens"
            )
    ...
    def extract_activations_from_pair(
        ...
        def get_activation_at_target_token(
            full_prompt: str, target_token: str
        ) -> torch.Tensor:
        ...
        try:
            ...
            positive_activation = get_activation_at_target_token(
                positive_full_prompt, target_token
            )
            negative_activation = get_activation_at_target_token(
                negative_full_prompt, target_token
            )
    ...
    def collect_activations_batch(
        ...
        if pairs:
            ...
            print(
                f"   📝 Has _prompt_strategy: {hasattr(first_pair, '_prompt_strategy')}"
            )
            ...
            print(
                f"   📝 Prompt: {first_pair.prompt[:50]}..."
                if hasattr(first_pair, "prompt")
                else "   📝 No prompt attr"
            )
            print(
