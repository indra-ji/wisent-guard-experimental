...
class SteeringDataExtractor:
    ...
    def extract_contrastive_pairs(
        ...
        if strategy not in self.pair_generation_strategies:
            raise ValueError(
                f"Unknown strategy: {strategy}. Available: {list(self.pair_generation_strategies.keys())}"
            )
        ...
        for problem in problems:
            try:
                ...
                logger.warning(
                    f"Failed to generate pairs for problem {problem.task_id}: {e}"
                )
                continue

        logger.info(
            f"Generated {len(all_pairs)} contrastive pairs from {len(problems)} problems"
        )
        return all_pairs

    def _generate_correct_vs_incorrect(
        self, problem: LiveCodeBenchProblem, num_pairs: int
    ) -> List[ContrastivePair]:
...
Write a solution with off-by-one errors:""",
    ...
    def _generate_complete_vs_incomplete(
        self, problem: LiveCodeBenchProblem, num_pairs: int
    ) -> List[ContrastivePair]:
...
Write a solution that's missing error handling:""",
    ...
    def _generate_efficient_vs_inefficient(
        self, problem: LiveCodeBenchProblem, num_pairs: int
    ) -> List[ContrastivePair]:
...
Write a solution with unnecessary nested loops:""",
    ...
    def _generate_readable_vs_unreadable(
        self, problem: LiveCodeBenchProblem, num_pairs: int
    ) -> List[ContrastivePair]:
