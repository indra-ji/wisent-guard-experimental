...
class Model:
    ...
    def _detect_format(self) -> PromptFormat:
        ...
        if re.search(r"llama-?3", model_name, re.IGNORECASE):
            ...
            if user_model_configs.has_config(self.name):
                ...
                response = (
                    input("Would you like to configure it now? (y/n): ").strip().lower()
                )
                ...
                if response == "y":
                    ...
                    return (
                        PromptFormat.LEGACY
                    )  # User-configured models use legacy format
                else:
                    raise ValueError(
        ...
        if self.format_type == PromptFormat.LLAMA31:
            ...
            if user_model_configs.has_config(self.name):
                ...
                if tokens:
                    ...
                    self.assistant_token = tokens.get(
    ...
    def format_multiple_choice(
        self, question: str, choice_a: str, choice_b: str, answer: str
    ) -> str:
    ...
    def convert_to_multiple_choice(
        self, harmful_phrase: str, harmless_phrase: str
    ) -> Tuple[str, str]:
        """
    ...
    def get_num_layers(self) -> int:
        ...
        elif hasattr(self.hf_model, "transformer") and hasattr(
            self.hf_model.transformer, "h"
        ):
        ...
        for token_idx in range(max_new_tokens):
            ...
            with torch.no_grad():
                outputs = self.hf_model(
                    current_input, output_hidden_states=True, return_dict=True
                )
        ...
        while attempt <= max_regeneration_attempts:
            try:
                ...
                if latency_tracker:
                    ...
                    with latency_tracker.time_generation(
                        operation_name, prompt_length
                    ) as gen_state:
                        generated_text, activations, token_count = (
                ...
                if nonsense_detector is not None and nonsense_action != "flag":
                    ...
                    if result["is_nonsense"]:
                        if nonsense_action == "stop":
                            print(
                                f"⚠️ Nonsense detected, stopping generation: {', '.join(result['issues'])}"
                            )
                            return "", None
                        elif (
                            ...
                            generation_kwargs["temperature"] = (
                                generation_kwargs.get("temperature", 0.7) + 0.1
                            )
                            continue
                        else:
                            print(
                                "⚠️ Max regeneration attempts reached, returning potentially nonsensical response"
                            )
                ...
                if nonsense_detector is not None and nonsense_action == "flag":
                    ...
                    if result["is_nonsense"]:
                        generated_text = (
                            f"[FLAGGED: {', '.join(result['issues'])}] {generated_text}"
                        )
    ...
    def _generate_with_timing(
        ...
        **generation_kwargs,
        ...
        if gen_state:
            ...
            if enable_gradients:
                ...
                with torch.inference_mode():
                    first_outputs = self.hf_model.generate(
                        **inputs, **first_token_config
                    )
        ...
        if hasattr(outputs, "hidden_states") and outputs.hidden_states:
            ...
            if (
                isinstance(last_hidden_states, tuple)
                and len(last_hidden_states) > layer_index
            ):
    ...
    def extract_activations(self, text: str, layer: "Layer"):
        ...
        inputs = self.tokenizer(
            formatted_text, return_tensors="pt", padding=True, truncation=True
        )
        inputs = {k: v.to(self.hf_model.device) for k, v in inputs.items()}
        ...
        if layer.index + 1 < len(outputs.hidden_states):
            layer_hidden_states = outputs.hidden_states[
                layer.index + 1
            ...
            raise ValueError(
                f"Layer {layer.index} not found in model with {len(outputs.hidden_states)} layers"
            )
    ...
    def optimize_parameters(
        ...
        for i in range(min_len):
            phrase_pairs.append(
                {"harmful": harmful_texts[i], "harmless": harmless_texts[i]}
            )
        ...
        thresholds_to_test = [
            threshold_range[0]
            + i * (threshold_range[1] - threshold_range[0]) / (num_threshold_steps - 1)
            for i in range(num_threshold_steps)
        ]

        total_combinations = (
            len(layers_to_test) * len(steering_types) * len(thresholds_to_test)
        )
                ...
                for threshold in thresholds_to_test:
                    try:
                        ...
                        steering_method = SteeringMethod(
                            method_type=SteeringType(steering_type), device=self.device
                        )
                        ...
                        train_results = train_pair_set.train_classifier(
                            steering_method.classifier, layer_obj
                        )
    ...
    def load_lm_eval_task(
        self, task_name: str, shots: int = 0, limit: Optional[int] = None
    ):
        """
    ...
    def split_task_data(
        ...
        return _task_manager.split_task_data(
            task_data, split_ratio=split_ratio, random_seed=random_seed
        )
...
class ModelParameterOptimizer:
    ...
    def optimize_parameters(
        self, harmful_texts: List[str], harmless_texts: List[str], **kwargs
    ) -> Dict[str, Any]:
...
class ActivationHooks:
    ...
    def __init__(
        self, model: torch.nn.Module, layers: List[int], token_strategy: str = "last"
    ):
                ...
                if isinstance(hidden_states, torch.Tensor):
                    ...
                    if torch.is_grad_enabled() and hidden_states.requires_grad:
                        ...
                        self.layer_activations[layer_idx] = (
                            hidden_states[:, last_token_idx, :].clone().to(device)
                        )
                    else:
                        # Standard extraction - detach for memory efficiency
                        self.layer_activations[layer_idx] = (
                            hidden_states[:, last_token_idx, :]
                            .detach()
                            .clone()
                            .to(device)
                        )
