        ...
        try:
            ...
            if processed_pairs:
                ...
                if hasattr(sample_pair, '_prompt_pair') and sample_pair._prompt_pair:
                    logger.info(f"   🔸 Positive prompt: {sample_pair._prompt_pair.positive_prompt[:100]}...")
                    logger.info(f"   🔸 Negative prompt: {sample_pair._prompt_pair.negative_prompt[:100]}...")
                    logger.info(f"   🎯 Target token: {sample_pair._prompt_pair.target_token}")
                    logger.info(f"   📊 Prompt strategy: {sample_pair._prompt_strategy.value}")
                    ...
                    logger.info(f"   ✅ Positive response: {sample_pair.positive_response[:50]}...")
            ...
            logger.info(f"🎯 Using activation aggregation method: {activation_method.value} (from token_aggregation: {token_aggregation})")
