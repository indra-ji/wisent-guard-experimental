...
class LogLikelihoodsEvaluator:
    ...
    def evaluate_classifier_on_task(
        ...
        try:
            ...
            task_data = evaluation_model.load_lm_eval_task(
                task_name, shots=0, limit=num_samples
            )
            docs, _ = evaluation_model.split_task_data(
            ...
            qa_pairs = ContrastivePairSet.extract_qa_pairs_from_task_docs(
                task_name, task_data, docs
            )

            if not qa_pairs:
                return self._error_result(
                    f"No QA pairs could be extracted from task: {task_name}"
                )
            ...
            logger.info(
                "🔍 EVALUATION MODE: Using DIRECT_COMPLETION prompt strategy instead of MULTIPLE_CHOICE"
            )
            ...
            if not contrastive_pairs:
                return self._error_result(
                    "No contrastive pairs could be created from QA pairs"
                )
            ...
            targeting_strategy = targeting_strategy_mapping.get(
                token_aggregation, TokenTargetingStrategy.MEAN_POOLING
            )
            ...
            if not processed_pairs:
                return self._error_result(
                    "No activations could be extracted from contrastive pairs"
                )
            ...
            if processed_pairs:
                ...
                if (
                    hasattr(sample_pair, "positive_activations")
                    and sample_pair.positive_activations is not None
                ):
                    logger.info(
                        f"   ✅ Positive activations shape: {sample_pair.positive_activations.shape}"
                    )
                if (
                    hasattr(sample_pair, "negative_activations")
                    and sample_pair.negative_activations is not None
                ):
                    logger.info(
                        f"   ✅ Negative activations shape: {sample_pair.negative_activations.shape}"
                    )

                if hasattr(sample_pair, "_prompt_pair") and sample_pair._prompt_pair:
                    logger.info(
                        f"   🔸 Positive prompt: {sample_pair._prompt_pair.positive_prompt[:100]}..."
                    )
                    logger.info(
                        f"   🔸 Negative prompt: {sample_pair._prompt_pair.negative_prompt[:100]}..."
                    )
                    logger.info(
                        f"   🎯 Target token: {sample_pair._prompt_pair.target_token}"
                    )
                    logger.info(
                        f"   📊 Prompt strategy: {sample_pair._prompt_strategy.value}"
                    )
                    logger.info(
                        f"   🔍 Token targeting: {targeting_strategy.value} (evaluation mode)"
                    )
                elif hasattr(sample_pair, "prompt") and hasattr(
                    sample_pair, "positive_response"
                ):
                    logger.info(f"   🔸 Question prompt: {sample_pair.prompt[:100]}...")
                    logger.info(
                        f"   ✅ Positive response: {sample_pair.positive_response[:50]}..."
                    )
                    logger.info(
                        f"   ❌ Negative response: {sample_pair.negative_response[:50]}..."
                    )
                    logger.info(
                    ...
                    logger.info(
                        "   📍 ACTIVATION COLLECTION: Unknown format - investigating..."
                    )
                    logger.info(
            ...
            activation_method = self._map_token_aggregation_to_activation_method(
                token_aggregation
            )
            logger.info(
    ...
    def _evaluate_classifier_on_sample(
        ...
        try:
            ...
            positive_prediction = classifier.predict_proba(
                [positive_features.cpu().numpy()]
            )
            if (
                isinstance(positive_prediction, (list, tuple))
                and len(positive_prediction) > 0
            ):
            ...
            negative_prediction = classifier.predict_proba(
                [negative_features.cpu().numpy()]
            )
            if (
                isinstance(negative_prediction, (list, tuple))
                and len(negative_prediction) > 0
            ):
            ...
            logger.info(
                f"      ✅ Correct answer score: {float(positive_prediction):.3f} (lower = more truthful)"
            )
            logger.info(
                f"      ❌ Incorrect answer score: {float(negative_prediction):.3f} (higher = less truthful)"
            )
            logger.info(
