...
class ModelExporter:
    ...
    def __init__(self, output_dir: str = "./exports"):
        ...
        logger.info(
            f"Initialized ModelExporter with output directory: {self.output_dir}"
        )
    ...
    def export_huggingface_model(
        ...
        try:
            ...
            if include_steering and (
                model_path.endswith(".pt") or model_path.endswith(".pth")
            ):
                ...
                if os.path.isdir(model_path):
                    ...
                    if (
                        isinstance(steering_data, dict)
                        and "steering_vectors" in steering_data
                    ):
                        # Handle experiment results format
                        for layer_idx, vector in steering_data[
                            "steering_vectors"
                        ].items():
            ...
            metadata = {
                "export_info": {
                    ...
                    "steering_info": model.get_steering_info()
                    if include_steering
                    else None,
                }
            }
    ...
    def export_vllm_model(
        self, model_path: str, export_name: str, base_model_name: str = "distilgpt2"
    ) -> str:
        """
        ...
        export_path = self.export_huggingface_model(
            model_path, export_name, base_model_name
        )
    ...
    def create_benchmark_script(
        self, benchmark_type: str, model_path: str, output_dir: str, **kwargs
    ) -> str:
        """
        ...
        if benchmark_type == "livecodebench":
            script_content = self._create_livecodebench_script(
                model_path, output_dir, **kwargs
            )
        elif benchmark_type == "bigcode":
            script_content = self._create_bigcode_script(
                model_path, output_dir, **kwargs
            )
        elif benchmark_type == "humaneval":
            script_content = self._create_humaneval_script(
                model_path, output_dir, **kwargs
            )
        else:
    ...
    def _create_livecodebench_script(
        self, model_path: str, output_dir: str, **kwargs
    ) -> str:
...
"""
    ...
    def _create_bigcode_script(self, model_path: str, output_dir: str, **kwargs) -> str:
