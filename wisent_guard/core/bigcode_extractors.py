...
class HumanEvalExtractor(BenchmarkExtractor):
    ...
    def _create_incorrect_humaneval(
        self, correct_code: str, doc: Dict[str, Any]
    ) -> str:
...
class MBPPExtractor(BenchmarkExtractor):
    ...
    def _create_incorrect_mbpp(self, correct_code: str, test_list: List[str]) -> str:
        ...
        has_numeric = any(
            "assert" in test and any(c.isdigit() for c in test) for test in test_list
        )
        has_string = any(
            "assert" in test and ("'" in test or '"' in test) for test in test_list
        )
        has_list = any(
            "assert" in test and ("[" in test or "list" in test.lower())
...
class CodeXGLUEExtractor(BenchmarkExtractor):
    ...
    def extract_qa_pair(
        ...
        try:
            code = doc.get("code", "")
            description = (
                doc.get("docstring") or doc.get("nl") or doc.get("comment") or ""
            )
    ...
    def extract_contrastive_pair(
        ...
        try:
            ...
            incorrect_answer = self._create_incorrect_docs(
...
def get_bigcode_extractor(task_name: str) -> BenchmarkExtractor:
    ...
    if task_name in BIGCODE_EXTRACTORS:
        ...
        logger.warning(
            f"No specific extractor for {task_name}, using HumanEvalExtractor"
        )
