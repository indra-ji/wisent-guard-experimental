...
class ManagedCachedBenchmarks:
            ...
            try:
                ...
                if not hasattr(extractor, "extract_qa_pair"):
                    raise AttributeError(
                        f"Extractor for {benchmark} missing extract_qa_pair method"
                    )
    ...
    def get_task_samples(
        self, task_name: str, limit: int, force_fresh: bool = False
    ) -> List[Dict[str, Any]]:
        """
        ...
        if cached_count >= limit:
            ...
            logger.info(
                f"Incremental download: need {needed} more samples for '{task_name}'"
            )

            new_samples = self._download_samples(
                task_name, needed, start_offset=cached_count
            )
    ...
    def _download_samples(
        self, task_name: str, limit: int, start_offset: int = 0
    ) -> List[Dict[str, Any]]:
        """
        ...
        """
        logger.info(
            f"Downloading {limit} samples for '{task_name}' (offset: {start_offset})"
        )
    ...
    def _save_to_cache(self, task_name: str, samples: List[Dict[str, Any]]):
        ...
        logger.info(
            f"Saved {len(samples)} samples to cache for '{task_name}' in {len(chunks)} chunks"
        )
    ...
    def cache_status(self) -> Dict[str, Any]:
        ...
        total_samples = sum(
            info.samples_count for info in self._metadata.tasks.values()
        )
        total_size = sum(
            sum(
