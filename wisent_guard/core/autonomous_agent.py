...
class AutonomousAgent:
    ...
    def __init__(
        ...
        self.hpr_beta = (
            hpr_beta if hpr_beta != 1.0 else steering_config.get("beta", 1.0)
        )
        self.dac_dynamic_control = (
            dac_dynamic_control
            if dac_dynamic_control
            else steering_config.get("dynamic_control", False)
        )
        self.dac_entropy_threshold = (
            dac_entropy_threshold
            if dac_entropy_threshold != 1.0
            else steering_config.get("entropy_threshold", 1.0)
        )
        self.bipo_beta = (
            bipo_beta if bipo_beta != 0.1 else steering_config.get("beta", 0.1)
        )
        self.bipo_learning_rate = (
            bipo_learning_rate
            if bipo_learning_rate != 5e-4
            else steering_config.get("learning_rate", 5e-4)
        )
        self.bipo_epochs = (
            bipo_epochs
            if bipo_epochs != 100
            else steering_config.get("num_epochs", 100)
        )
        self.ksteering_num_labels = (
            ksteering_num_labels
            if ksteering_num_labels != 6
            else steering_config.get("num_labels", 6)
        )
        self.ksteering_hidden_dim = (
            ksteering_hidden_dim
            if ksteering_hidden_dim != 512
            else steering_config.get("hidden_dim", 512)
        )
        self.ksteering_learning_rate = (
            ksteering_learning_rate
            if ksteering_learning_rate != 1e-3
            else steering_config.get("learning_rate", 1e-3)
        )
        ...
        self.ksteering_alpha = (
            ksteering_alpha
            if ksteering_alpha != 50.0
            else steering_config.get("alpha", 50.0)
        )
        ...
        if steering_mode:
            print(
                f"   🔧 Steering mode enabled with {normalization_method} normalization"
            )
    ...
    async def initialize(
        ...
        self.marketplace = ClassifierMarketplace(
            model=self.model, search_paths=classifier_search_paths
        )
    ...
    async def respond_autonomously(
        ...
        if classifier_configs:
            print(
                f"   📊 Initializing diagnostics with {len(classifier_configs)} classifiers"
            )
            self.diagnostics = ResponseDiagnostics(
                model=self.model, classifier_configs=classifier_configs
            )
            ...
            print(
                "   ⚠️ No classifiers selected - proceeding without advanced diagnostics"
            )
        ...
        while attempt < max_attempts:
            ...
            improvement = await self.steering.improve_response(
                prompt, current_response, analysis
            )

            if improvement.success:
                print(
                    f"   Improvement successful! Score: {improvement.improvement_score:.2f}"
                )
    ...
    def _create_steering_method(self):
        ...
        if self.steering_method == "CAA":
            ...
            target_labels = [
                int(x.strip())
                for x in self.ksteering_target_labels.split(",")
                if x.strip()
            ]
            avoid_labels = [
                int(x.strip())
                for x in self.ksteering_avoid_labels.split(",")
                if x.strip()
            ]
    ...
    async def evaluate_response_quality(
        ...
        activations_tensor = self.model.extract_activations(
            response, classifier_params.optimal_layer
        )
        ...
        score = (
            prob_result[0] if isinstance(prob_result, (list, tuple)) else prob_result
        )

        # Use model to determine if this score represents acceptable quality
        acceptable = await self._model_determines_quality_threshold(
    ...
    async def _model_determines_quality_threshold(
        self, response: str, classifier_score: float
    ) -> bool:
        """
        ...
        result = self.model.generate(
            threshold_prompt, layer_index=15, max_new_tokens=20
        )
    ...
    async def _determine_classifier_parameters(
        ...
        result = self.model.generate(
            parameter_prompt, layer_index=15, max_new_tokens=150
        )
    ...
    async def _determine_steering_parameters(
        ...
        # Generate model response
        result = self.model.generate(
            steering_prompt, layer_index=15, max_new_tokens=150
        )
        response = result[0] if isinstance(result, tuple) else result
    ...
    def _parse_steering_params(self, response: str) -> "SteeringParams":
        ...
        try:
            ...
            print(
                f"   📋 Using defaults: method={method}, initial={initial}, increment={increment}, max={maximum}"
            )
    ...
    async def _get_or_determine_classifier_parameters(
        ...
        if stored_params:
            stored_params.reasoning = (
                f"Retrieved from parameter memory: {stored_params.reasoning}"
            )
            print(
                f"   📚 Using stored parameters (success rate: {getattr(stored_params, 'success_rate', 0.0):.2%})"
            )
            return stored_params

        # Step 2: Fall back to model determination
        print("   🧠 No stored parameters found, using model determination...")
        fresh_params = await self._determine_classifier_parameters(
            prompt, benchmark_names
        )
    ...
    async def _get_or_determine_steering_parameters(
        ...
        if stored_params:
            ...
            adjusted_strength = stored_params.initial_strength + (
                stored_params.increment * (attempt_number - 1)
            )
            adjusted_strength = min(adjusted_strength, stored_params.maximum_strength)

            stored_params.initial_strength = adjusted_strength
            stored_params.reasoning = (
                f"Retrieved from memory (adjusted): {stored_params.reasoning}"
            )
            print(
        ...
        fresh_params = await self._determine_steering_parameters(
            prompt, current_quality, attempt_number
        )
        fresh_params.reasoning = f"Model-determined: {fresh_params.reasoning}"
    ...
    def _get_stored_classifier_parameters(self, prompt: str) -> "ClassifierParams":
        ...
        try:
            ...
            params.aggregation_method = classifier_config.get(
                "aggregation_method", "last_token"
            )
            params.token_aggregation = classifier_config.get(
                "token_aggregation", "average"
            )
            ...
            params.early_stopping_patience = classifier_config.get(
                "early_stopping_patience", 10
            )
    ...
    def _get_stored_steering_parameters(
        self, prompt: str, attempt_number: int
    ) -> "SteeringParams":
        """
    ...
    def _classify_prompt_type(self, prompt: str) -> str:
        ...
        try:
            ...
            for prompt_type, keywords in prompt_classification.items():
                score = sum(
                    1 for keyword in keywords if keyword.lower() in prompt_lower
                )
                if score > 0:
    ...
    def _store_successful_parameters(
        ...
        try:
            ...
            print(
                f"   💾 Storing successful parameters for '{prompt_type}' (quality: {final_quality:.3f})"
            )
    ...
    async def _respond_with_quality_control_impl(
        ...
        classifier_params = await self._get_or_determine_classifier_parameters(
            prompt, benchmark_names
        )
        print(
        ...
        classifier_decision = (
            await self.decision_system.create_single_quality_classifier(
                task_analysis,
                classifier_params,
                time_budget_minutes=classifier_time_budget,
            )
        )
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            quality_result = await self.evaluate_response_quality(
                current_response, classifier, classifier_params
            )
            ...
            steering_params = await self._get_or_determine_steering_parameters(
                prompt, quality_result.score, attempt
            )
            steering_params_used = steering_params

            print(
                f"   🎛️ Steering: {steering_params.steering_method} "
                f"(strength {steering_params.initial_strength})"
            )
            ...
            try:
                steered_response = await self._generate_with_steering(
                    prompt, steering_params
                )
        ...
        final_quality = await self.evaluate_response_quality(
            current_response, classifier, classifier_params
        )
        ...
        if final_quality.acceptable:
            self._store_successful_parameters(
        ...
        print(
            f"   ⏰ Time used: {timeout_mgr.get_elapsed_time():.1f}s / {time_budget * 60:.1f}s"
        )

        return result

    async def _generate_with_steering(
        self, prompt: str, steering_params: "SteeringParams"
    ) -> str:
        """
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        successful_improvements = sum(
            1 for imp in self.improvement_history if imp.success
        )
        success_rate = (
            successful_improvements / total_improvements
...
async def demo_autonomous_agent():
    ...
    try:
        ...
        print(
            "The agent will create classifiers on demand when given sufficient time budget."
        )
