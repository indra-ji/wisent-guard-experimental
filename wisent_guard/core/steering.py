...
class SteeringMethod:
        ...
        **kwargs,
        ...
        if self.is_vector_based:
            if layer_index is None:
                raise ValueError(
                    "layer_index required for vector-based steering methods"
                )
    ...
    def apply_steering(
        self, activations: torch.Tensor, strength: float = 1.0
    ) -> torch.Tensor:
        """
    ...
    def enable_response_logging(
        self, log_file_path: str = "./harmful_responses.json"
    ) -> None:
        """
    ...
    def get_logged_responses(
        ...
        try:
            ...
            if category is not None:
                log_entries = [
                    entry for entry in log_entries if entry.get("category") == category
                ]
    ...
    def _get_layer_module(self, model, layer):
        ...
        try:
            ...
            elif hasattr(hf_model, "transformer") and hasattr(
    ...
    def _compute_steering_loss(
        self, batch_X, batch_y, layer_module, regularization_strength
    ):
        """
    ...
    def evaluate(self, contrastive_pair_set: ContrastivePairSet) -> Dict[str, Any]:
        ...
        false_positive_rate = (
            false_positives / len(pos_predictions) if pos_predictions else 0
        )
