...
sys.path.append(
    os.path.join(
        os.path.dirname(__file__), "..", "..", "core", "lm-harness-integration"
    )
)
...
try:
    ...
    def get_task_samples_for_analysis(task_name, num_samples):
        return {
            "task_name": task_name,
            "error": "Sample retrieval function not available",
            "samples": [],
        }
    ...
    if _llama_model_cache is None:
        ...
        try:
            ...
            _llama_model_cache = pipeline(
                ...
                device=0
                if device == "cuda"
                else (device if device != "auto" else None),
                max_new_tokens=1000,
                temperature=0.3,
                do_sample=True,
                pad_token_id=50256,
            ...
            print(
                "⚠️  transformers library not found. Install with: pip install transformers torch"
            )
            return None
...
def call_llama_instruct(
    prompt: str, model_path: str = "meta-llama/Llama-3.1-8B-Instruct"
) -> str:
    """
...
def parse_llama_response(response: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not result["cognitive_traits"]:
            ...
            if not tags_match:
                tags_match = re.search(
                    r"TAGS:\s*\[(.*?)\]", response, re.DOTALL | re.IGNORECASE
                )
            if not tags_match:
                tags_match = re.search(
                    r"TAGS:\s*(.*?)(?=\n[A-Z]|\n$|$)",
            ...
            if tags_match:
                ...
                )  # This already removes duplicates
    ...
    try:
        with open("tasks.json", "r") as f:
            ...
            for i in range(len(core_parts)):
                for j in range(i + 1, len(core_parts) + 1):
                    ...
                    matching_tasks = [
                        ...
                        if t == pattern
                        or t.startswith(pattern + "_")
                        or t.endswith("_" + pattern)
                    ]
            ...
            if len(name_parts) >= 3:
                ...
                matching_tasks = [
                    t for t in all_task_names if t.startswith(reduced_name)
                ]
                related_tasks.extend(matching_tasks)
                ...
                if len(name_parts) >= 3:
                    ...
                    matching_tasks = [
                        t for t in all_task_names if t.startswith(pattern)
                    ]
                    related_tasks.extend(matching_tasks)
...
def try_related_tasks(task_name: str, num_samples: int = 5) -> Dict[str, Any]:
    ...
    for attempt in range(min(3, len(related_tasks))):
        ...
        if "error" not in task_samples and task_samples.get("samples"):
            ...
            print(
                f"   ❌ Related task {random_task} failed: {task_samples.get('error', 'Unknown error')}"
            )
...
def generate_cognitive_tags(task_name: str, num_samples: int = 5) -> Dict[str, Any]:
    ...
    print(
        f"✅ Retrieved {task_samples['sampled_docs']} samples from {task_samples['total_docs']} total documents"
    )
...
def test_llama_analysis(task_name: str = "truthfulqa_mc1"):
    ...
    print(
        f"📊 Samples: {result['analysis']['samples_analyzed']}/{result['analysis']['total_samples']}"
    )
    ...
    if len(sys.argv) > 3 and sys.argv[3] == "verbose":
        ...
        print("-" * 40)
        print(
            result["llama_response"][:500] + "..."
            if len(result["llama_response"]) > 500
            else result["llama_response"]
        )
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "test":
        ...
        print("Usage:")
        print(
            "  python generate_tags.py test [task_name]  - Analyze task with Llama 3.1B Instruct"
        )
        print("Examples:")
