...
def process_all_tasks(start_index: int = 0, save_every: int = 1):
    ...
    for i in range(start_index, total_tasks):
        ...
        if processed_count % save_every == 0:
            ...
            elapsed_str = (
                f"{elapsed / 60:.1f}min" if elapsed > 60 else f"{elapsed:.1f}sec"
            )
            rate_str = (
                f"{rate * 60:.1f}/min" if rate * 60 > 1 else f"{rate * 3600:.1f}/hour"
            )

            print(f"⏱️  Elapsed: {elapsed_str}, ETA: {eta_str}, Rate: {rate_str}")
            save_tasks(tasks_data)
            print(
                f"📊 Stats: {updated_count} updated, {error_count} errors, {skipped_count} skipped"
            )
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print("Examples:")
        print(
            "  python process_all_tasks.py process 0 1     # Start from beginning, save every task"
        )
        print(
            "  python process_all_tasks.py process 100 5  # Start from task 100, save every 5"
        )
        print(
            "  python process_all_tasks.py resume          # Resume from where we left off"
        )
        sys.exit(1)
